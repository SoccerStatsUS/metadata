world = {

    'Ciudad Madero': 'CF Ciudad Madero',

    'German American S.C.': 'German-American SC',
    'Reading German Americans': 'Reading German-Americans',

    'Reading FC': 'Reading',


    'SAP': 'SAP FC',
    'Hitachi Centre SAP FC': 'SAP FC',

    'Supersport United': 'SuperSport United',

    'Vancouver Whitecaps U23': 'Vancouver Whitecaps U-23',
    'Vancouver Whitecaps FC U-23': 'Vancouver Whitecaps U-23',

    'Montreal Impact U23': 'Montreal Impact U-23',

    'Mérida FC': 'C.F. Mérida',
    'Altamira': 'Altamira FC',

    
    'St. Catherines Roma': 'St Catharines Roma Wolves',
    'St. Catharines Roma': 'St Catharines Roma Wolves',
    'St. Catharine Wolves': 'St Catharines Roma Wolves',
    'St Catharines Wolves': 'St Catharines Roma Wolves',

    'Sy. Catherines Heidelberg': 'St. Catherines Heidelberg',

    'Sebija Kitchener': 'Serbja Kitchener',
    'Srbija Kitchener': 'Serbja Kitchener',

    'Toronto Malta': 'Toronto Malta United',

    'Toronto Transit Commission': 'Toronto Transit',

    'Toronto Ukraina': 'Toronto Ukrainians',
    'Toronto Ukrainia': 'Toronto Ukrainians',

    'Operário-P': 'Operário-PR',
    'Operário/CG': 'Operário/CG-MT',

    'Uniao SJ': 'União São João',
    'Uniao S Joao': 'União São João',
    'União S. João': 'União São João',
    'Uniao São Joao': 'União São João',

    'União Nacional FC': 'União Nacional',

    'Paduano': 'Paduano EC',


    'RC Arbâa': 'RC Arbaâ',

    'USM Algér': 'USM Alger',
    'Ríver': 'River',

    'BidVest Wits': 'Bidvest Wits',
    'BidVest Wits University': 'Bidvest Wits',
    'Bidvest Wits University': 'Bidvest Wits',


    'Sanat-Naft': 'Sanat Naft',
    'Umm-Salal': 'Umm Salal',
    'Olímpia': 'Olimpia',
    'Internațional Curtea de Argeș': 'Internaţional Curtea de Argeş',
    'Western Mass. Pioneers': 'Western Mass Pioneers',

    'Gottschee Blau-weiss': 'Gottschee Blau-Weiss',
    'USM El-Harrach': 'USM El Harrach',
    'Ashantigold': 'AshantiGold',
    'JSM Bejaïa': 'JSM Béjaïa',
    'Galícia': 'Galicia',
    'Roumde Adjia': 'Roumdé Adjia',
    'Aracatuba': 'Araçatuba',
    'Ál-Nasr': 'Al-Nasr',
    'ES Setif': 'ES Sétif',
    'AS Ain M\'lila': 'AS Aïn M\'lila',



    'Amazulu': 'AmaZulu',
    'CR Bélouizdad': 'CR Belouizdad',


    'Ottawa Fury FC': 'Ottawa Fury',

    # indoor
    'Revolución Tijuana': 'Toros Mexico',

    # Algeria/Egypt

    'CA Bordj Bou Arréridj': 'CABB Arrédidj',
    'CA B. Bou Arreridj': 'CABB Arrédidj',
    'CABB Arreridj': 'CABB Arrédidj',
    'CA BB Arréridj': 'CABB Arrédidj',
    'CA Bordj Bou Arreridj': 'CABB Arrédidj',
    'CAB Bou Arréridj': 'CABB Arrédidj',
    'CAB B.Arreridj': 'CABB Arrédidj',
    'CABB Arréridj': 'CABB Arrédidj',

    'AS Ain-M\'Lila': 'AS Aïn M\'lila',
    'AS Ain-Mlila': 'AS Aïn M\'lila',
    'AS Ain-M\'lila': 'AS Aïn M\'lila',
    'AS Aïn M\'Lila': 'AS Aïn M\'lila',
    'AS Ain m\'lila': 'AS Aïn M\'lila',

    'JSM Bejaia': 'JSM Bejaïa',
    'JSM Béjaia': 'JSM Bejaïa',
    'USM Béjaia': 'JSM Bejaïa',

    # Egypt

    'Assyut Petrol': 'Asyut Petrol',










    'Univ. Centroamericana': 'Universidad Centroamericana',


    'ASJ Mana': 'AS Javouhey Mana',

    'Moskva XI': 'Moscow XI',

    'Centro Dominguito': 'RKSV Centro Dominguito',

    'Harlem Bombers': 'Harlem United',

    'Valencia Football Club': 'Valencia',

    'Ural Sverdlovsk Oblast': 'FC Ural',

    'Mordovia': 'Mordovia Saransk',

    'Boussu-Dour': 'Boussu Dour Borinage',

    'Ballenas': 'Galeana',

    'Arouca': 'FC Arouca',

    'FC Dynamo Moscow': 'Dynamo Moscow',

    'OH Leuven': 'Oud-Heverlee Leuven',

    'Alebrijes': 'Alebrijes de Oaxaca',



    'Pacifico FC': 'Pacífico FC',
    'San Martín (San Juan)': 'San Martín de San Juan',

    'C13.Portuguesa FC': 'Portuguesa FC',
    'Portuguesa FC': 'Portuguesa (Venezuela)',

    'U.T.C.': 'Universidad Técnica de Cajamarca',
    'San Agustín': 'San Agustin',

    'Estudiantes FC': 'Estudiantes',


    'AC Mineros': 'Mineros de Guayana',

    'Estudiantes Grau': 'Estudiantes de Medicina',

    'Godoy Cruz Antonio Tomba': 'Godoy Cruz',

    'FC Washington': 'Baltimore Bays',
    'Washington Stars': 'Baltimore Bays',




    'Sta Barbara': 'Santa Barbara',

    'C.A.I. FC': 'CAI',
    'C.A.I.': 'CAI',

    'CNFE': 'CNFE FF',

    'Montreal (CSL)': 'Montreal Supra',# fix in source.
    'Vancouver (CSL)': 'Vancouver 86ers', # fix in source.


    'Juvisy': 'Juvisy FC',
    'FCF Juvisy': 'Juvisy FC',
    'Juvisy FCF': 'Juvisy FC',
    
    'FF Nord Allier': 'Yzeure Allier Auvergne',

    'FCF Hénin-Beaumont': 'Hénin-Beaumont',
    'Hénin-B.': 'Hénin-Beaumont',
    'Hénin': 'Hénin-Beaumont',

    'Soyaux AJ': 'Soyaux ASJ',
    'Soyaux ASJ': 'ASJ Soyaux',

    'Trion': 'IF Trion',

    'KIF örebro DFF': 'KIF Örebro',
    'KIF Örebro DFF': 'KIF Örebro',
    'KIF Örebro IF DFF': 'KIF Örebro',

    'USO Bruay-laBuissière': 'USO Bruay-La-Buissière',

    'Montpellier-le Crès': 'Montpellier-Le Cres',

    'QBIK (Karlstad)': 'QBIK',
    'QBIK Karlstad': 'QBIK',

    'St-Brieuc FF': 'Saint-Brieuc',
    'Saint-Brieuc FF': 'Saint-Brieuc',
    'Saint-Brieuc SC': 'Saint-Brieuc',
    'St-Brieuc': 'Saint-Brieuc',

    'Saint-Memmie': 'Saint-Memmie Olympique',
    'Saint-Memmie Ol.': 'Saint-Memmie Olympique',
    'St.-Memmie Olympique': 'Saint-Memmie Olympique',
    'St-Memmie': 'Saint-Memmie Olympique',

    'TOAFC Toulouse': 'Toulouse OAC',

    'BK Kenty FF': 'BK Kenty',
    'Kenty': 'BK Kenty',

    'CNFE Football Fém.': 'CNFE FF',
    'CNFE Football Féminin': 'CNFE FF',

    'Djurgårdens IF D': 'Djurgårdens DFF',
    'Djurgårdens IF DFF': 'Djurgårdens DFF',
    'Djurgårdens IF Dam': 'Djurgårdens DFF',

    'ESOFV La Roche-sur-Yon': 'ESOF La Roche',
    'La Roche/Yon ESOF': 'ESOF La Roche',
    'La Roche-sur-Yon ESOF': 'ESOF La Roche',
    'La Roche': 'ESOF La Roche',
    'ESOF la Roche': 'ESOF La Roche',

    'Hammarby IF DFF (Stockholm)': 'Hammarby IF DFF',



    'Jitex BK (Mölndal)': 'Jitex BK',

    'Kopparbergs/Göteborg': 'Kopparbergs/Göteborg FC',
    'Kopparbergs/ Göteborg': 'Kopparbergs/Göteborg FC',
    'Kopparberg/Göteborg': 'Kopparbergs/Göteborg FC',

    'Umea': 'Umeå IK',
    'Umeå': 'Umeå IK',
    'Umea IK': 'Umeå IK',

    'Tyreso FF': 'Tyresö FF',

    'Djurgården/Älvsjö': 'Djurgårdens IF Dam',



    'Kopparsberg/L': 'Kopparbergs/Göteborg',
    'Kopparsberg/Landvetter': 'Kopparbergs/Göteborg',
    'Kopparsberg/L.': 'Kopparbergs/Göteborg',
    'Kopparbergs/Landvetter': 'Kopparbergs/Göteborg',
    'K/Landvetter': 'Kopparbergs/Göteborg',
    'Kopparberg': 'Kopparbergs/Göteborg',

    'Alingsås': 'HIF Alingsås',

    'Kristianstad/Wa': 'Kristianstads DFF',
    'Kristianstad/Wä': 'Kristianstads DFF',

    'Mallbacken': 'Mallbackens IF',
    'Mallbackens': 'Mallbackens IF',

    'Bälinge': 'Bälinge IF',
    'Balinge': 'Bälinge IF',
    'Bälinge IF (Uppsala)': 'Bälinge IF',
    'Bälinge IF (Upplands Bälinge)': 'Bälinge IF',

    'Sunnanå SK (Skellefteå)': 'Sunnanå SK',
    'Sunnanå': 'Sunnanå SK',

    'Själevads': 'Själevad',

    'Linkopings FC': 'Linköpings FC',
    'Linköpings': 'Linköpings FC',
    'Linköping': 'Linköpings FC',

    'Alvsjo': 'Älvsjö AIK FF',
    'Älvsjö': 'Älvsjö AIK FF',

    'Alviks': 'Alviks IK',
    'Alvik': 'Alviks IK',

    'St. Catharines': 'St Catharines', 
    'VFR': 'VfR', 
    'U.S.S.R. Olympic': 'USSR Olympic',

    #'Internaţional Curtea de Argeş': 'Internațional Curtea de Argeș',
    'Jiul Petroşani': 'Jiul Petroșani',
    'Gloria Bistriţa': 'Gloria Bistrița',
    'Atletico Pompeya': 'Atlético Pompeya',

    'Santa Fé': 'Santa Fe',

    'Paysandú': 'Paysandu',

    'Al Hilal': 'Al-Hilal',
    'Etoile Sahel': 'Étoile du Sahel',

    'KS Flamurtari Vlorë': 'Flamurtari Vlorë',

    '1. FFC Turbine Potsdam': 'Turbine Potsdam',
    '1. FFC Frankfurt': 'FFC Frankfurt',
    'FFC Brauweiler Pulheim 2000': 'FFC Brauweiler',

    'TuS Niederkirchen': '1. FFC 08 Niederkirchen',


    'Sjalevad': 'Själevad',



    'Vorwärts Berlin (GDR)': 'Vorwärts Berlin',
    'ASK Vorwärts': 'Vorwärts Berlin',
    'Vorwärts Berlin': 'Frankfurter FC Viktoria', # name map.

    'Motor Jena': 'Carl Zeiss Jena',

    # Russia

    'Luch-Energiya Vladivostok': 'Luch-Energiya',

    'FK Moskva': 'FC Moscow',

    'Lokomotiv Moskva': 'Lokomotiv Moscow',
    'Lokomotiv M': 'Lokomotiv Moscow',

    'Sibir': 'Sibir Novosibirsk',

    'Terek': 'Terek Grozny',

    'Zenit': 'Zenit Saint Petersburg',

    
    'Anzhi': 'Anzhi Makhachkala',

    'Spartak N': 'Spartak Nalchik',

    'Tom': 'Tom Tomsk',

    'Kuban': 'Kuban Krasnodar',

    'Amkar': 'Amkar Perm',

    'Spartak M': 'Spartak Moscow',

    'Rubin': 'Rubin Kazan',

    'Krasnodar': 'FC Krasnodar',

    'Rostov': 'FC Rostov',
    'FK Rostov': 'FC Rostov',

    'Krylya Sovetov Samara': 'Krylia Sovetov',
    'Krylia Sovetov Samara': 'Krylia Sovetov',
    'Krylya S': 'Krylia Sovetov',
    'Krylya So': 'Krylia Sovetov Samara',
    'Krylya Sov': 'Krylia Sovetov',
    'Krylya Sovetov': 'Krylia Sovetov',


    'Volga': 'Volga Nizhny Novgorod',

    'FK Ufa': 'FC Ufa',




    # Misc


    # Scandinavia


    'Enköpings SK FK': 'Enköpings SK',
    'Lillestrøm SK': 'Lillestrøm',
    'Kongsvinger IL': 'Kongsvinger',

    'Sarpsborg 08 FF': 'Sarpsborg FK',
    'Randers FC': 'Randers',
    'IF Brommapojkarna': 'Brommapojkarna',
    'Hvidovre': 'Hvidovre IF',

    'Sandefjord Fotball': 'Sandefjord',

    'Molde FK': 'Molde',

    'Mjällby AIF': 'Mjällby',



    'Al Wahda': 'Al-Wahda',
    'Al-Sadd Club': 'Al-Sadd',

    # Mexico

    'Morelia Sub-17': 'Morelia U-17',
    'Chivas Sub-17': 'Chivas U-17',
    'Atlas Sub-17': 'Atlas U-17',
    'San Luis Sub-17': 'San Luis U-17', #?
    'Cruz Azul Sub-17': 'Cruz Azul U-17',
    'Necaxa Sub-17': 'Necaxa U-17',
    'Club Tijuana Sub-17': 'Club Tijuana U-17',
    'Santos Sub-17': 'Santos Laguna U-17',
    'Monterrey Sub-17': 'Monterrey U-17',

    'Pachuca Sub-20': 'Pachuca U-20',
    'Indios Sub-20': 'Indios U-20',
    'Cruz Azul Sub-20': 'Cruz Azul U-20',
    'Necaxa Sub-20': 'Necaxa U-20',
    'Tigres Sub-20': 'Tigres U-20',
    'Monterrey Sub-20': 'Monterrey U-20',
    'Atlante Sub-20': 'Atlante U-20',
    'América Sub-20': 'Club America U-20',
    'Atlas Sub-20': 'Atlas U-20',

    'Estudiantes Sub-17': 'Estudiantes U-17',
    'Toluca Sub-17': 'Toluca U-17',

    'Estudiantes Sub-20': 'Estudiantes U-20',
    'Toluca Sub-20': 'Toluca U-20',
    'Club Tijuana Sub-20': 'Club Tijuana U-20',
    'Morelia Sub-20': 'Morelia U-20',

    'Atlante Sub-17': 'Atlante U-17',

    'Puebla Sub-20': 'Puebla U-20',

    'Tigres Sub-17': 'Tigres U-17',
    'Pumas Sub-17': 'UNAM U-17',

    'Pumas Sub-20': 'UNAM U-20',

    'Jaguares Sub-17': 'Jaguares U-17',
    'Puebla Sub-17': 'Puebla U-17',

    'San Luis Sub-20': 'San Luis U-20', # specific

    # Congo

    'Tp Mazembe': 'TP Mazembe',

    # Cayman Islands

    'Bodden Town FC - Cayman Island': 'Bodden Town',



    'América Cochahuayco': 'U América FC', # name map.

    'Valledupar FC': 'Valledupar',

    'Deportes Pereira': 'Deportivo Pereira',
    'Dep. Pereira': 'Deportivo Pereira',
    'Pereira': 'Deportivo Pereira',

    'Llaneros FC (Villavicencio)': 'Llaneros FC',

    'Patriotas FC (Tunja)': 'Patriotas FC',

    'Real Santander CD (Floridablanca)': 'Real Santander',

    'Expreso Rojo (Fusagasugá)': 'Expreso Rojo',

    'Magdalena': 'Unión Magdalena',
    'Unión Magdalena (Santa Marta)': 'Unión Magdalena',
    'Un. Magdalena': 'Unión Magdalena',
    'Union Magdalena': 'Unión Magdalena',

    'Mogi-Mirim': 'Mogi Mirim',

    'AbooMoslem': 'Aboumoslem',
    'Aboo-Moslem': 'Aboumoslem',

    'Envigado FC': 'Envigado',

    'Teraktor-Sazi': 'Tractor Sazi',
    'TeraktorSazi': 'Tractor Sazi',

    'Ĺtvidabergs FF (Swe)': 'Åtvidabergs FF',
    'Åtvidaberg': 'Åtvidabergs FF',
    'Ĺtvidabergs FF': 'Åtvidabergs FF',


    'Aalesund FK': 'Aalesund',
    'Aalesunds FK': 'Aalesund',

    # Ukraine?

    'Shakhtar D.': 'Shakhtar Donetsk',
    'Shakhtar Donets\'k': 'Shakhtar Donetsk',
    'Dynamo Kyiv': 'Dynamo Kiev',
    'Dnipro Dnipropetrovs\'k': 'Dnipro Dnipropetrovsk',
    'PFC Oleksandrija': 'PFC Oleksandriya',
    'Tavrija Simferopol': 'Tavriya Simferopol',
    'Kryvbas Kryvyj Rih': 'Kryvbas Kryvyi Rih',
    'Karpaty L\'viv': 'Karpaty Lviv',
    'Karpaty L\'viv': 'Karpaty Lviv',                          
    'Metalurh Donets\'k': 'Metalurh Donetsk',

    'Zorja Luhansk': 'Zorya Luhansk',
    'Zorja Luhans\'k': 'Zorya Luhansk',
    'Zaria Voroshilovgrad': 'Zorya Luhansk',

    'Cork Celtic (Irl)': 'Cork Celtic',

    'Portadown (Nir)': 'Portadown F.C.',

    'St.Patrick\'s Athletic (Irl)': 'St. Patrick\'s Athletic',
    'St. Patrick\'s Athletic (Irl)': 'St. Patrick\'s Athletic',

    'Vitosha Sofia': 'Levski Sofia',

    'TPS Turku': 'Turun Palloseura',

    'IBV Vestmannaeyjar': 'ÍBV Vestmannaeyjar',

    'Ilves Tampere': 'Ilves',

    'OPS Oulu': 'Oulun Palloseura',

    'Kuusysi Lahti': 'FC Kuusysi',

    'Coleraine (Nir)': 'Coleraine F.C.',



    'FC Magdeburg (GDR)': '1. FC Magdeburg',
    'Magdeburg (GDR)': '1. FC Magdeburg',

    'Zeljeznicar Sarajevo': 'Željezničar Sarajevo',

    'KuPS Kuopio': 'KuPS',

    'Floriana (Mlt)': 'Floriana',

    'Shelbourne (Irl)': 'Shelbourne',


    'HPS (Helsinki-Fin)': 'Helsingin Palloseura',

    'Kawasaki F.': 'Kawasaki Frontale',
    'G.Osaka': 'Gamba Osaka',

    #'Kashiwa Antlers': 'Kashima Antlers',
    'Consadore Sapporo': 'Consadole Sapporo',

    'Bellmare Hiratsuka': 'Shonan Bellmare',

    'S Lorenzo': 'San Lorenzo',
    #'San Lorenzo': 'San Lorenzo de Almagro', # ?

    'LDU de Loja': 'LDU Loja',

    'Primero de Mayo': '1 de Mayo',
    '1° de Mayo (Beni)': '1 de Mayo',

    'Dep Quito': 'Deportivo Quito',
    'Dep. Quito': 'Deportivo Quito',

    'Delfin': 'Delfín',

    'Dep Cuenca': 'Deportivo Cuenca',
    'Dep. Cuenca': 'Deportivo Cuenca',
    'Cuenca': 'Deportivo Cuenca',

    'Indep. José Terán': 'Independiente José Terán',

    'Tacaury': 'Tacuary',


    'Tecnico Univ.': 'Tecnico Universitario',
    'Tecnico Univ': 'Tecnico Universitario',
    'Tecnico U': 'Tecnico Universitario',
    'Tecnico': 'Tecnico Universitario',

    'G Cross': 'Green Cross',

    'Republicana': 'Guardia Republicana',


    'Atlético Pompeya (Beni)': 'Atlético Pompeya',

    'IMI F.C.': 'IMI',

    'Wanka': 'Deportivo Wanka',
    'Deportivo Sipesa': 'Deportivo Wanka',


    'Independiente (CG)': 'Independiente de Campo Grande',
    'Independiente CG': 'Independiente de Campo Grande',

    'CNI': 'Colegio Nacional Iquitos',

    'C.N.I.': 'Colegio Nacional Iquitos',

    'Saturn': 'Saturn Ramenskoye',
    'Saturn Ren TV': 'Saturn Ramenskoye',
    'Saturn Moskovskaya Oblast': 'Saturn Ramenskoye',

    'Sp.San Lorenzo': 'Sportivo San Lorenzo',
    'Sp. San Lorenzo': 'Sportivo San Lorenzo',

    'A. Atlético': 'Alianza Atlético', 
    'Alanza Atlético': 'Alianza Atlético',
    'Alianza Atl': 'Alianza Atlético', 
    'Alianza Atl.': 'Alianza Atlético',

    'Iberoamericano': 'Iberoamericana',
    'Iberoamericana': 'Universidad Iberoamericana FC',

    'Medicina': 'Estudiantes de Medicina',

    'Bolognesi': 'FC Bolognesi',
    'Bolognesi FC': 'FC Bolognesi',

    #'Coronel Bolognesi',
    'Coronel Bolognesi (Tacna)': 'Coronel Bolognesi',

    'General Caballero (ZC)': 'General Caballero',
    'Gral. Caballero ZC': 'General Caballero',
    'General Caballero ZC': 'General Caballero',

    'Real Sta Cruz': 'Real Santa Cruz',
    'Real Sta. Cruz': 'Real Santa Cruz',

    'Grau-Estudiantes': 'Estudiantes Grau',
    'Grau Estudiantes': 'Estudiantes Grau',
    'Grau Estudiantes de Medicina': 'Estudiantes Grau',

    'Tres de Febrero': '3 de Febrero',

    'Oriente': 'Oriente Petrolero', 
    'Santa Cruz Oriente': 'Oriente Petrolero',

    'Strongest': 'The Strongest',

    'Jorge Wilstermann': 'Wilstermann',

    'Aurich': 'Juan Aurich',
    'Juan Aurich (Chiclayo)': 'Juan Aurich',



    'P.Hayes': 'Presidente Hayes',
    'P Hayes': 'Presidente Hayes',
    'P. Hayes': 'Presidente Hayes',


    'Inti Gas Deportes': 'Inti Gas',
    'Inti Gas Deportes (Ayacucho)': 'Inti Gas',
    'Inti Gas Deportes (Ica)': 'Inti Gas',

    'Universidad César Vallejo (Trujillo)': 'Universidad Cesar Vallejo',
    'Universidad CV': 'Universidad Cesar Vallejo',
    'César Vallejo':  'Universidad Cesar Vallejo',
    'Univ.Cesar Vallejo': 'Universidad Cesar Vallejo',
    'Univ. Cesar Vallejo': 'Universidad Cesar Vallejo',
    'Univ. César Vallejo': 'Universidad Cesar Vallejo',

    
    'Galvez': 'José Gálvez',
    'Jose Galvez': 'José Gálvez',
    'José Gélvez': 'José Gálvez',
    'José Galvez': 'José Gálvez',
    'CS Jose Galez': 'José Gálvez',
    'José Gálvez FBC (Chimbote)': 'José Gálvez',
    'José Gálvez (Chimbote)': 'José Gálvez',
    'Jose Galvez FBC': 'José Gálvez',
    'José Gálvez FBC': 'José Gálvez',


    'AS La Jeunesse D\'Esch': 'Jeunesse d\'Esch',

    'FC Luzern (Sui)': 'FC Luzern',

    'Etar Veliko Tarnovo (Bul)': 'Etar Veliko Tarnovo',
    'Etar Veliko Tarnovo': 'Etar 1924',

    'KS Flamurtari Vlorë (Alb)': 'KS Flamurtari Vlorë',

    'EPA Larnaca (Cyp)': 'EPA Larnaca',


    'Vllaznia Shkodër (Alb)': 'Vllaznia Shkodër',

    'Rabat Ajax (Mlt)': 'Rabat Ajax',

    'FC Beroe Stara Zagora (Bul)': 'FC Beroe Stara Zagora',
    'FC Beroe Stara Zagora': 'Beroe',

    'Pezoporikos Larnaca (Cyp)': 'Pezoporikos Larnaca',

    'Servette FC (Sui)': 'Servette FC',
    'Servette FC Genčve (Sui)': 'Servette FC',

    'AS La Jeunesse D\'Esch (Lux)': 'AS La Jeunesse D\'Esch',

    'CH Bratislava (Tch)': 'CH Bratislava',

    'Csepel SC (Hun)': 'Csepel SC',



    'Ilves Tampere (Fin)': 'Ilves Tampere',

    'Labinoti Elbasan (Alb)': 'Labinoti Elbasan',
    'Labinoti Elbasan': 'KF Elbasani',

    'Trakia Plovdiv (Bul)': 'Trakia Plovdiv',

    'TJ Vítkovice': 'FC Vítkovice',
    'TJ Vítkovice (TCH)': 'FC Vítkovice',
    'TJ Vítkovice':'FC Vítkovice',
    'SSK Vitkovice': 'FC Vítkovice',
    'FC Vitkovice': 'FC Vítkovice',


    'Vardar Skopje (YUG)': 'Vardar Skopje',
    'Vardar Skopje': 'FK Vardar',

    'Crusaders Belfast (Nir)': 'Crusaders Belfast',

    'Bohemians Dublin (Irl)': 'Bohemians Dublin',
    'Bohemians Dublin': 'Bohemians FC',

    'SK Beveren Waas': 'KSK Beveren',
    'K.S.K. Beveren': 'KSK Beveren',
    'SK Beveren (Bel)': 'KSK Beveren',
    'SK Beveren': 'KSK Beveren',

    'Hamrun Spartans': 'Ħamrun Spartans',

    'Trakia Plovdiv': 'Botev Plovdiv',

    'Red Boys Differdange (Lux)': 'Red Boys Differdange',

    'Dinamo Tbilisi (URS)': 'Dinamo Tbilisi',

    'Start Kristiansand (Nor)': 'Start Kristiansand',
    'Start Kristiansand': 'IK Start',
    'Start': 'IK Start',



    'Ararat Yerevan (URS)': 'Ararat Yerevan',



    'Stal Mielec (Pol)': 'Stal Mielec',

    'ASK Vorwärts (GDR)': 'ASK Vorwärts',

    'Zaria Voroshilovgrad (URS)': 'Zaria Voroshilovgrad',

    'Viking Stavanger (Nor)': 'Viking Stavanger',

    'Zeljeznicar Sarajevo (YUG)': 'Zeljeznicar Sarajevo',



    'IA Akranes (Isl)': 'IA Akranes',

    'FK Sarajevo (YUG)': 'FK Sarajevo',

    'KuPS Kuopio (Fin)': 'KuPS Kuopio',



    'Avenir Beggen (Lux)': 'Avenir Beggen',

    'KPV Kokkola (Fin)': 'KPV Kokkola',

    'Carl Zeiss Jena (GDR)': 'Carl Zeiss Jena',
    'Motor Jena (GDR)': 'Motor Jena',

    'Aris Bonnevoie (Lux)': 'Aris Bonnevoie',

    'Sliema Wanderers (Mlt)': 'Sliema Wanderers',

    'La Chaux-de-Fonds (Sui)': 'La Chaux-de-Fonds',

    'Reipas Lahti (Fin)': 'Reipas Lahti',

    'Keflavik (Isl)': 'Keflavik',


    'Omonia Nicosia (Cyp)': 'Omonia Nicosia',

    'Hvidovre IF (Den)': 'Hvidovre IF',

    'Levski-Spartak Sofia (Bul)': 'Levski-Spartak Sofia',

    'Slovan Bratislava (TCH)': 'Slovan Bratislava',



    'Valkeakosken Haka (Fin)': 'FC Haka',
    'Valkeakosken Haka': 'FC Haka',
    'Haka Valkeakoski (Fin)': 'FC Haka',


    'Jeunesse d\'Esch (Lux)': 'Jeunesse d\'Esch',


    'Distillery Belfast (Nir)': 'Distillery Belfast',

    'HIFK Helsinki (Fin)': 'HIFK Helsinki',
    'HIFK (Fin)': 'HIFK Helsinki',




    'Anorthosis Famagusta (Cyp)': 'Anorthosis Famagusta',

    'UT Arad (Rom)': 'UT Arad',
    'U.T. Arad': 'UT Arad',
    'U.T.A. (Arad)': 'UT Arad',
    'U.T.Arad': 'UT Arad',
    'FCM UTA Arad': 'UT Arad',



    'CSKA Moskva': 'CSKA Moscow',
    'Cska Moskva': 'CSKA Moscow',
    'CSKA Moscow (URS)': 'CSKA Moscow',

    'Hibernians (Mlt)': 'Hibernians (Malta)',
    'Hibernians FC (Mlt)': 'Hibernians (Malta)',


    'Cork Hibernians (Irl)': 'Cork Hibernians',

    'Dynamo Dresden (GDR)': 'Dynamo Dresden',

    'APOEL Nicosia (Cyp)': 'APOEL Nicosia',

    'HJK Helsinki (Fin)': 'HJK Helsinki',
    'HJK Helsinki': 'HJK',



    'Valletta FC (Mlt)': 'Valletta FC',

    'Spartak Plovdiv (Bul)': 'Spartak Plovdiv',


    'Botev Plovdiv (Bul)': 'Botev Plovdiv',

    'Skeid Oslo (Nor)': 'Skeid Oslo',
    'Skeid Oslo': 'Skeid',

    'Levski-Spartak Sofia': 'Levski Sofia',

    'Distillery Belfast': 'Lisburn Distillery',

    'Spartak Moskva': 'Spartak Moscow',
    'Spartak Moscow (URS)': 'Spartak Moscow',

    'Kuusysi Lahti (Fin)': 'Kuusysi Lahti',

    'Hamrun Spartans (Mlt)': 'Hamrun Spartans',

    'IBV Vestmannaeyjar (Isl)': 'IBV Vestmannaeyjar',

    'OPS Oulu (Fin)': 'OPS Oulu',

    'Fram Reykjavik (Isl)': 'Fram Reykjavik',

    'KR Reykjavik (Isl)': 'KR Reykjavik',

    'Chemie Leipzig (GDR)': 'Chemie Leipzig',

    'Lokomotiv Sofia (Bul)': 'Lokomotiv Sofia',

    'DWS Amsterdam (Ned)': 'DWS Amsterdam',

    'Stade Dudelange (Lux)': 'Stade Dudelange',

    'Vojvodina Novi Sad (YUG)': 'Vojvodina Novi Sad',

    'Olympiakos Nicosia (Cyp)': 'Olympiakos Nicosia',





    'Athlone Town (Irl)': 'Athlone Town',

    'Vikingur Reykjavik (Isl)': 'Vikingur Reykjavik',
    'Vikingur Reykjavik': 'Vikingur',


    'Dnepr Dnepropetrovsk (URS)': 'Dnepr Dnepropetrovsk',
    'Dnepr Dnepropetrovsk': 'Dnipro Dnipropetrovsk',



    'Partizani Tirana (Alb)': 'Partizani Tirana',

    '17 Nëntori Tirana': 'KF Tirana',
    '17 Nëntori Tirana (Alb)': 'KF Tirana',
    '17 Nentori Tirana (Alb)': 'KF Tirana',
    'Nentori Tirana (Alb)': 'KF Tirana',
    '17. Nëntori Tirana': 'KF Tirana',

    'Dinamo Tirana (Alb)': 'Dinamo Tirana',

    'Olympiakos Piraeus (Gre)': 'Olympiakos Piraeus',
    'Olympiakos Piraeus': 'Olympiakos',
    'Olympiakos S.F.P. Pireas': 'Olympiakos',
    'Olympiakos (Gre)': 'Olympiakos',

    'Sparta Rotterdam (Ned)': 'Sparta Rotterdam',

    'Drumcondra (Irl)': 'Drumcondra',
    'Drumcondra FC (Irl)': 'Drumcondra',


    'Levski Sofia (Bul)': 'Levski Sofia',

    'TPS Turku (Fin)': 'TPS Turku',

    'KA Akureyri (Isl)': 'KA Akureyri',


    'Vitosha Sofia (Bul)': 'Vitosha Sofia',


    'Nk Dinamo Zagreb': 'Dinamo Zagreb',
    'NK Dinamo Zagreb (Yug)': 'Dinamo Zagreb',
    'NK Dinamo (Zagreb-Yug)': 'Dinamo Zagreb',
    'Dinamo Zagreb (YUG)': 'Dinamo Zagreb',
    'NK Dinamo Zagreb': 'Dinamo Zagreb',

    'Spartak Trnava (TCH)': 'Spartak Trnava',

    'CDNA (Sofia)': 'CDNA Sofia',
    'CDNA (Bul)': 'CDNA Sofia',
    'CDNA (Sofia-Bul)': 'CDNA Sofia',
    'CDNA Sofia (Bul)': 'CDNA Sofia',
    'CDNA Sofia': 'CSKA Sofia',
    'Cska Sofia': 'CSKA Sofia',
    'CSKA Sofia (Bul)': 'CSKA Sofia',

    'Sredets Sofia (Bul)': 'Sredets Sofia',
    'Sredets Sofia': 'CSKA Sofia',


    'Derry City (Irl)': 'Derry City',
    'Derry City (Nir)': 'Derry City',

    'PAOK Thessaloniki (Gre)': 'PAOK Thessaloniki',

    'Sligo Rovers (Irl)': 'Sligo Rovers',


    'Shamrock Rovers (Irl)': 'Shamrock Rovers',

    'AC Perugia': 'Perugia',
    'AC Venezia': 'Venezia',
    'AS Livorno Calcio': 'Livorno',

    # MISC

    'Stabæk Fotball': 'Stabæk',


    # France

    'Gfcoa': 'GFC Ajaccio',

    'Olympique Lyonnais': 'Lyon',
    'Olymp. Lyonnais': 'Lyon',
    'Olympique Lyonn.': 'Lyon',
    'Olympique Lyon': 'Lyon',
    'Lyon FC': 'Lyon',

    'Lyon Féminin FC': 'Lyon FCF',
    'FC Lyon Féminin': 'Lyon FCF',



    'AS Cannes': 'Cannes',

    'Stade Brestois': 'Brest',
    'Stade Brestois 29': 'Brest',

    'Nantes FC': 'Nantes',
    'FC Nantes (Fra)': 'Nantes',

    'Ac Arles Avignon': 'Arles-Avignon',
    'AC Arles-Avignon': 'Arles-Avignon',

    'AC Ajaccio': 'Ajaccio',
    'AJ Auxerre': 'Auxerre',

    'As Nancy Lorraine': 'AS Nancy',
    'AS Nancy Lorraine': 'AS Nancy',
    'AS Nancy-Lorraine': 'AS Nancy',

    'Saint Etienne': 'Saint-Étienne',
    'AS Saint-Etienne': 'Saint-Étienne',
    'AS Saint-Étienne (Fra)': 'Saint-Étienne',
    'RC Saint-Etienne': 'Saint-Étienne',

    'ATAC Troyes': 'Troyes',
    'ES Troyes AC': 'Troyes',

    'En Avant Guingamp': 'Guingamp',


    'FC Metz': 'Metz',

    'FC Nantes Atlantique': 'Nantes',
    'FC Nantes': 'Nantes',

    'FC Sochaux-Montbéliard': 'FC Sochaux',
    
    'Havre AC': 'Havre',

    'Grenoble Foot 38': 'Grenoble',

    'Le Havre AC': 'Havre',

    'Le Mans UC 72': 'Le Mans',
    'Le Mans UC72': 'Le Mans',

    'Montpellier-Hérault SC': 'Montpellier Herault SC',
    'Montpellier Hsc': 'Montpellier Herault SC',
    'Montpellier HSC': 'Montpellier Herault SC',
    'Montpellier-Herault': 'Montpellier Herault SC',
    'Montpellier-Hérault': 'Montpellier Herault SC',
    
    'Marsella': 'Marseille',
    'Olympique de Marseille': 'Marseille',
    'Olympique Marseille (Fra)': 'Marseille',
    'Olympique Marseille': 'Marseille',

    'Marseille FC': 'Marseille',

    'Paris-St.-Germain FC': 'Paris Saint-Germain',
    'Paris Saint-Germain - France': 'Paris Saint-Germain',
    'Paris Saint-Germain FC': 'Paris Saint-Germain',
    'Paris-Saint-Germain FC': 'Paris Saint-Germain',

    'RC Lens': 'Lens',

    'SC Bastia': 'Bastia',
    'Sc Bastia': 'Bastia',

    'SM Caen': 'Caen',
    'Stade Malherbe Caen': 'Caen',

    'Stade Rennais': 'Rennes',
    'Stade Rennais FC': 'Rennes',

    'Stade De Reims': 'Reims',
    'Stade de Reims': 'Reims',
    'Stade de Reims (Fra)': 'Reims',

    'Toulouse FC': 'Toulouse',

    'CS Sedan Ardennes': 'Sedan',
    'CS Sedan-Ardennes': 'Sedan',

    'US Boulogne-sur-mer CO': 'Boulogne',
    'U.S. Boulogne': 'Boulogne',




    # Poland




    'Hutnik Krakow': 'Hutnik Nowa Huta',

    'Groclin-Dyskobolia Grodzisk': 'Dyskobolia Grodzisk Wielkopolski',

    'Górnik Leczna': 'Górnik Łęczna',



    'Cracovia Kraków': 'KS Cracovia',
    'Cracovia (Kraków)': 'KS Cracovia',

    'Petrochemia Płock': 'Wisła Plock',
    'Petrochemia Plock': 'Wisła Plock',
    'Petrochemia': 'Wisła Plock',

    'Wisla Plock': 'Wisła Plock',
    'Wisla P': 'Wisla Plock',
    'Wisła P': 'Wisla Plock',
    'Wisla Plock': 'Wisła Plock',

    'Wisla K': 'Wisla Krakow',
    'Wisla Kraków (Pol)': 'Wisla Krakow',

    'Górnik L': 'Górnik Łęczna',
    'Górnik Ł': 'Górnik Łęczna',
    'Gornik Leczna': 'Górnik Łęczna',

    'Zaglębie Lubin': 'Zagłębie Lubin',

    'Górnik P': 'KS Polkowice',
    'Górnik Polkowice': 'KS Polkowice',

    'Belchatów': 'GKS Belchatow',



    'Garbarnia': 'Garbarnia Kraków',

    'Swit': 'Świt Nowy Dwór',

    'Polonia Warszawa': 'Polonia Warsaw',

    'Groclin Dyskobolia': 'Dyskobolia Grodzisk Wielkopolski',


    # Netherlands

    'Almere City - Netherlands': 'Almere City',
    'Almere City FC': 'Almere City',

    'Willem II (Tilburg)': 'Willem II',
    'Willem Ii Tilburg': 'Willem II',


    'DOS': 'FC Utrecht',
    'Fc Utrecht': 'FC Utrecht',
    'DOS (Utrecht) (Ned)': 'FC Utrecht',

    'De Graafschap (Doetinchem)': 'De Graafschap',
    'Graafschap': 'De Graafschap',

    'Go Ahead': 'Go Ahead Eagles',

    'Den Haag': 'ADO Den Haag',

    'Pec Zwolle': 'PEC Zwolle',
    'Zwolle': 'PEC Zwolle',
    'FC Zwolle': 'PEC Zwolle',

    'Eindhoven': 'PSV Eindhoven',
    'MVV': 'MVV Maastricht',

    'Volendam': 'FC Volendam',
    'FC Volendam - Netherlands': 'FC Volendam',

    'Cambuur': 'SC Cambuur',
    'Cambuur Leeuwarden': 'SC Cambuur',

    'sc Heerenveen': 'Heerenveen',
    'Sc Heerenveen': 'Heerenveen',
    'SC Heerenveen': 'Heerenveen',

    'RBC': 'RBC Roosendaal',
    'Helmond': 'Helmond Sport',

    'Dordrecht': 'FC Dordrecht',
    'Dordrecht \'90': 'FC Dordrecht',

    'Vitesse (Arnhem)': 'Vitesse',

    'Emmen': 'FC Emmen',

    # Scotland

    'Dunfermline Athletic': 'Dunfermline',
    'Dunfermline A': 'Dunfermline',
    'Dunfermline Ath': 'Dunfermline',

    'Dundee U.': 'Dundee United',
    'Dundee United (Sco)': 'Dundee United',
    'Dundee U': 'Dundee United',
    'Dundee Utd': 'Dundee United',
    'Dundee United Fc': 'Dundee United',

    'Dundee': 'Dundee FC', # Careful here.
    'Dundee FC (Sco)': 'Dundee FC',

    'St Johnstone': 'St. Johnstone',
    'Saint Johnstone': 'St. Johnstone',


    'Inverness': 'Inverness CT',
    'Inverness Caled. Th.': 'Inverness CT',
    'Caledonian Thistle': 'Inverness CT',

    'Saint Mirren': 'St. Mirren',
    'St Mirren': 'St. Mirren',

    'Partick': 'Partick Thistle',

    # France

    'St Etienne': 'Saint-Étienne',

    'Havre': 'Le Havre',
    'Istres': 'FC Istres',

    # Belgium

    'KSV Roeselare': 'Roeselare',

    'K.R.C. Zuid-West-Vlaanderen': 'KRC Harelbeke',
     'Harelbeke': 'KRC Harelbeke',
    'RC Harelbeke': 'KRC Harelbeke',

    'K.S.V. Waregem': 'KSV Waregem',
    'Waregem': 'KSV Waregem',
    
    'Seraing': 'R.F.C. Seraing',

    'K.V. Oostende': 'Oostende',
    'KV Oostende': 'Oostende',

    'VC Westerlo': 'Westerlo',
    'K.V.C. Westerlo': 'Westerlo',
    'KVC Westerlo': 'Westerlo',

    'KFC Verbroedering (Geel)': 'Verbroedering Geel',
    'Geel': 'Verbroedering Geel',

    'K Heusden-Zolder': 'Heusden-Zolder',


    'SK Lokeren': 'Lokeren',
    'KSC Lokeren': 'Lokeren',
    'KSC Lokeren OV': 'Lokeren',
    'K.S.C. Lokeren Oost-Vlaanderen': 'Lokeren',
    'Sporting Lokeren SNW': 'Lokeren', # check?

    'R.F.C. de Liège': 'RFC Liège',

    'Mechelen': 'KV Mechelen',
    'Y.R. K.V. Mechelen': 'KV Mechelen',
    'KV Mechelen (Bel)': 'KV Mechelen',


    'Aalst': 'Eendracht Aalst',
    'SC Eendracht Aalst': 'Eendracht Aalst',
    'KSC Eendracht Aalst': 'Eendracht Aalst',
    'K.S.C. Eendracht Aalst': 'Eendracht Aalst',

    'Lommel SK': 'KFC Lommelse',
    'Lommel': 'KFC Lommelse',
    'KFC Lommelse SK': 'KFC Lommelse',
    'K.F.C. Lommel S.K.': 'KFC Lommelse',



    'Germinal Ekeren': 'Beerschot AC', # merged, inherited.

    'K. Beerschot AC': 'Beerschot AC',
    'G. Beerschot': 'Beerschot AC',

    'KFC Germinal Beerschot': 'Beerschot AC',
    'K.F.C. Germinal Beerschot': 'Beerschot AC',
    'Beerschot': 'Beerschot AC',
    'Germinal': 'Beerschot AC',
    'Germinal Beerschot': 'Beerschot AC',
    'Germinal Beerschot (Antw.)': 'Beerschot AC',
    'Germinal Beerschot Antwerpen': 'Beerschot AC',
    'Germ.Beerschot': 'Beerschot AC',

    'AA Louvièroise': 'La Louvière',
    'AA Louviéroise': 'La Louvière',
    'RAA Louviéroise': 'La Louvière',
    'La Louviere': 'La Louvière',

    'K. Lierse SK': 'Lierse SK',
    'Lierse S.K.': 'Lierse SK',
    'Lierse': 'Lierse SK',
    'Lierse SK (Bel)': 'Lierse SK',

    'Dender': 'Dender EH',
    'GBA': 'Beerschot AC',

    'Beveren': 'Waasland-Beveren',
    'KV Red Star Waasland-SK Beveren': 'Waasland-Beveren',

    # Portugal

    'GD Estoril Praia': 'GD Estoril-Praia',
    'GD Estoril-Praia': 'Estoril',
    'Estoril Praia': 'Estoril',

    'CD Aves': 'Aves',
    'Desportivo Aves': 'Aves',

    'CD Feirense': 'Feirense',





    'C.D. Santa Clara': 'CD Santa Clara',
    'CD Trofense': 'Trofense',


    'Sp. Braga': 'Braga',
    'Sporting de Braga': 'Braga',
    'Sc Braga': 'Braga',

    'SC Beira Mar': 'Beira-Mar',
    'SC Beira-Mar': 'Beira-Mar',
    'Rio Ave FC': 'Rio Ave',

    'SC Campomaiorense': 'Campomaiorense',
    'Sporting Campomaiorense': 'Campomaiorense',

    'FC Alverca': 'Alverca',

    'Gil Vicente FC': 'Gil Vicente',

    # Hungary

    'Honved Budapest (Hun)': 'Honved',
    'Honvéd Budapest (Hun)': 'Honved',
    'Honved': 'Honvéd',

    'III. kerulet': 'III. Kerületi TUE',

    'BVSC': 'BVSC Budapest',
    
    'Eger': 'Egri FC',

    'Pécs': 'Pécsi MFC',
    'PMFC': 'Pécsi MFC',
    
    'Vac': 'Vác',

    'Videoton FC': 'Videoton',

    'Siofok': 'BFC Siófok',
    'Siófok': 'BFC Siófok',
    'Balaton': 'BFC Siófok',
    'Balatón': 'BFC Siófok',
    'Balaton FC': 'BFC Siófok',

    'MTK Budapest FC': 'MTK Budapest',
    'Vörös Lobogó': 'MTK Budapest',
    'MTK Hungária': 'MTK Budapest',
    'MTK (Budapest) (Hun)': 'MTK Budapest',
    'MTK Budapest (Hun)': 'MTK Budapest',
    'MTK (Budapest-Hun)': 'MTK Budapest',

    'Kaposvári Rákóczi': 'Kaposvár',

    'Budapest Honvéd': 'Honvéd',
    'Bp. Honvéd': 'Honvéd',
    'Kispest': 'Honvéd',

    'Dunaferr': 'Dunaújváros FC',

    'Fehérvár': 'Videoton',
    'Fehervár': 'Videoton',
    'FC Fehérvár': 'Videoton',
    
    'Szombathelyi Haladás': 'Haladás',

    'Győri ETO': 'Győr',
    'Györ': 'Győr',
    'Gyor': 'Győr',
    'Gyori ETO': 'Győr',
    'Gyori Eto Fc': 'Győr',
    'ETO Györ (Hun)': 'Győr',
    'ETO Győr (Hun)': 'Győr',

    'Haladas': 'Haladás',

    'Debreceni VSC': 'Debrecen',
    'DVSC': 'Debrecen',

    'Nyíregyháza': 'Nyíregyháza Spartacus',
    'Nyirseg': 'Nyíregyháza Spartacus',

    'Paksi SE': 'Paks',

    'Ujpesti TE': 'Újpest FC',
    'Újpest': 'Újpest FC',
    'UTE': 'Újpest FC',
    

    'Diósgyőri VTK': 'Diósgyőr',
    'Diósgyõr': 'Diósgyőr',
    'Diósgyõri VTK': 'Diósgyőr',
    'Diosgyõr': 'Diósgyőr',
    'Diosgyor': 'Diósgyőr',

    'Tatabanya': 'Tatabánya',



    'Kecskeméti TE': 'Kecskemét',

    'Lombard-Pápa': 'Pápa',

    'Vasas': 'Vasas SC',
    'Vasas SC (Hun)': 'Vasas SC',
    'Vasas Budapest (Hun)': 'Vasas SC',

    'Gyõr': 'Győr',
    'Gyõri ETO': 'Győr',

    'Zalaegerszeg': 'Zalaegerszegi TE',
    'Zalaegerszegi': 'Zalaegerszegi TE',
    'ZTE': 'Zalaegerszegi TE',

    'FTC': 'Ferencvaros',

    'Sopron': 'FC Sopron',

    'REAC': 'Rákospalotai EAC',

    # Czech

    'FK Teplice': 'Teplice',



    #'FC Dukla Pribram': 

    '1. FK Příbram': 'FK Příbram',
    '1.FK Příbram': 'FK Příbram',
    'Pribram': 'FK Příbram',
    'Príbram': 'FK Příbram',


    'Jabonec': 'FK Baumit Jablonec',

    'Blsany': 'Blšany',

    'Opava': 'SFC Opava',
    
    'Karvina': 'MFK Karviná',


    'Drnovice': 'FK Drnovice',
    '1.FKD (Drnovice)': 'FK Drnovice',
    

    'Jihlava': 'Vysočina Jihlava',


    'FC Hradec Králové': 'Hradec Králové',
    'Spartak Hradec Králové (Tch)': 'Hradec Králové',
    'Spartak Hradec Králová (Tch)': 'Hradec Králové',
    'Spartak Hradec Králové': 'Hradec Králové',

    'SK Hradec Kralove': 'Hradec Králové',
    'SK Hradec Kralové': 'Hradec Králové',
    'SK Hradec Králové': 'Hradec Králové',

    'SK Fomei Hradec Kralove': 'Hradec Králové',

    'Hradec Kralove': 'Hradec Králové',
    'Hr.Králové': 'Hradec Králové',
    'Hr. Kralove': 'Hradec Králové',
    'Hr.Kralove': 'Hradec Králové',


    'České Budějovice': 'Dynamo České Budějovice',
    'Č. Budějovice': 'Dynamo České Budějovice',
    'Č.Budějovice': 'Dynamo České Budějovice',
    'C.Budejovice': 'Dynamo České Budějovice',
    'Ceske Budejovice': 'Dynamo České Budějovice',
    'Cekse Budejovice': 'Dynamo České Budějovice',
    'SK Dynamo Ceske Budejovice': 'Dynamo České Budějovice',
    'SK Dynamo České Budějovice': 'Dynamo České Budějovice',

    'SK JCE Ceske Budejovice': 'Dynamo České Budějovice',
    'SK Ceské Budejovice': 'Dynamo České Budějovice',

    'FC Dukla': 'Dukla Prague',
    'FC Dukla Praha': 'Dukla Prague',
    'UDA Dukla Praha': 'Dukla Prague',
    'Uda Dukla': 'Dukla Prague',
    'FK Dukla Praha': 'Dukla Prague',

    'Dukla Prague (TCH)': 'Dukla Prague',
    'Dukla Praha (Tch)': 'Dukla Prague',
    'Dukla Praha (TCH)': 'Dukla Prague',
    'Dukla Praha T(CH)': 'Dukla Prague',
    'Dukla Praha': 'Dukla Prague',
    'Dukla': 'Dukla Prague',

    'FC Tescoma Zlin': 'FC Tescoma Zlín',

    'FC Vysocina Jihlava': 'FC Vysočina Jihlava',

    'FK Chmel Blsany': 'Blšany',

    'FK Svarc Benesov': 'SK Benešov',

    'FC Marila Príbram': 'FK Marila Příbram',
    'FC Marila Pribram': 'FK Marila Příbram',
    'FK Marila Pribram': 'FK Marila Příbram',
    'FK Marila Příbram': 'FK Příbram',

    'Příbram': 'FK Příbram',

    'Slavia Praha': 'Slavia Prague',
    'Zlin': 'FC Fastav Zlín',
    'Jablonec': 'FK Baumit Jablonec',
    'Jablonec 97': 'FK Baumit Jablonec',

    'FK Jablonec \'97': 'FK Baumit Jablonec',
    'FK Jablonec 97': 'FK Baumit Jablonec',
    'FK Jablonec nad Nisou 97': 'FK Baumit Jablonec',
    'FK Jablonec nad Nisou \'97': 'FK Baumit Jablonec',
    'FK Jablonec nad Nisou': 'FK Baumit Jablonec',
    'Jablonec nad Nisou': 'FK Baumit Jablonec',

    'FC Karvina': 'FC Karviná',

    'FC Viktoria Plzen': 'Viktoria Plzeň',
    'Plzeň': 'Viktoria Plzeň',
    'Plzen': 'Viktoria Plzeň',

    'Žižkov': 'Viktoria Žižkov',
    'Zizkov': 'Viktoria Žižkov',
    'FK Viktoria Zizkov': 'FK Viktoria Žižkov',
    'FK Viktoria Žižkov': 'FK Viktoria Žižkov',
    'FK Viktoria Žižkov': 'Viktoria Žižkov',


    'Ostrava': 'Baník Ostrava',
    'Baník Ostrava (TCH)': 'Baník Ostrava',
    'FC Baník Ostrava': 'Baník Ostrava',
    'FC Banik Ostrava': 'Baník Ostrava',

    'Kladno': 'SK Kladno',
    'M.Boleslav': 'Mladá Boleslav',
    'M. Boleslav': 'Mladá Boleslav',
    'FK Mladá Boleslav': 'Mladá Boleslav',

    'Zbrojovka Brno (TCH)': 'Zbrojovka Brno',
    'Brno': 'Zbrojovka Brno',
    'FC Zbrojovka Brno': 'Zbrojovka Brno',

    'Synot': 'Slovácko',
    '1.FC Synot': 'Slovácko',
    '1.FC Synot (Stare Mesto)': 'Slovácko',

    'Slovacko': 'Slovácko',
    '1. FC Slovácko': 'Slovácko', 
    '1.FC Slovácko': 'Slovácko', 
    '1.FC Slovacko': 'Slovácko', 

    'FC Slovan WSK Liberec': 'Slovan Liberec',
    'Liberec': 'Slovan Liberec',
    'FC Slovan Liberec': 'Slovan Liberec',


    'SK Olomouc Sigma MZ': 'Sigma Olomouc',
    'SK Sigma Olomouc': 'Sigma Olomouc',
    'SK Sigma MZ Olomouc': 'Sigma Olomouc',
    'Olomouc': 'Sigma Olomouc',
    'FC Sigma Olomouc MZ': 'Sigma Olomouc',

    'Ústí nad Labem': 'FK Ústí nad Labem',
    'Ústí': 'FK Ústí nad Labem',

    'FC Siad Most': 'Baník Most',
    'Most': 'Baník Most',

    'Zlín': 'FC Fastav Zlín',

    'Bohemians/Strížkov': 'Bohemians Prague (Střížkov)',
    'FC Bohemians/Strížkov Praha': 'Bohemians Prague (Střížkov)',

    'FC Bohemians Praha': 'Bohemians Praha',
    'FC Bohemians 1905 Praha': 'Bohemians Praha',
    'Bohemians 1905 Praha': 'Bohemians Praha',
    'CU Bohemians Praha': 'Bohemians Praha',
    'SK Bohemians Praha': 'Bohemians Praha',
    'Bohemians Praha (TCH)': 'Bohemians Praha',
    'Bohemians Praha': 'Bohemians 1905',

    

    'Okanagan Valley Challenge': 'Okanagan Challenge',

    'Brighton Seagulls': 'Brighton & Hove Albion',

    'Playa del Carmen': 'Inter Playa del Carmen',
    'New Bedford Portuguese SC': 'New Bedford Portuguese',


    'Vila Nova Futebol Clube': 'Vila Nova',
    'Vilanova': 'Vila Nova',
    'Vila Nova-GO': 'Vila Nova',

    'Clube Náutico Capibaribe': 'Náutico',
    

    'AS Rodos': 'Rodos',
    'Tonelle de Yaounde': 'Tonnelle de Yaoundé',
    'Canon de Yaounde': 'Canon Yaoundé',
    'Africa Sport': 'Africa Sports',
    'Cracovia Krakow': 'KS Cracovia',


    'Okanagan Predators': 'Okanagan Challenge',


    'CD Arica': 'San Marcos de Arica',
    'Deportes Arica': 'San Marcos de Arica',




    'SC Braga': 'Braga',

    'XV de Jau': 'XV de Jaú',
    'Jau XV': 'XV de Jaú',
    'XV de Novembro (Jaú)': 'XV de Jaú',
    'Xv De Novembro/J': 'XV de Jaú',
    'XV de Novembro/J': 'XV de Jaú',

    'XV de Novembro (Piracicaba)': 'XV de Piracicaba',
    'XV Piracicaba': 'XV de Piracicaba',
    'XV de Novembro/P': 'XV de Piracicaba',
    'XV de Novembro': 'XV de Piracicaba',
    'XV de Pìracicaba': 'XV de Piracicaba',

    'Rosenheim 1860': 'TSV 1860 Rosenheim',

    'Mjondalen FC': 'Mjøndalen IF',

    'Fredrikstad F.C.': 'Fredrikstad FK',
    'Fredrikstad FK (Nor)': 'Fredrikstad FK',

    'Trelleborg FF': 'Trelleborg',

    'SFK Lyn': 'Lyn Oslo',
    'Lyn F.C.': 'Lyn Oslo',
    'FC Lyn Oslo': 'Lyn Oslo',


    # US

    'Waypoyset FC (RI)': 'Waypoyset FC',
    'Raftery Painters': 'St. Louis Raftery Painters',
    'Squantum': 'Squantum FC',
    'Schenectady FC': 'Schenectady SC',
    'Saylesville': 'Saylesville FC',
    'Rochester City': 'Rochester City AFC',
    'Richard Rich Tigers (Milwaukee)': 'Milwaukee Richard Rich Tigers',
    'Richard Rich Tigers': 'Milwaukee Richard Rich Tigers',
    'Prospect Unity': 'Prospect Unity FC',
    'Pontiac City': 'Pontiac City FC',
    'Philadelphia German American SC': 'Philadelphia German Americans',
    'Ottawa Pioneers': 'Ottawa Intrepid',
    'Oneida Community': 'Oneida Community FC',
    'Necaxa (Chicago)': 'Chicago Necaxa',
    'Harmarville FC (Pittsburgh)': 'Harmarville FC',
    'Harmarville SC': 'Harmarville FC', # Hurricanes?
    'Hibernian FC': 'Hibernians FC',
    'Hummer Sports Cafe (MD)': 'Hummers Sports Cafe (MD)',
    'Lincoln Park': 'Lincoln Park FC',


    'Haverford': 'Haverford College',
    'Polish Falcons (NJ)': 'Polish Falcons',
    'Fort Lauderdale Kicks': 'Fort Lauderdale Strikers',
    'Oklahoma City Heat': 'Oklahoma City Slickers',
    'New York Capital District Alleycats': 'Albany Alleycats',
    'Roanoke RiverDawgs': 'Roanoke Wrath',
    'Jackson Generals': 'Jackson Chargers',
    'Miami Gatos': 'Miami Toros',
    'Los Angeles Toros': 'San Diego Toros',
    'Ft. Lauderdale Sun': 'Fort Lauderdale Sun',

    'Bay Area CyberRays': 'San Jose CyberRays',
    'Bridgeport Vasco Da Gama': 'Bridgeport Vasco da Gama',
    'University of California Berkeley': 'University of California, Berkeley',
    'Boston Aztecs': 'Boston Aztec',
    'Aztec MA': 'Boston Aztec',
    'Portland Rain FC': 'Portland Rain',
    'Fall River Canonicuts': 'Fall River Conanicuts', # inform mel smith.
    'St George\'s Colts': 'St. George\'s Colts',
    'I.R.T. Celtics': 'IRT Celtics',
    'Irish American FC': 'Irish-American FC',
    'SoccerPlus CT': 'SoccerPlus Connecticut',

    'San Diego SeaLions': 'San Diego WFC SeaLions',
    'San Diego WFC': 'San Diego WFC SeaLions',

    'Federal Ship FC': 'Kearny Federal Ship',
    'Thornton Hosieries Albions': 'Thornton Hosiery Albions',
    'Kearny Clarks ONT 2nds': 'Kearney Clarks ONT 2nds',





    # uncategorized

    'Thespa Kusatsu': 'Thespakusatsu Gunma',
    'Atletico Santista': 'Atlético Santista',
    'Dublín': 'Dublin',
    'Larne F.C.': 'Larne FC',




    'Nuevo Leon': 'Nuevo León', 
    'CF Nuevo Leon': 'Nuevo León', 

    

    'Racing des Gonaives': 'Racing des Gonaïves',

    'Sacachipas': 'Sacachispas',
    'Teculután': 'Deportivo Teculután',

    'Leo Victor': 'SV Leo Victor',
    'Juventus (Netherlands Antilles)': 'SV Juventus',

    'St. Leo': 'St. Leo\'s',
    'St. Leo FC': 'St. Leo\'s',

    'Orgryte Idrottssallskopf': 'Örgryte IS',


    'Djerv F.C.': 'SK Djerv 1919',
    'Vard F.C.': 'SK Vard Haugesund',
    'Brage F.C.': 'IK Brage',
    'Brann F.C.': 'SK Brann',


    'Toros Hidalgo': 'Toros Neza',

    #'Neza': 'Toros Neza',
    #'Deportivo Neza': 'Toros Neza',
    #'Tranvias': 'Necaxa', # merger

    'Cementos Novello': 'Cementos Novella',
    'Luz y Fuerza': 'Necaxa',# name map.

    'Moctezuma': 'Moctezuma de Orizaba',
    
    'Oaxtepec': 'CF Oaxtepec',

    'U.de Curtidores': 'Unión de Curtidores',
    'Curtidores': 'Unión de Curtidores', # check.

    'C.F. La Piedad': 'La Piedad',


    'Ceara': 'Ceará',

    'Reunion': 'Réunion',

    'Union de Curtidores': 'Unión de Curtidores',
    'Deportivo Espanol': 'Deportivo Español',
    'Porteno': 'Porteño',
    'Racing Club Saint Joseph': 'Racing Club Saint-Joseph',
    'Emigrantes Das Ilhas': 'Emigrantes das Ilhas',

    'Vitoria FC': 'Vitória FC',

    'Corinthian FC': 'Corinthian F.C.',
    'SC Corinthians': 'Corinthians',

    'Corinthians (PP)': 'Corinthians de Presidente Prudente',

    'Huracan Buceo': 'Huracán Buceo',
    'Wollongong Wolves': 'South Coast Wolves FC',

    'Uniao Micalense': 'CU Micaelense',
    'Instant Dictation': 'Double Flower FA',

    'FC Boavista': 'Boavista',




    # India

    'Mohun Begun': 'Mohun Bagan',

    # Iran

    'Naft': 'Naft Tehran',

    'Sepahan': 'Sepahan Isfahan FC',

    'Rah-Ahan': 'Rah Ahan',
    'Zob-Ahan': 'Zob Ahan',

    'Shahin': 'Shahin Bushehr',

    'Peykan': 'Paykan',

    'Esteghlal A': 'Esteghlal Ahvaz',
    
    'Esteghlal T': 'Esteghlal Tehran',

    'Steel-Azin': 'Steel Azin',

    'Fajr-Sepasi': 'Fajr Sepasi',

    'Moghavemat': 'Moghavemat Tehran',

    'Hamadan-Paas': 'PAS Hamedan',

    'Saba': 'Saba Qom',

    'Pirouzi': 'Persepolis',

    'Aluminium': 'Aluminium Hormozgan',

    'Esteghlal': 'Esteghlal Tehran',


    # Korea

    'Busan': 'Busan IPark',
    'Pusan': 'Busan IPark',

    'Incheon': 'Incheon United',

    'Gyeongnam': 'Gyeongnam FC',
    'Jeju': 'Jeju United FC',
    'Jeonbuk': 'Jeonbuk Hyundai Motors',
    'Gangwon': 'Gangwon FC',


    'Seongnam': 'Seongnam Ilhwa Chunma',
    'Songnam': 'Seongnam Ilhwa Chunma', 
    'Bucheon': 'Bucheon SK',
    'Anyang': 'Anyang LG Cheetahs',
    'Chunnam': 'Chunnam Dragons',
    'Chunnam Dragons (Gwangyang)': 'Chunnam Dragons',


    'Ulsan Hyundai': 'Ulsan Hyundai FC',

    'Jeonbuk Motors': 'Jeonbuk Hyundai Motors',
    'Jeonbuk Hyundai Motors (Jeonju)': 'Jeonbuk Hyundai Motors',

    'Busan I Park': 'Busan IPark',

    'Hallelujah': 'Hallelujah FC',

    'Daegu': 'Daegu FC',

    'Chonbuk': 'Jeonbuk Hyundai Motors',

    'Pohang': 'Pohang Steelers',

    'Ulsan': 'Ulsan Hyundai FC',

    'Daejeon': 'Daejeon Citizen',
    'Taejon': 'Daejeon Citizen',



    # Belarus
    'Fc Dinamo Minsk': 'Dinamo Minsk',
    'Dynamo Minsk': 'Dinamo Minsk',
    'Dinamo Minsk (URS)': 'Dinamo Minsk',


    'Real Sociedad Zacatecas': 'Real Sociedad de Zacatecas',

    'Angeles': 'Ángeles de Puebla',


    'Son-Sin': 'Guerra y Marina',








    'Arabe Marte': 'C.D. Atlético Marte', # add alias?
    'Pinar del Rio': 'Pinar del Río',


    'St. George Colts': 'St. George\'s Colts',

    # Korea
    'Kookmin Bank': 'Goyang KB Kookmin Bank FC',

    'Sangmu': 'Sangju Sangmu Phoenix',
    'Gwangju Sangmu Phoenix': 'Sangju Sangmu Phoenix',
    'Sangju Sangmu Phoenix': 'Sangju Sangmu',
    'Sangju Sangmu FC': 'Sangju Sangmu',
    'Gwangju Sangmu': 'Sangju Sangmu',
    'Gwangju': 'Sangju Sangmu',
    'Sangju': 'Sangju Sangmu',

    'Suwon': 'Suwon Samsung Bluewings',

    'Puchon': 'Bucheon SK',
    'Puchon SK': 'Bucheon SK',
    'Puchon Yukong': 'Bucheon SK',

    'Ilhwa Chunma': 'Seongnam Ilhwa Chunma',
    'Songnam Ilhwa Chunma': 'Seongnam Ilhwa Chunma',
    'Seongnam Ilhwa': 'Seongnam Ilhwa Chunma',

    'Hyundai Horang-i': 'Ulsan Hyundai FC',
    'Ulsan Hyundai Horang-i': 'Ulsan Hyundai FC',
    'Chonbuk Hyundai Horang-i': 'Ulsan Hyundai FC', # curious...

    'Busan I\'Park': 'Busan IPark',
    'Busan Icons': 'Busan IPark',
    'Pusan Icons': 'Busan IPark',
    'Daewoo': 'Busan IPark',
    'Daewoo Royals': 'Busan IPark',
    'Pusan Daewoo Royals': 'Busan IPark',

    'Jeju United': 'Jeju United FC',

    'Chobuk Hyundai Dinos': 'Jeonbuk Hyundai Motors',
    'Chonbuk Dinos': 'Jeonbuk Hyundai Motors',
    'Chonbuk Hyundai Dinos': 'Jeonbuk Hyundai Motors',

    'Chunan Ilhwa Chunma': 'Seongnam Ilhwa Chunma',
    'Chunan Ilwha Chunma': 'Seongnam Ilhwa Chunma', # typo

    'LG Cheetahs': 'Anyang LG Cheetahs',

    'Lucky-Goldstar Hwangso': 'Anyang LG Cheetahs',
    'Lucky-Goldstar Hwangwo': 'Anyang LG Cheetahs',

    'Taejon Citizen': 'Daejeon Citizen',
    'Taejon Citizon': 'Daejeon Citizen',

    'Pohang Atoms': 'Pohang Steelers',
    'POSCO': 'Pohang Steelers',
    'POSCO Atoms': 'Pohang Steelers',
    'POSCO Dolphins': 'Pohang Steelers',
    'Yukong Elephants': 'Jeju United FC',
    'Chonbuk Hyundai Motors': 'Jeonbuk Hyundai Motors',

    'Rentistas': 'CA Rentistas',



    'Union La Calera': 'Unión La Calera',
    'U. La Calera': 'Unión La Calera',

    'Santiago National Juventus': 'Santiago National',

    'Iberia Los Angeles': 'Iberia Los Ángeles',


    'Trasandino': 'Trasandino de Los Andes',
    'Turrialba': 'Turrialba F.C.',
    'CS Once Tigres': 'Once Tigres',
    'SG Espanola': 'SG Española de San José',


    'Maccabee Los Angeles': 'Los Angeles Maccabee AC',

    'Xilotepelt (Jinotepe)': 'Xilotepelt',
    'Scorpion FC': 'Scorpión FC',

    # Saint Vincent and the Grenadines

    'Chelsea FC (St Vincent)': 'Camdonia Chelsea SC',

    'Stubborn Youth FC': 'Stubborn Youth',


    'Atletico Morazan': 'Atlético Morazán',

    'Paradise FC': 'Paradise SC',
    'Elite FC': 'Elite SC',
    'Northern United': 'Northern United All Stars',
    'Ideal FC': 'Ideal SC',

    'Waterhouse FC': 'Waterhouse F.C.',
    'Waterhouse': 'Waterhouse F.C.',

    'SV Nacional': 'SV Deportivo Nacional',
    'Milerock': 'Milerock FC',
    'System 3': 'System 3 FC',
    'Systems 3': 'System 3 FC',

    'Notre Dame (Barbados)': 'Notre Dame SC',
    'South East United': 'Sagicor South East United',
    'Sagicor South East': 'Sagicor South East United',
    'L\'Aiglon': 'Aiglon du Lamentin',


    # Curacao
    'CS Barber': 'Centro Barber',
    'Jong Colombia': 'CRKSV Jong Colombia',
    'Jong Holland': 'CRKSV Jong Holland',
    'RKVFC Sithoc': 'RKV FC SITHOC',
    'SUBT': 'S.V. S.U.B.T.',
    'VESTA': 'SV VESTA',
    'SV Hubentud Fortuna': 'Hubentud Fortuna',
    'Hubentud Fortuna': 'Hubentut Fortuna',
    'SV Centro Social Deportivo Barber': 'Centro Barber',
    'Sithoc': 'RKV FC SITHOC',


    # Saint Kitts
    'Suzuki Newtown United': 'Newtown United (Saint Kitts)',
    'Newtown United (Saint Kitts)': 'Newtown United',

    # Puerto Rico



    'Pr Islanders': 'Puerto Rico Islanders',
    'River Plate PR': 'River Plate Puerto Rico',

    'Sevilla Bayamon FC': 'Bayamón FC',
    'Bayamon FC - Puerto Rico': 'Bayamón FC',
    'Bayamon FC': 'Bayamón FC',

    'Hoppers': 'Hoppers FC',
    'Cavaly': 'AS Cavaly',

    # Suriname
    'Inter Moengo Tapoe': 'Inter Moengotapoe',

    'WBC - Suriname': 'Walking Bout Company',
    'Walking Boyz Company': 'Walking Bout Company',
    'Walking Boys Company': 'Walking Bout Company',

    # Panama

    'Projusa (Veraguas)': 'Bravos del Projusa',

    'CD Policía Nacional': 'Policía Nacional', # turned into Atletico Nacional #2.

    'Atletico Chiriqui': 'Atlético Chiriquí',
    'Atletico Veraguense': 'Atlético Veragüense',


    'Cd Arabe Unido': 'Arabe Unido',
    'Arabe Unido - Panama': 'Arabe Unido',
    'CD Arabe Unido': 'Arabe Unido',
    'Deportivo Árabe Unido': 'Árabe Unido',
    'Àrabe Unido': 'Arabe Unido',
    'Deportivo Arabe Unido': 'Arabe Unido',
    'Deportivo Arabe Unido --Panama': 'Arabe Unido',
    'Arabe Unido': 'Árabe Unido',
    'CD Árabe Unido': 'Árabe Unido',

    'CD Plaza Amador': 'Plaza Amador',

    'Chiriqui FC': 'Chiriquí F.C.',
    'Chiriquí FC': 'Chiriquí F.C.',

    'Chiriqui': 'Chiriquí',

    'CD Pan de Azucar': 'Pan de Azúcar',
    'Pan de Azucar FC': 'Pan de Azúcar',
    'CD Pan de Azúcar': 'Pan de Azúcar',
    'Pan Azucar': 'Pan de Azúcar',

    'Panama Viejo': 'Panamá Viejo',
    'Panama Viejo FC': 'Panamá Viejo',
    'Panamá Viejo FC': 'Panamá Viejo',
    'Panamá Viejo F.C.': 'Panamá Viejo',

    'Coclé': 'Sporting San Miguelito', #verify
    'Sporting Coclé': 'Sporting San Miguelito', #verify
    'Sporting Cocle': 'Sporting San Miguelito', #name map.
    'Sporting Cocle FC': 'Sporting San Miguelito',
    'Sporting \'89': 'Sporting San Miguelito',
    'Sporting 89': 'Sporting San Miguelito', # name map.
    'Sporting (Panama)': 'Sporting San Miguelito',
    'Sporting SM': 'Sporting San Miguelito',

    'River Plate (Panama)': 'Colón River FC',

    'Chorrillo': 'Chorrillo F.C.',
    'Chorrillo Fc': 'Chorrillo FC',
    'El Chorrillo FC': 'Chorrillo F.C.',
    'Municipal Chorrillo': 'Chorrillo F.C.',

    'El Chorrillo': 'Chorrillo F.C.',
    'Mun. Chorrillo': 'Chorrillo F.C.',
    'Mpal. Chorrillo': 'Chorrillo F.C.',
    'Chorrillo F.C.': 'Chorrillo FC',

    'Atletico Nacional (Panama)': 'Atlético Nacional (Panama)',

    # Costa Rica

    'Orion F.C.': 'Orión FC',

    'A.D. Limonense': 'Limón FC',
    'AD Limonense - Costa Rica': 'AD Limonense',
    'AD Limonense': 'Limón FC',
    'Limón': 'Limón FC',
    'Limon - Costa Rica': 'Limon',
    'Limon': 'Limón FC',

    'Santos de Guapiles': 'Santos de Guápiles',
    'SD Santos (Guapiles)': 'Santos de Guápiles',
    'AD Santos': 'Santos de Guápiles',
    'SD Santos': 'Santos de Guápiles',
    'SD Santos (Guapilés)': 'Santos de Guápiles',

    'A.D.Sagrada F.': 'A.D. Sagrada Familia',

    'San Carlos FC': 'A.D. San Carlos',
    'AD San Carlos': 'A.D. San Carlos',


    'AD Barrio Mexico': 'AD Barrio México',

    'Santacrucena': 'Santacruceña',
    'AD Santacruceña': 'Santacruceña',

    'Belen Siglo XXI': 'Belén Siglo XXI',
    'Belén Siglo XXI': 'A.D. Belén',
    'Deportivo Belen - Costa Rica': 'Belen',
    'AD Belén': 'A.D. Belén',
    'Belen': 'A.D. Belén',
    'AD Belen': 'A.D. Belén',
    'Belén': 'A.D. Belén',
    'CD Saprissa': 'Saprissa',
    'Águilas Guanacastecas': 'Liberia Mía', # add name map
    'AD Municipal Liberia': 'Liberia Mía', # add name map?
    'Liberia Mia': 'Liberia Mía', 
    'Brujas Escazú': 'Brujas FC',
    'Brujas FC Escazú': 'Brujas FC',

    'Club Sport Cartaginés': 'Cartaginés',
    'CS Cartaginés': 'Cartaginés',
    'CD Cartagines': 'Cartaginés',
    'CS Cartagines': 'Cartaginés',
    'Cartagines': 'Cartaginés',
    'C.S. Cartaginés': 'Cartaginés',

    'D Municipal Puntarenas FC': 'Puntarenas FC',

    'Guanacaste': 'AD Guanacasteca',
    'Guanacasteca': 'AD Guanacasteca',

    'San Ramon': 'San Ramón',
    'San Ramón': 'AD Ramonense',
    'Ramonense': 'AD Ramonense',

    'Carmelita': 'AD Carmelita',
    'Carmen FC': 'AD Carmelita',

    # Guatemala

    'Nueva Concepción': 'Deportivo Nueva Concepción',

    'Marquense': 'Deportivo Marquense',

    'Chimaltenango': 'Deportivo Chimaltenango',

    'Coatepeque': 'Deportivo Coatepeque',
    'Deportivo Coatepequez': 'Deportivo Coatepeque',

    'Xinabajul': 'Deportivo Xinabajul',

    'Atl. Choloma': 'Atlético Choloma',
    'Atletico Choloma': 'Atlético Choloma',
    'Atlético Cholomeño': 'Atlético Choloma',
    'Atlético Cholomeño (Choloma)': 'Atlético Choloma',



    'Atletico Olanchano': 'Atlético Olanchano',
    'Real Sociedad (Tocoa)': 'CD Real Sociedad',
    
    'Patepluma': 'Real Patepluma',
    #'Real Patepluma': 'Real Juventud',

    'Universidad (Choluteca)': 'Universidad Choluteca',

    'Universidad (USAC)': 'Universidad de San Carlos',
    'Universidad (Guatemala)': 'Universidad de San Carlos',
    'Univ. San Carlos': 'Universidad de San Carlos',
    'USAC': 'Universidad de San Carlos',
    'Universidad San Carlos': 'Universidad de San Carlos',

    'Santa Lucia Cotzumalguapa': 'Santa Lucía Cotzumalguapa',
    'Aurora FC': 'Aurora F.C.',
    'Malacateco': 'Deportivo Malacateco',
    'Mictlán': 'Deportivo Mictlán',
    'Mictlan': 'Deportivo Mictlán',

    'Amatitlán': 'Deportivo Amatitlán',
    'Achuapa': 'Deportivo Achuapa',

    #Honduras

    'Real Patepluma': 'Real Maya', # add name map

    #'Vida': 'CDS Vida', # ambiguous?
    'Vida (La Ceiba)': 'CDS Vida', # ambiguous?

    'Savio': 'Deportes Savio',
    'Dep. Savio (Santa Rosa de Copán)': 'Deportes Savio',

    # China
    'Guangzhou GPC Zhongyi': 'Guangzhou Evergrande',
    'Zhejiang Greentown': 'Hangzhou Greentown',
    'Inter Xian': 'Guizhou Renhe',
    'Shenzhen Xiangxue Shangqingyin': 'Shenzhen Ruby',
    'Dalian Haichang': 'Dalian Shide',
    'Shanghai International': 'Guizhou Renhe',
    'Shaanxi Baorong': 'Guizhou Renhe',
    'Shenyang Ginde': 'Guangzhou R&F',
    'Changsha Ginde': 'Guangzhou R&F',

    'Jiangsu Guoxin Sainty': 'Jiangsu Sainty',
    'Jiangsu Sainty (Nanjing)': 'Jiangsu Sainty',

    'Nanchang August 1st':'Nanchang Hengyuan',
    'Nanchang Hengyuan': 'Shanghai Shenxin',
    'Zhejiang Lucheng': 'Zhejiang Greentown',
    'Tianjin MasterKong': 'Tianjin Teda',

    'Shaanxi Neo-China Chanba': 'Shaanxi Zhongxin',
    'Shaanxi Chanba': 'Shaanxi Zhongxin',
    'Shaanxi Zhongxin': 'Guizhou Renhe',
    'Shaanxi Renhe': 'Guizhou Renhe',
    'Guizhou Moutai': 'Guizhou Renhe',
    'Qingdao Shengwen Jonoon': 'Qingdao Jonoon',
    'Qingdao Zhongcheng': 'Qingdao Jonoon',
    'Henan Siwulaojiao': 'Henan Jianye',
    'Guangzhou GPC': 'Guangzhou GPC Zhongyi',
    'Guangzhou Pharmaceutical': 'Guangzhou GPC Zhongyi',
    'Guangzhou Xiangxue': 'Guangzhou GPC Zhongyi',
    'Shanghai Shenhua Liansheng': 'Shanghai Shenhua', # Shanghai united merger?
    'Shanghai Zobon': 'Shanghai United',
    'Xi\'an Chanba': 'Inter Shanghai', # add name map.

    'Shandong Luneng Taishan': 'Shandong Luneng',
    'Shandong Luneng (Ji\'nan)': 'Shandong Luneng',

    'Shenyang Jinde': 'Shenyang Ginde',
    'Inter Shanghai': 'Shanghai International',
    'Tianjin Kangshifu': 'Tianjin Teda',
    'Xiamen Lanshi': 'Xiamen Blue Lions',
    'Shenzhen Jianlibao': 'Shenzhen Ruby',
    'Shenzhen Xiangxue': 'Shenzhen Ruby',
    'Shenzhen Shangqingyin': 'Shenzhen Ruby',
    'Shenzhen Kingway': 'Shenzhen Ruby',

    'Liaoning Yuandong': 'Liaoning Whowin',
    'Liaoning Hongyun': 'Liaoning Whowin',
    'Liaoning FC': 'Liaoning Whowin',
    'Liaoning F.C.': 'Liaoning Whowin',
    'Liaoning Whowin (Shenyang)': 'Liaoning Whowin',

    'Liaoning Xiyang': 'Liaoning Whowin', # Imagine this is correct.



    'Henan Siwu': 'Henan Jianye',
    'Henan Construction': 'Henan Jianye',
    'Hangzhou Lucheng': 'Hangzhou Greentown',

    'Wuhan Guanggu': 'Wuhan Optics Valley',
    'Wuhan South Asia Optics Valley': 'Wuhan Optics Valley',
    'Wuhan Huanghelou': 'Wuhan Optics Valley',

    'Qingdao Zhongneng': 'Qingdao Jonoon',
    'Qingdao Beilaite': 'Qingdao Jonoon',
    'Beijing Hyundai': 'Beijing Guoan',
    'Shanghai Shenhua SVA': 'Shanghai Shenhua',
    'Chongqing Qiche': 'Chongqing Lifan',
    'Liaoning Zhongyu': 'Liaoning Whowin',


    # Other

    'Western Samoa': 'Samoa',
    'Virginia Tech University': 'Virginia Tech',


    'Ujpest': 'Újpest FC',
    'Újpesti Dózsa': 'Újpest FC',
    'Espoli': 'ESPOLI',
    'Guaraní': 'Club Guaraní',
    'Deportivo Italia': 'Deportivo Petare', # add alias.
    'Exiles SC (South CA)': 'Exiles SC',
    'Real Madrid (TX)': 'Real Madrid AC',
    'Dallas Mustangs Legends': 'Legends FC',
    'Lynch\'s FC': 'Lynch\'s Irish Pub',
    'DS United': 'Lynch\'s Irish Pub',
    'Atlanta FC': 'Atlanta Silverbacks Reserves',
    'Croatian Eagles': 'Milwaukee Croatian Eagles', # Make sure there are no other Croatian Eagles.

    'North Queensland Fury': 'Northern Fury',

    'AS Monaco (Fra)': 'Monaco',
    'AS Monaco': 'Monaco',
    'AS Monaco FC': 'Monaco',

    'Bon Sucesso': 'Bonsucesso',




    




    'Pepeganga': 'Pepeganga Margarita',


    'San José (Bolivia)': 'CD San José',
    'Libertad': 'Club Libertad',
    'Monagas': 'Monagas SC',

    'Atletico Goianiense': 'Atlético Goianiense',
    'Atlético/GO': 'Atlético Goianiense',
    'Atletico GO': 'Atlético Goianiense',
    'Atlético-Go': 'Atlético Goianiense',

    'Paulista': 'Paulista FC',
    'Paulista (Jundiaí)': 'Paulista FC',
    'O Paulista (Jundiaí)': 'Paulista FC',
    'Lousano Paulista': 'Paulista FC',




    'Union Comercio (Moyobamba)': 'Unión Comercio',
    'Union Comercio': 'Unión Comercio',
    'Unión Comercio (Moyobamba)': 'Unión Comercio',

    'CD Universitario San Francisco Xavier': 'Club Universitario',
    'Universitario (Bolivia)': 'Club Universitario',


    'Grêmio Prudente': 'Grêmio Barueri',
    'Gremio Prudente': 'Grêmio Barueri',
    'Barueri': 'Grêmio Barueri',
    'Prudente': 'Grêmio Barueri',

    'Union Atletico Maracaibo': 'UA Maracaibo',
    'U.A. Maracaibo': 'UA Maracaibo',

    'Coritiba': 'Coritiba FC',
    'Alianza Atletico': 'Alianza Atlético',

    'Universidad Cesar Vallejo': 'Universidad César Vallejo',
    'Zamora': 'Zamora FC',
    'Universidad Los Andes': 'Universidad de Los Andes',



    'Deportivo Canarias': 'Deportiva Canarias',
    'Santo Andre': 'Santo André',

    'Universidad Técnica de Cajamarca': 'U.T. Cajamarca',


    'Deportivo Concepción': 'Deportes Concepción',
    'Dep. Concpeción': 'Deportes Concepción',
    'Dep. Concepción': 'Deportes Concepción',
    'Concepción': 'Deportes Concepción',

    'Colón': 'CA Colón',

    'Dvo. Anzoátegui': 'Deportivo Anzoátegui',
    'Deportivo Anzoategui': 'Deportivo Anzoátegui',

    'Always Ready': 'Club Always Ready',
    #'Nuevo de Octubre': '12 de Octubre',
    'St. Louis Mules': 'St. Louis Frogs',
    'Brooklyn Hispanos': 'Brooklyn Hispano',
    'Morse Dry Dock': 'Brooklyn Morse Dry Dock',
    'St. Francis Solanus College': 'Quincy University',
    'Galt FBC': 'Galt F.C.',
    'Randolph Macon College': 'Randolph-Macon College',
    'Stevens Tech': 'Stevens Institute of Technology',
    'Western Reserve University': 'Case Western Reserve University',
    'Washington & Lee University': 'Washington and Lee University',
    'Washburn College': 'Washburn University',
    'East Tennessee State': 'East Tennessee State University',


    'Fall River County St Rovers': 'Fall River County Street Rovers',
    'Tijuana Chivas': 'Chivas Tijuana',



    'British Guiana': 'Guyana',
    'FYR Macedonia': 'Macedonia',
    'Zaire': 'Congo DR',
    'DR Congo': 'Congo DR',
    'Burma': 'Myanmar',
    'Pegasus AFC': 'Pegasus A.F.C.',
    
    'Crystal P': 'Crystal Palace',
    'Crystal Palace - England': 'Crystal Palace',
    'Crystal Palace FC': 'Crystal Palace',
    'Crystal Palace F.C.': 'Crystal Palace',

    'Bromsgrove Rovers FC': 'Bromsgrove Rovers F.C.',
    'Barking FC': 'Barking F.C.',
    'Peterborough United FC': 'Peterborough United F.C.',
    'Newmarket Town FC': 'Newmarket Town F.C.',
    'Ely City FC': 'Ely City F.C.',
    'King\'s Lynn FC': 'King\'s Lynn F.C.',
    'Wisbech Town FC': 'Wisbech Town F.C.',
    'Republic of Ireland': 'Ireland',
    'China PR': 'China',
    'Holland': 'Netherlands',
    'West Germany': 'Germany',


    'Cruzeiro EC': 'Cruzeiro',
    'Esporte Clube Cruzeiro': 'Cruzeiro',


    # 'Parmalat FC': 'Videoton FC', #team map',

    'AC Parma': 'Parma F.C.',
    'Parma': 'Parma F.C.',
    'Parma FC': 'Parma F.C.',
    'Parma AC': 'Parma F.C.',


    'CCA Bucureşti (Rom)': 'FC Steaua București',
    'CCA Bucureşti': 'FC Steaua București',
    'CCA Bucuresti': 'FC Steaua București',

    'FC Steaua Bucureşti': 'FC Steaua București',
    'FC Steaua Bucuresti': 'FC Steaua București',
    'Steaua Bucureşti': 'FC Steaua București',
    'Steaua': 'FC Steaua București',
    'Steaua Bucharest': 'FC Steaua București',
    'Steaua Bucurest': 'FC Steaua București',
    'Steaua Bucuresti (Rom)': 'FC Steaua București',
    'Steaua Bucuresti': 'FC Steaua București',

    'FC Rapid Bucureşti': 'Rapid Bucuresti',
    'UFC Rapid Bucuresti': 'Rapid Bucuresti',
    'AFC Rapid Bucureşti': 'Rapid Bucuresti',
    'AFC Rapid Bucuresti': 'Rapid Bucuresti',
    'Rapid Bucarest': 'Rapid Bucuresti',
    'Rapid Bucuresti (Rom)': 'Rapid Bucuresti',
    'Rapid Bucuresti': 'Rapid București',

    'Grasshopper': 'Grasshoppers',
    'Grasshoppers Club': 'Grasshoppers',
    'Grasshopper Club': 'Grasshoppers',
    'Grasshopper-Club Zürich': 'Grasshoppers',
    'Grasshoppers Zürich (Sui)': 'Grasshoppers',

    'Gremio FBPA': 'Grêmio',
    'SV River Plate': 'SV River Plate Aruba',

    'Pembroke-Hamilton Zebras': 'PHC Zebras',
    'Reveil Sportif': 'Réveil-Sportif',
    'Solidarite Scolaire': 'Solidarité Scolaire',

    'Seba United': 'Montego Bay United', # name map.
    'Seba U.': 'Seba United',

    'Santiagueno': 'C.D. Santiagueño',
    'Border Bandits (South TX)': 'Border Bandits',

    'Banat Arsenal - USASA': 'Banat Arsenal',
    'Banat Arsenal': 'Phoenix Banat Storm', #name map
    'Phoenix Pride': 'Phoenix Inferno', #name map
    'Lubbock Tornado': 'Lubbock Lazers', #name map
    'Indiana Kick': 'New York Kick', # name amp.
    'Chicago Vultures': 'Chicago Shoccers', # name map
    'Orlando City FC': 'Orlando City SC',
    'La Victoria (Belize)': 'Nizhee Corozal',
    'A.F.C. Euro Kickers': 'Euro Kickers',
    'Eurokickers': 'Euro Kickers',
    'Boulder Nova': 'Colorado Rapids U-23',

    'LA Blues 23': 'OC Blues Strikers FC',

    'Minnesota Stars FC': 'Minnesota United FC',
    'CFC Azul': 'Connecticut FC Azul',
    'University of Nebraska at Omaha': 'University of Nebraska Omaha',
    'Phoenix FC Wolves': 'Phoenix FC',
    'Pennsylvania Freedom': 'Philadelphia Freedom', # name map
    'Oklahoma City Stampede': 'Oklahoma City Slickers', # name map
    'San Francisco All-Blacks United': 'San Francisco Seals', # name map
    'San Francisco All Blacks': 'San Francisco Seals', # name map
    'San Fernando Valley Golden Eagles': 'Los Angeles Heroes', # name map
    'San Fernando Valley Heroes': 'Los Angeles Heroes', # name map
    'Lubbock Lasers': 'Lubbock Lazers',

    'Hampton Roads Hurricane': 'Virginia Beach Mariners',
    'Los Angeles Salsa U-23\'s': 'Los Angeles Cobras',
    'East Los Angeles Cobras': 'Los Angeles Cobras',
    'East L.A. Cobras': 'East Los Angeles Cobras',
    'East L.A. Cobars': 'East L.A. Cobras',
    'San Antonio Generals': 'San Antonio Pumas',
    'San Antonio Heat': 'San Antonio Pumas',
    'Permian Basin Shootin\' Stars': 'Permian Basin Shooting Stars',
    'Los Angeles Fireballs': 'Tucson Fireballs',# team map
    'Tampa Bay Cyclones': 'Jacksonville Cyclones',# team map
    'Gwinnett County Steamers': 'Atlanta Express',# team map
    'Greensboro Dynamo': 'Carolina Dynamo',# team map
    'Chicago Stingers': 'Chicago Sockers',# team map
    'Richardson Rockets': 'Dallas Rockets',# team map
    'Eastern Shore Sharks': 'Baltimore Bays',# team map
    'Columbus Xoggz': 'Ohio Xoggz', # team map
    'Columbia Heat': 'Columbia Spirit', # team map
    'Vermont Wanderers': 'Vermont Voltage', # team map
    'Tyler Lightning': 'Texas Lightning', # team map
    'Dallas Lightning': 'Texas Lightning', # team map
    'South Florida Sun': 'Fort Lauderdale Sun', # team map
    'Charlotte Gold': 'Carolina Lightnin', # team map
    'Carolina Lightnin': 'Carolina Lightnin\'',
    'Atlanta Lightning': 'Atlanta Quicksilver',  # team map
    'Georgia Steamers': 'Atlanta Quicksilver', # team map
    'Atlanta Lasers': 'Atlanta Magic', # team map
    'Arkansas A\'s': 'Arkansas Diamonds', # team map
    'Ann Arbor Elites': 'Michigan Madness',# team map
    'Albuquerque Geckos': 'Sacramento Geckos', # team map
    'Albuquerque Gunners': 'New Mexico Chiles',
    'Albuquerque Outlaws': 'New Mexico Chiles',
    'New Mexico Roadrunners': 'New Mexico Chiles',

    'Bolívar': 'Club Bolívar',

    'Minervén': 'AC Minervén FC',
    'Texas Dutch Lions': 'Houston Dutch Lions',# name map
    'Seattle Wolves': 'Washington Crossfire',# name map

    'Portland Timbers U23\'s': 'Portland Timbers U23s',
    'Orlando City U23': 'Orlando City U-23',
    'Colorado Springs Stampede': 'Colorado Springs Ascent',#name map
    'New Hampshire Phantoms': 'Seacoast United Phantoms',#name map
    'Augusta Fireball': 'Augusta FireBall',
    'Daytona Tigers': 'Orlando Nighthawks',#name map
    'Lancaster Rattlers': 'FC Santa Clarita', #name map
    'Lafayette Lightning': 'Austin Lightning',#name map

    'Hamilton Rage': 'K-W United FC',#name map
    'K-W United': 'K-W United FC',

    'Grand Rapids Explosion': 'West Michigan Edge',#name map
    'West Michigan Explosion': 'West Michigan Edge',#name map
    'Abbotsford Mariners': 'Fraser Valley Mariners', #name map
    'Abbotsford Athletes in Action': 'Fraser Valley Mariners', #name map
    'Fox River Rebels': 'Wisconsin Rebels', #name map
    'Colorado Comets': 'Denver Cougars', #name map
    'Dayton Gems': 'Dayton Gemini', # name map
    'RGV Grandes FC': 'Rio Grande Valley Grandes',
    'FC Jax Destroyers': 'FC JAX Destroyers',
    'Jax Destroyers': 'FC JAX Destroyers',
    'New Jersey Rangers FC': 'New Jersey Rangers',
    'Victoria Highlanders FC': 'Victoria Highlanders',
    'North Sound SeaWolves FC': 'North Sound SeaWolves',
    'Palmetto FC Bantams': 'SC United Bantams',
    'Ocean City FC': 'Ocean City Nor\'easters',# name map.
    'Chattanooga Railroaders': 'Chattanooga Express', # name map
    'Capital District Shockers': 'Albany Alleycats',# name map.
    'Central Florida Lionhearts': 'Central Florida Kraze', # name map.
    'Miami Freedom': 'Miami Sharks',
    'Valenciennes': 'Valenciennes FC',


    'Vit. Guimarães': 'Vitória de Guimarães',
    'VSC Guimarães': 'Vitória de Guimarães',
    'Guimaraes': 'Vitória de Guimarães',
    'Vitoria Guimaraes': 'Vitória de Guimarães',
    'Vitoria Guimarães': 'Vitória de Guimarães',

    '1 Schwechater SC': 'SV Schwechat',
    'Schwechater': 'SV Schwechat',
    'Groningen': 'FC Groningen',

    'Zenit St. Petersburg': 'FC Zenit Saint Petersburg',
    'Zenit Leningrad': 'FC Zenit Saint Petersburg',
    'Zenit Leningrad (URS)': 'FC Zenit Saint Petersburg',



    'Inter-Bratislava': 'FK Inter Bratislava',
    'CH Bratislava': 'FK Inter Bratislava',

    'RSD Alcala': 'RSD Alcalá',
    'VfL Bad Oldesloe': 'VfL Oldesloe',
    'SV Kaufbeuren': 'SpVgg Kaufbeuren',

    'Southampton F.C.': 'Southampton',
    'Southampton Fc': 'Southampton',

    'Vitória FC (Setúbal)': 'Vitória de Setúbal',
    'Vit. Setúbal': 'Vitória de Setúbal',
    'VFC Setubal': 'Vitória de Setúbal',
    'Vitoria Setubal': 'Vitória de Setúbal',
    'Vitória Setúbal': 'Vitória de Setúbal',

    'Hibernian Fc': 'Hibernian F.C.',
    'Hibernians': 'Hibernian F.C.',
    'Hibernian': 'Hibernian F.C.',

    'ASV Bergsdorf': 'ASV Bergedorf',
    'VfL Osnabruck': 'VfL Osnabrück',
    'SV Muenster': 'SV Munster',
    
    'Girona': 'Girona FC',
    'Luton Town': 'Luton Town F.C.',
    'National Fast Club': 'National Fast Clube',
    'Coleraine (Northern Ireland)': 'Coleraine F.C.',
    'Coleraine': 'Coleraine F.C.',


    'Cantazaro': 'US Catazaro 1929',
    'Catazaro': 'US Catazaro 1929',
    'Southampton Saints': 'Southampton F.C.',

    'R.S.C. Anderlecht': 'RSC Anderlecht',
    'Anderlecht': 'RSC Anderlecht',
    'RSC Anderlecht (Bel)': 'RSC Anderlecht',
    'Anderlecht (Bel)': 'RSC Anderlecht',

    'Bangor City': 'Bangor City F.C.',
    'Den Bosch': 'FC Den Bosch',
    'DS79': 'FC Dordrecht',
    'SC Wageningen': 'SV Wageningen',
    'Naestved': 'Næstved BK',
    'BK Hacken': 'BK Häcken',
    'Stabaek': 'Stabæk Fotball',
    'SV Itsehoe': 'Itzehoer SV',
    'Ado Den Haag': 'ADO Den Haag',


    'C.A. Peñarol': 'CA Peñarol',
    'CD San Jose': 'CD San José',
    # Santiago>, <Team: SANTIAGO>]
    'Upton Park FC': 'Upton Park F.C.',
    'Nautico': 'Náutico',
    
    'Atlético De Madrid': 'Atlético de Madrid',
    'Atletico de Madrid': 'Atlético de Madrid',
    'Atlético de Madrid': 'Atlético Madrid',
    'Atletico Madrid': 'Atlético Madrid',
    'Atlético Madrid (Esp)': 'Atlético Madrid',
    
    'Galt FC': 'Galt F.C.',

    'Lduq': 'LDU Quito',
    'L.D.U. Quito': 'LDU Quito',
    'LDU de Quito': 'LDU Quito',


    'US Pro 40': 'Nike Project-40',
    'US Project 40': 'Nike Project-40',
    'Project-40': 'Nike Project-40',
    'Project40': 'Nike Project-40',
    'Project 40': 'Nike Project-40',
    
    'Independencia': 'Independência',

    
    'EC Vitoria': 'EC Vitória',


    'University of Massachusetts-Amherst': 'University of Massachusetts Amherst',

    'Houston University': 'University of Houston',
    'Rockhurst College': 'Rockhurst University',
    'Kutztown State College': 'Kutztown University',
    'Glassboro State College': 'Rowan University',
    'Florida Tech': 'Florida Institute of Technology',
    'South Florida University': 'University of South Florida',
    'Metro State College': 'Metropolitan State University of Denver',
    'South East Massachusetts University': 'University of Massachusetts Dartmouth',
    'San Francisco State': 'San Francisco State University',
    'North Adams State College': 'Massachusetts College of Liberal Arts',
    'Southern Maine University': 'University of Southern Maine',
    'Spring Arbor College': 'Spring Arbor University',
    'Brockport State College': 'SUNY Brockport',
    'Lock Haven State College': 'Lock Haven University',
    'Plymouth State': 'Plymouth State University',
    'DeAnza Junior College': 'DeAnza College',

    'USC Spartanburg': 'University of South Carolina Upstate',
    'Cortland State College': 'SUNY Cortland',

    'Foothill Junior College': 'Foothill College',
    'Auburn University-Montgomery': 'Auburn University at Montgomery',

    'MPS Portland Phoenix': 'GPS Portland Phoenix',
    'Portland Phoenix': 'GPS Portland Phoenix',
    'Incarnate Word': 'University of the Incarnate Word',
    'Mount Union College': 'University of Mount Union',
    'Nova Southeastern': 'Nova Southeastern University',
    'Nike Project 40': 'Project 40',
    'US Project 40': 'Project 40',
    'San Antonio Scorpions FC': 'San Antonio Scorpions',

    # add name-maps.
    'Willamette Valley Firebirds': 'Palo Alto Firebirds',
    'Portland Firebirds': 'Palo Alto Firebirds',
    'Silicon Valley Firebirds': 'Palo Alto Firebirds',

    'Embry-Riddle Aeronautical University': 'Embry-Riddle',

    'Tulsa Ambush': 'St. Louis Ambush', # set name map.

    'Bahia': 'EC Bahia',
    'E.C. Bahia': 'EC Bahia',

    'Trujillanos': 'Trujillanos FC',
    'Club Blooming': 'Blooming',
    'MTK': 'MTK Budapest',

    

    'Brooklyn Hispanos': 'Brooklyn Hispano',

    'Diriangen': 'Diriangén FC',

    
    'Baltimore Spirit': 'Baltimore Blast',

    'St Lawrence': 'St. Lawrence',
    'Litoral': 'Lítoral',
    'Lítoral': 'Club Litoral',

    'Pepsi Cola': 'Pepsi-Cola',
    'Critchleys': 'Critchley\'s',
    'Mexico FC': 'México FC',
    'IRT FC': 'IRT F.C.',
    'Caribbean All Stars': 'Caribbean All-Stars',
    'St Lawrence': 'St. Lawrence',
    'Dublin FC': 'Dublin F.C.',
    'Santa Lucía': 'Santa Lucia', # Is this saint lucia? (NO!)


    # Uncategorized.

    #Berlin (Kitchener) High School
    #Bigelow Sanford (NY)

    # Canada

     # MLS

    'Impact De Montreal': 'Montreal Impact',
    'Impact de Montreal': 'Montreal Impact',

     # CPL

    'York9': 'Inter Toronto FC',
    'York United': 'Inter Toronto FC',
    'Atletico Ottawa': 'Atlético Ottawa',

     # Other

    'Toronto Thunderhawks': 'Toronto ThunderHawks',

    'Detroit-Windsor Border Stars': 'Windsor Stars',

    'Hamilton Steloos': 'Hamilton Stelcos',

    #Canadian Explosives (Beloeil)

    #Fairhill (New York)
    #Hermes SC (East NY)
    #Swiss SC (East NY)

    #German SC (Reading)

    #K.B. & B. (93)

    #Olympique (Morne-a-l'Eau)

    #St. Raphael (D5)

    #Vancouver (CSL)
    #Montreal (CSL)
    'Vancouver Canadians': 'Vancouver Royals',
    'Vancouver Royal Canadians': 'Vancouver Royals',

    'Italian Army Team': 'Italian Army',
    'All Skane': 'All-Skane',

    # Asia

    # Japan

    'Kasiwa Reysol': 'Kashiwa Reysol',

    'V.Kawasaki': 'Verdy Kawasaki',

    'Fukuoka': 'Avispa Fukuoka',

    'C.Osaka': 'Cerezo Osaka',

    'Yokohama Flugels': 'Yokohama Flügels',

    'JEF United': 'JEF United Ichihara Chiba',


    'Yokohama Marinos': 'Yokohama F. Marinos',
    'Yokohama F-Marinos': 'Yokohama F. Marinos',
    'Yokohama F.Marinos': 'Yokohama F. Marinos',
    'Urawa Reds': 'Urawa Red Diamonds',

    'Verdy Kawasaki': 'Tokyo Verdy', # name map.
    'Tokyo Verdy 1969': 'Tokyo Verdy',
    'Tokyo Verdy 69': 'Tokyo Verdy',
    
    'F.C. Tokyo': 'FC Tokyo',

    'Nagoya Grampus Eight': 'Nagoya Grampus',

    'Kyoto Purple Sanga': 'Kyoto Sanga FC',
    'Kyoto Sanga F.C.': 'Kyoto Sanga FC',

    'Jubilo Iwata': 'Júbilo Iwata',
    'JEF United Chiba': 'JEF United Ichihara Chiba',
    'JEF United Ichihara': 'JEF United Ichihara Chiba',
    'Gamba Osaka - Japan': 'Gamba Osaka',
    'Grampus Eight - Japan': 'Grampus Eight',
    'Grampus Eight': 'Nagoya Grampus',
    'Instant Dict - Japan': 'Double Flower FA',
    'Shimizu S-Pulse - Japan': 'Shimizu S-Pulse',
    'Vissel Kobe - Japan': 'Vissel Kobe',

    # China

    #East (Hong Kong)
    'Beijing Guo\'an': 'Beijing Guoan',
    'Beijing Guoan - China': 'Beijing Guoan',
    'Guoan (China)': 'Beijing Guoan F.C.',
    'Quan Xing (China)': 'Sichuan Guancheng',
    'Sichuan Quanxing - China': 'Sichuan Quanxing',
    'Sichuan Quanxing': 'Sichuan Guancheng',
    'Team Seiko (Hong Kong)': 'Team Seiko',
    'Team Seiko': 'Seiko SA',

    # Egypt

    'Al Ahli Fc': 'Al Ahly SC',
    'Al-Ahly': 'Al Ahly SC',
    'Al-Ahly S.C.': 'Al Ahly SC',
    'Al Ahly': 'Al Ahly SC',
    'Al Ahly Sporting Club': 'Al Ahly SC',
    'Zamalek': 'Zamalek SC',

    # Australia

    'West Sydney Wanderers': 'Western Sydney Wanderers',

    'South Melbourne':'South Melbourne FC',


    'Adelaide United FC': 'Adelaide United',
    'Brisbane Roar FC': 'Brisbane Roar',
    'Central Coast Mariners FC': 'Central Coast Mariners',
    'Gold Coast United FC': 'Gold Coast United',

    'Melbourne Victory - Australia': 'Melbourne Victory',
    'Melbourne Victory FC': 'Melbourne Victory',
    'Melbourne Heart FC': 'Melbourne Heart',

    'Newcastle United Jets': 'Newcastle Jets',
    'Newcastle Jets FC': 'Newcastle Jets',
    'Newcastle United Jets FC': 'Newcastle United Jets',

    'North Queensland Fury FC': 'North Queensland Fury',
    'Perth Glory FC': 'Perth Glory',
    'Queensland Roar FC': 'Queensland Roar',

    'Victoria State (Australia)': 'Victoria XI',

    'Adelaide City (Australia)': 'Adelaide City',
    'Sydney FC - Australia': 'Sydney FC',
    'Queensland Roar': 'Brisbane Roar',

    # New Zealand
    'New Zealand Knights FC': 'New Zealand Knights',
    'Auckland City': 'Auckland City FC',
    'Wellington Phoenix FC': 'Wellington Phoenix',


    # Europe

    # Israel
    'hapoel': 'Hapoel',

    'Hapoel Tel Aviv F.C.': 'Hapoel Tel Aviv',

    'Hapoel Petah Tikvah': 'Hapoel Petah Tikva',
    'Hapoel Petah Tikva F.C.': 'Hapoel Petah Tikva',

    'Hapoel Haifa': 'Hapoel Haifa F.C.',

    'Maccabi Tel Aviv F.C.': 'Maccabi Tel Aviv',
    'Maccabi Tel-Aviv Fc': 'Maccabi Tel Aviv',


    # Portugal

    'Oporto (Portugal)': 'Oporto',



    'Uniao de Leiria': 'UD Leiria',
    'Uniao Leiria': 'UD Leiria',
    'Uniã o de Leiria': 'UD Leiria',
    'Leiria': 'UD Leiria',
    'U. Leiria': 'UD Leiria',
    'U Leiria': 'UD Leiria',
    'União de Leiria': 'UD Leiria',

    'Estrela Amadora': 'Estrela da Amadora',
    'Estrela A': 'Estrela da Amadora',
    'CF Estrela da Amadora': 'Estrela da Amadora',
    'E. Amadora': 'Estrela da Amadora',
    'CF Estrela de Amadora': 'Estrela da Amadora',
    'Estrêla Amadora': 'Estrela da Amadora',
    'Estrêla da Amadora': 'Estrela da Amadora',
    'CFE Amadora': 'Estrela da Amadora',

    'Desp. Chaves': 'GD Chaves',

    'Leixões SC': 'Leixões',

    'Leça': 'Leça FC',

    'SC Olhanense': 'Olhanense',

    'Moreirense FC': 'Moreirense',

    'FC Penafiel': 'Penafiel',




    'Beira Mar': 'Beira-Mar',

    'Académica De Coimbra': 'Académica de Coimbra',
    'Academica Coimbra': 'Académica de Coimbra',
    'Académica Coimbra': 'Académica de Coimbra',


    #Oporto (Portugal)
    #Caldas (Portugal)
    
    'Porto': 'FC Porto',
    'FC Porto (Por)': 'FC Porto',


    'Sporting (Portugal)': 'Sporting Lisbon',
    'Sporting Club Lisbon': 'Sporting Lisbon',
    'Sporting C. Lisboa': 'Sporting Lisbon',
    'Sporting CP (Por)': 'Sporting Lisbon',
    'Sporting CP (Lisbon-Por)': 'Sporting Lisbon',
    'Sporting CP Lisbon (Por)': 'Sporting Lisbon',
    'Sporting CP Lisboa (Por)': 'Sporting Lisbon',
    'Sporting Lisbon': 'Sporting CP',

    'Academica de Coimbra': 'Académica de Coimbra',

    #'Benfica': 'S.L. Benfica',

    'Benfica (Por)': 'S.L. Benfica',
    'Benfica - Portugal': 'S.L. Benfica',
    'Benfica (Portugal)': 'S.L. Benfica',
    'S.L. Benfica': 'SL Benfica',
    'SL Benfica (Por)': 'SL Benfica',

    'Marinhense': 'A.C. Marinhense',

    'Cs Marítimo':  'C.S. Maritimo',
    'Mariitimo': 'C.S. Maritimo',
    'Maritimo': 'C.S. Maritimo',
    'C.S. Maritimo': 'Marítimo',
    'CS Maritimo': 'Marítimo',
    'CS Marítimo': 'Marítimo',

    'SC Salgueiros': 'Salgueiros',

    'Sp. Farense': 'Farense',
    'SC Farense': 'Farense',
    'Sporting Farense': 'Farense',

    'Boavista FC': 'Boavista',

    'FC do Porto': 'FC Porto',


    'FC Paços de Ferreira': 'Paços de Ferreira',
    'Paços F': 'Paços de Ferreira',
    'Pacos De Ferreira': 'Paços de Ferreira',
    'Pacos Ferreira': 'Paços de Ferreira',
    'Paços Ferreira': 'Paços de Ferreira',
    'Paços de F': 'Paços de Ferreira',

    'Vitória Guimarães': 'Vitória de Guimarães',
    'Vitória SC (Guimarães)': 'Vitória de Guimarães',
    'Guimarães': 'Vitória de Guimarães',
    'Vitória De Guimaraes': 'Vitória de Guimarães',
    'Vitoria SC (Guimaraes)': 'Vitória de Guimarães',

    'Vitoria SC (Guimarães)': 'Vitória de Guimarães',
    'Vitoria G': 'Vitória de Guimarães',
    'Vitória G': 'Vitória de Guimarães',

    'Vitoria S': 'Vitória de Setúbal',
    'Setubal': 'Vitória de Setúbal',
    'Setúbal': 'Vitória de Setúbal',
    'Vitoria FC (Setubal)': 'Vitória de Setúbal',
    'Vitoria FC (Setúbal)': 'Vitória de Setúbal',


    'CF Belenenses': 'C.F. Os Belenenses',
    'CF "Os Belenenses"': 'C.F. Os Belenenses',
    'Belenenses - Portugal': 'C.F. Os Belenenses',
    'Belenenses': 'C.F. Os Belenenses',
    'CF \'Os Belenenses\'': 'C.F. Os Belenenses',
    'C.F. Os Belenenses': 'Os Belenenses',

    'CD Santa Clara (Azores)': 'Santa Clara',
    'Santa Clara - Portugal': 'Santa Clara',
    'Santa Clara': 'C.D. Santa Clara',
    'Sporting Lisbon XI - Portugal': 'Sporting Lisbon XI',
    
    'Sporting Clube de Portugal': 'Sporting Lisbon',
    'varzim': 'Varzim',
    'Varzim': 'Varzim S.C.',
    'Varzim SC': 'Varzim S.C.',
    'Varzim (Portugal)': 'Varzim S.C.',

    'Uniao de Santarem': 'União de Santarém',

    # Spain

    'Elche': 'Elche CF',


    'Racing Santander': 'Racing de Santander',

    'Granada': 'Granada CF',
    'Granada Cf': 'Granada CF',

    'Merida': 'Mérida',
    'Mérida CP': 'Mérida',

    'Gimnástic De Tarragona': 'Gimnàstic de Tarragona',
    'Club Gimnàstic de Tarragona': 'Gimnàstic de Tarragona',

    'Xerez CD': 'Xerez',

    'Villarreal CF': 'Villarreal',

    'Getafe CF': 'Getafe',

    'Hércules CF (Alicante)': 'Hercules CF',

    'Levante UD': 'Levante',

    'Málaga CF': 'Málaga',

    'CD Numancia (Soria)': 'Numancia',
    'Numancia (Soria)': 'Numancia',

    'RC Recreativo de Huelva': 'Recreativo Huelva',
    'Recreativo De Huelva': 'Recreativo Huelva',

    'CD Tenerife': 'Tenerife',

    'Cádiz CF': 'Cádiz',

    'Deportivo Alavés (Vitoria)': 'Deportivo Alavés',

    'Albacete Balompié': 'Albacete',

    'CA Osasuna (Pamplona)': 'Osasuna',

    'Real Betis Balompié (Sevilla)': 'Real Betis',
    'Real Betis (Sevilla)': 'Real Betis',

    'CD Tenerife (Santa Cruz)': 'CD Tenerife',

    'Levante UD (Valencia)': 'Levante UD',


    'RCD Mallorca (Palma de M.)': 'Mallorca',
    'RCD Mallorca': 'Mallorca',

    'Real Murcia CF': 'Real Murcia',

    'Real Sociedad (San Sebastián)': 'Real Sociedad',
    'Real Sociedad (Esp)': 'Real Sociedad',

    'Las Palmas': 'UD Las Palmas',


    'Deportivo Lc': 'Deportivo La Coruña',
    'Deportivo la Coruna': 'Deportivo La Coruña',
    'RC Deportivo (La Coruña)': 'Deportivo La Coruña',
    'RC Deportivo': 'Deportivo La Coruña',

    'Compostela': 'SD Compostela',

    'Rayo Vallecano (Madrid)': 'Rayo Vallecano',
    'Rayo': 'Rayo Vallecano',

    'Malaga': 'Málaga',

    'Alaves': 'Deportivo Alavés',
    'Alavés': 'Deportivo Alavés',


    'Gimnástic': 'Gimnàstic',
    'Murcia': 'Real Murcia',
    'Salamanca': 'UD Salamanca',
    'Oviedo': 'Real Oviedo', # Too broad?

    'Celta de Vigo': 'Celta Vigo',
    'Celta De Vigo': 'Celta Vigo',
    'Celta': 'Celta Vigo',
    'RC Celta (Vigo)': 'Celta Vigo',
    'RC Celta': 'Celta Vigo',

    'Sporting De Gijón': 'Sporting de Gijón',
    'Sporting de Gijon': 'Sporting de Gijón',

    'Madrid': 'Real Madrid', # Uhhhhh...
    'Real Madrid CF': 'Real Madrid',
    'Real Madrid - Spain': 'Real Madrid',
    'Real Madrid (Esp)': 'Real Madrid',


    'Hércules': 'Hércules CF', # dubious
    'Hercules CF': 'Hércules CF', # dubious

    'Xérez': 'Xerez',

    'Zaragoza': 'Real Zaragoza',

    'Athletic De Bilbao': 'Athletic Bilbao',
    'Athletic Bilbao (Esp)': 'Athletic Bilbao',
    'Athletic Club de Bilbao': 'Athletic Bilbao',
    'Athletic de Bilbao': 'Athletic Bilbao',
    'Atletico Bilbao': 'Athletic Bilbao',

    'Betis': 'Real Betis',

    'Almeria': 'Almería',
    'UD Almería': 'Almería',

    'FC Barcelona (Esp)': 'FC Barcelona',
    #'Barcelona': 'FC Barcelona',
    'Barcelona (Spain)': 'FC Barcelona',
    'Barcelona B': 'FC Barcelona B',

    'RCD Espanyol (Barcelona)': 'RCD Espanyol',
    'RCD Español': 'RCD Espanyol',
    'Español': 'RCD Espanyol',
    'Espanyol': 'RCD Espanyol',

    'Sabadell': 'CE Sabadell FC',
    'Valladolid': 'Real Valladolid',

    'Sevilla': 'Sevilla FC',
    'Sevilla CF': 'Sevilla FC',

    #'Valencia': 'Valencia CF',
    'Valencia (Esp)': 'Valencia CF',

    'Vitoria Setúbal': 'Vitória de Setúbal',

    # Belgium

    'AFC Tubize': 'Tubize',

    'R.E. Mouscron': 'Mouscron',
    'R. Excelsior Mouscron': 'Mouscron',
    'Excelsior Mouscron': 'Mouscron',

    'Excelsior (Rotterdam)': 'Excelsior', # ambiguous.


    'Molenbeek': 'RWD Molenbeek',
    'R.W.D. Molenbeek': 'RWD Molenbeek',
    'RWDM': 'RWD Molenbeek', # dubious.
    'RWD Molenbeek (Bel)': 'RWD Molenbeek',

    'FC Verbroedering Dender EH': 'Dender EH',

    
    'FC Brussels': 'RWDM Brussels', 

    'FC Brugge': 'Club Brugge',
    'Club Brugge (Bel)': 'Club Brugge',
    'Club Brugge KV': 'Club Brugge',
    'Brugge': 'Club Brugge',

    'Cerc. Brugge': 'Cercle Brugge',
    'Cercle Brugge K.S.V.': 'Cercle Brugge',
    'Cercle Brugge KSV': 'Cercle Brugge',

    'Royal Antwerp FC': 'Royal Antwerp',
    'RFC Antwerp': 'Royal Antwerp',
    'Antwerp': 'Royal Antwerp',
    'Royal Antwerp F.C.': 'Royal Antwerp',
    'R. Antwerp F.C.': 'Royal Antwerp',
    
    'Sporting du Pays de Charleroi': 'Charleroi SC',
    'RSC Charleroi': 'Charleroi SC',
    'R. Charleroi S.C.': 'Charleroi SC',
    'R Charleroi SC': 'Charleroi SC',
    'R. Charleroi SC': 'Charleroi SC',
    'SC Charleroi': 'Charleroi SC',
    'Charleroi': 'Charleroi SC',

    'KAA Gent': 'Gent',
    'K.A.A. Gent': 'Gent',
    'RAA Gent': 'Gent',
    'AA Gent': 'Gent',
    'Kaa Gent': 'Gent',




    #'K.S.V. Waregem': 
    'SV Zulte-Waregem': 'Zulte Waregem',
    'Zulte-Waregem': 'Zulte Waregem',
    'Zulte-W.': 'Zulte Waregem',

    'KV Kortrĳk': 'KV Kortrijk',
    'Kortrijk': 'KV Kortrijk',

    'Sint Truidense VV': 'Sint-Truiden',
    'St Truiden': 'Sint-Truiden',
    'Sint Truiden': 'Sint-Truiden',
    'K. Sint-Truiden VV': 'Sint-Truiden',
    'K. Sint-Truidense V.V.': 'Sint-Truiden',

    'K.R.C. Genk': 'Racing Genk',
    'RC Genk': 'Racing Genk',
    'KRC Genk': 'Racing Genk',
    'Krc Genk': 'Racing Genk',
    'Genk': 'Racing Genk',

    'Evian': 'Evian TG',
    'Evian Tg': 'Evian TG',
    'Evian-Thonon': 'Evian TG',
    'Evian-Thonon Gaillard FC': 'Evian TG',

    #'R.F.C. de Liège'

    'RAEC Mons': 'Mons',

    'Standard Luik': 'Standard Liège',
    'Standard CL (Liège)': 'Standard Liège',
    'Standard CL': 'Standard Liège',
    'R. Standard de Liège': 'Standard Liège',
    'Standard Lieja': 'Standard Liège',
    'Liege': 'Standard Liège',
    'St Liege': 'Standard Liège',
    'Standard Liege': 'Standard Liège',
    'Standard Liege (Bel)': 'Standard Liège',
    'Standard CL Ličge (Bel)': 'Standard Liège',
    'Standard CL Liège (Bel)': 'Standard Liège',
    'Standard CL (Liège-Bel)': 'Standard Liège',
    'Standard Ličge (Bel)': 'Standard Liège',

    # Luxembourg

    'Union Luxembourg (Lux)': 'Union Luxembourg',
    'Progrčs Niederkorn (Lux)': 'Progrès Niedercorn',


    'CA Spora Luxembourg': 'Spora Luxembourg',
    'CA Spora Luxembourg (Lux)': 'Spora Luxembourg',
    'CA Spora (Lux)': 'Spora Luxembourg',

    'Avenir Beggen': 'FC Avenir Beggen',

    # Croatia

    'Hadjuk Split': 'Hajduk Split',
    'Hajduk Split (YUG)': 'Hajduk Split',

    'Radnički Split': 'RNK Split',
    'Rnk Split': 'RNK Split',

    'Lokomotiva Zagreb': 'NK Lokomotiva',

    'Cibalia Vinkovci': 'HNK Cibalia',

    # Netherlands

    'AZ (Alkmaar)': 'AZ',
    'Az Alkmaar': 'AZ',
    'AZ Alkmaar (Ned)': 'AZ Alkmaar',
    'AZ Alkmaar': 'AZ',

    'Rkc Waalwijk': 'RKC Waalwijk',
    'RKC Waalwĳk': 'RKC Waalwijk',
    'RKC': 'RKC Waalwijk',

    'Vvv-Venlo': 'VVV-Venlo',
    'VVV': 'VVV-Venlo',
    'VVV Venlo': 'VVV-Venlo',
    'PEC': 'PEC Zwolle',

    'NAC': 'NAC Breda',
    'Nac Breda': 'NAC Breda',

    'NEC Nijmegen': 'NEC',
    'NEC (Nijmegen)': 'NEC',
    'NEC (Nĳmegen)': 'NEC',
    'N.E.C.': 'NEC',
    'Nec': 'NEC',

    'FC Twente (Enschede)': 'FC Twente',
    'Fc Twente': 'FC Twente',
    'Twente': 'FC Twente',

    'Utrecht': 'FC Utrecht',

    'ADO': 'Ado Den Haag',
    'A.D.O.': 'ADO Den Haag',

    'Ajax (Amsterdam)': 'Ajax',
    'AFC Ajax - Netherlands': 'Ajax',
    'Ajax (Ned)': 'Ajax',
    'Ajax - Netherlands': 'Ajax',
    'Ajax': 'AFC Ajax',
    'Ajax Amsterdam': 'AFC Ajax',

    'Ajax Reserves': 'AFC Ajax Reserves',

    'PSV (Eindhoven)': 'PSV Eindhoven',
    'Psv': 'PSV Eindhoven',
    'PSV Eindhoven -- Netherlands': 'PSV Eindhoven',
    'PSV': 'PSV Eindhoven',
    'PSV Eindhoven (Ned)': 'PSV Eindhoven',

    'Feyenoord (Rotterdam)': 'Feyenoord',
    'Feyenoord - Netherlands': 'Feyenoord',
    'Feyenoord (Ned)': 'Feyenoord',
    'Feĳenoord (Ned)': 'Feyenoord',
    'Feijenoord': 'Feyenoord',
    'Feijenoord (Ned)': 'Feyenoord',
    'Fejenoord (Ned)': 'Feyenoord',

    'Heracles': 'Heracles Almelo',

    'Rapid JC Heerlen': 'Roda JC',
    'Roda JC (Kerkrade)': 'Roda JC',
    'Roda JC Kerkrade': 'Roda JC',
    'Roda': 'Roda JC',
    'Roda Jc Kerkrade': 'Roda JC',

    # France

    'Girondins de Bordeaux': 'Bordeaux',
    'FC Girondins de Bordeaux': 'Bordeaux',
    'FC Girondins Bordeaux': 'FC Girondins de Bordeaux',
    'Girondins Bordeaux (Fra)': 'FC Girondins de Bordeaux',


    'Nancy': 'AS Nancy',
    'Lorient': 'FC Lorient',

    'Sochaux': 'FC Sochaux',

    'Istres (D2)': 'FC Istres',

    'Nantes (France)': 'FC Nantes',

    'Nice reserves (D4)': 'OGC Nice Reserves',

    'Stade Rennais F.C.': 'Rennes',

    'Montpellier': 'Montpellier Herault SC',

    'Strasbourg': 'RC Strasbourg',
    'RC Strasbourg (Fra)': 'RC Strasbourg',

    'Saint-Etienne': 'Saint-Étienne',
 
    'Olympic Gymnaste Club Nice': 'OGC Nice',
    'Nice': 'OGC Nice',
    'Olympique Nice': 'OGC Nice',
    'OGC Nice (Fra)': 'OGC Nice',

    'Paris St.-Germain': 'Paris Saint-Germain F.C.',
    'Paris SG': 'Paris Saint-Germain F.C.',
    'Paris-SG': 'Paris Saint-Germain F.C.',
    'PSG': 'Paris Saint-Germain F.C.',
    'Psg': 'Paris Saint-Germain F.C.',
    'Paris Saint-Germain (Fra)': 'Paris Saint-Germain',
    'Paris St. Germain': 'Paris Saint-Germain',
    'Paris Saint-Germain F.C.': 'Paris Saint-Germain',

    # Serbia

    'Red Star Belgrade (YUG)': 'Red Star Belgrade',
    'Red Star (Belgrade)': 'Red Star Belgrade',
    'SD Crvena Zvezda': 'Red Star Belgrade',
    'Crvena Zvezda Beograd (YUG)': 'Red Star Belgrade',
    'Crvena zvezda': 'SD Crvena Zvezda',
    'Crvena Zvezda': 'SD Crvena Zvezda',
    'Crvena zvezda Beograd': 'Red Star Belgrade',
    'Red Star (Yug)': 'Red Star Belgrade',
    'Crvena zvezda (Yug)': 'Red Star Belgrade',


    # Partizan Belgrad, Beograd, ...FK Partizan
    'Fk Partizan': 'Partizan Belgrade',
    'FK Partizan (Yug)': 'Partizan Belgrade',
    'FK Partizan': 'Partizan Belgrade',
    'Partizan': 'Partizan Belgrade',
    'Partizan Beograd': 'Partizan Belgrade',
    'Partizan Belgrad': 'Partizan Belgrade',
    'Partizan Beograd (YUG)': 'Partizan Belgrade',
    'Partizan Belgrade (YUG)': 'Partizan Belgrade',

    # Romania


    'Petrolul Ploiesti (Rom)': 'Petrolul Ploiesti',
    'Petrolul Ploieşti (Rom)': 'Petrolul Ploieşti',
    'Petrolul Ploiesti': 'Petrolul Ploieşti',
    'FC Petrolul Ploiesti': 'Petrolul Ploieşti',
    'FC Petrolul Ploieşti': 'Petrolul Ploieşti',
    'FC Ploiesti': 'Petrolul Ploieşti',
    'Petrolul': 'Petrolul Ploieşti',


    'Astra Ploiesti': 'Astra Ploieşti',
    'SC Astra Ploiesti': 'Astra Ploieşti',
    'Astra': 'Astra Ploieşti',
    'FC Astra Giurgiu': 'Astra Giurgiu',
    'Astra Ploieşti': 'Astra Giurgiu',

    'FC Olt Scorniceşti': 'Olt Scorniceşti',

    'CS Concordia Chiajna': 'Concordia Chiajna',

    'FC Victoria Brăneşti': 'Victoria Brăneşti',
    
    'F.C. Maramures Baia Mare [P]': 'FCM Baia Mare',

    'SC Bacău': 'FCM Bacău',
    'FCM Bacau': 'FCM Bacău',
    'AS Bacau': 'FCM Bacău',
    'FC Selena Bacau': 'FCM Bacău',

    'F.C. Inter Sibiu': 'FC Inter Sibiu',



    'Dynamo Bucharest': 'Dinamo Bucharest',
    'Dinamo Bucureşti (Rom)': 'Dinamo Bucharest',
    'Dinamo Bucuresti (Rom)': 'Dinamo Bucharest',
    'FC Dinamo Bucuresti': 'Dinamo Bucharest',
    'FC Dinamo Bucureşti': 'Dinamo Bucharest',
    'Dinamo Bucuresti': 'Dinamo Bucharest',
    'Dinamo Bucureşti': 'Dinamo Bucharest',



    'Ceahlaul Piatra Neamt': 'Ceahlăul Piatra Neamț',
    'Ceahlaul': 'Ceahlăul Piatra Neamț',
    'Ceahlăul': 'Ceahlăul Piatra Neamț',
    'FC Ceahlăul Piatra Neamţ': 'Ceahlăul Piatra Neamț',
    'Ceahlăul Piatra Neamţ': 'Ceahlăul Piatra Neamț',

    'U.M. Timisoara': 'UM Timișoara',
    'UMT': 'UM Timișoara',

    'FCU Politehnica Timişoara': 'Poli Timișoara',
    'FCU Politehnica Timisoara': 'Poli Timișoara',
    'Politehnica Timisoara': 'Poli Timișoara',
    'Politehnica Timişoara': 'Poli Timișoara',
    'Poli AEK': 'Poli Timișoara',
    'Poli Timişoara': 'Poli Timișoara',
    'Poli Timisoara': 'Poli Timișoara',
    'P Timişoara': 'Poli Timișoara',
    'Timişoara': 'Poli Timișoara',
    'Poli AEK Timisoara': 'Poli Timișoara',
    'Politehnica AEK Timisoara': 'Poli Timișoara',
    
    'Vaslui': 'FC Vaslui',
    'SC Vaslui': 'FC Vaslui',

    'Internațional': 'Internațional Curtea de Argeș',

    'Progresul Bucuresti': 'Progresul Bucureşti',
    'Naţional': 'Progresul Bucureşti',
    'FC National Bucuresti': 'Progresul Bucureşti',
    'FC Naţional Bucureşti': 'Progresul Bucureşti',
    'F.C. National Bucuresti': 'Progresul Bucureşti',


    'Otopeni': 'CS Otopeni',

    'Gaz Metan': 'Gaz Metan Mediaș',
    'Gaz Metan Medias': 'Gaz Metan Mediaș',
    'CS Gaz Metan Mediaş': 'Gaz Metan Mediaș',


    'Braşov': 'FC Braşov',

    'FC Farul Constanta': 'FC Farul Constanţa',
    'F.C. Farul Constanta': 'FC Farul Constanţa',
    'Farul': 'FC Farul Constanţa',
    'FC Farul Constanţa': 'Farul Constanța',

    'FC Onesti': 'FCM Dinamo Onești',

    'F.C. Brasov': 'FC Braşov',
    'Brasov': 'FC Braşov',
    'FCM Brasov': 'FC Braşov',
    'FCM Braşov': 'FC Braşov',
    'FC Brasov': 'FC Braşov',

    'Oţelul': 'Oțelul Galați',

    'F.C. Arges Pitesti [P]': 'Argeș Pitești',
    'FC Arges Dacia Pitesti': 'Argeș Pitești',
    'FC Arges Pitesti': 'Argeș Pitești',
    'FC Argeş Piteşti': 'Argeș Pitești',
    'Argeş': 'Argeș Pitești',
    'Piteşti': 'Argeș Pitești',
    'Arges Pitesti': 'Argeș Pitești',
    'Arges Pitesti (Rom)': 'Arges Pitesti',
    'Arges': 'Arges Pitesti',
    'FC 2005 Piteşti': 'Argeș Pitești',

    

    'Apulum Alba-Iulia': 'Unirea Alba Iulia',
    'Alba Iulia': 'Unirea Alba Iulia',

    'Urziceni': 'Unirea Urziceni',
    'Unirea Valahorum Urziceni': 'Unirea Urziceni',

    'Poli Unirea Iasi': 'Poli Iași',
    'Politehnica Iasi': 'Poli Iași',
    'Politehnica Iaşi': 'Poli Iași',
    'Poli Iaşi': 'Poli Iași',
    'P Iaşi': 'Poli Iași',
    'Poli Iasi': 'Poli Iași',

    'G Buzău': 'Gloria Buzău',
    'Gloria Buzau': 'Gloria Buzău',

    'FC Oradea': 'FC Bihor Oradea',
    'Oradea': 'FC Bihor Oradea',

    'Dacia Mioveni': 'CS Mioveni',

    'G Bistriţa': 'Gloria Bistrița',
    'Gloria Bistrita': 'Gloria Bistrița',
    'ACF Gloria 1922 Bistriţa': 'Gloria Bistrița',



    'Universitatea Craiova (Rom)': 'Universitatea Craiova',
    'U Craiova': 'Universitatea Craiova',
    'AS Universitatea FC Craiova': 'Universitatea Craiova',

    'Universitatea Cluj-Napoca': 'Universitatea Cluj',
    'CFM Universitatea Cluj': 'Universitatea Cluj',
    'U Cluj': 'Universitatea Cluj',
    'FC Universitatea Cluj': 'Universitatea Cluj',

    'CFR 1907 Cluj': 'CFR Cluj',
    'CFR Ecomax Cluj': 'CFR Cluj',

    'Oțelul Galați': 'Oţelul Galaţi',
    'Otelul': 'Oţelul Galaţi',
    'Otelul Galati': 'Oţelul Galaţi',
    'FC Oţelul Galaţi': 'Oţelul Galaţi',

    'Pandurii': 'Pandurii Târgu Jiu',
    'Pandurii Lignitul Targu Jiu': 'Pandurii Târgu Jiu',
    'Pandurii Lignitul Târgu Jiu': 'Pandurii Târgu Jiu',
    'CS Pandurii Târgu Jiu': 'Pandurii Târgu Jiu',

    'ASA Tg.Mures': 'ASA Târgu Mureș',

    'CF Chindia Târgoviste': 'Chindia Târgovişte',
    

    'CSM Resita': 'FCM Reșița',

    'Electroputere Craiova': 'FC Caracal', # name map
    'Extensiv Craiova': 'FC Caracal', # name map

    'Jiul Petrosani': 'Jiul Petroşani',
    'Jiul': 'Jiul Petroşani',

    'Foresta Falticeni': 'Foresta Fălticeni',
    

    'Sportul studentesc Bucuresti': 'Sportul Studențesc',
    'Sportul Studenţesc Bucureşti': 'Sportul Studențesc',
    'Sportul Studentesc Bucureşti': 'Sportul Studențesc',
    'Sportul Studentesc Bucuresti': 'Sportul Studențesc',
    'Sportul Stud.': 'Sportul Studențesc',
    'Sportul': 'Sportul Studențesc',
    'Sportul stud.': 'Sportul Studențesc',


    # Greece

    'A.S. Aris Thessalonikis': 'Aris',
    'A.S. Aris Thessaloniki': 'Aris',
    'Aris Thessaloniki': 'Aris',


    'P.A.O.K. Thessalonikis': 'PAOK',
    'PAOK Thessaloniki': 'PAOK',
    'Paok Fc': 'PAOK',


    'Ofi Crete Fc': 'OFI',
    'OFI Heraklion': 'OFI',
    'O.F.I. Iraklion Kritis': 'OFI',

    'Ergotelis Heraklion': 'Ergotelis',

    'Iraklis Thessaloniki': 'Iraklis',
    'A.S. Iraklis Thessaloniki': 'Iraklis',
    'A.S. Iraklis Thessalonikis': 'Iraklis',

    'A.O. Kavala': 'Kavala',
    'AO Kavala': 'Kavala',

    'Ethikos O.F.P. Pireas': 'Ethnikos Piraeus',
    'Ethnikos O.F.P. Pireas': 'Ethnikos Piraeus',

    'Aigaleo': 'Egaleo',
    'Xanthi': 'Skoda Xanthi',

    'Kerkira': 'PAEK',
    'Kerkyra': 'PAEK',
    'Kerkyra AO': 'PAEK',


    'Panathinaikos A.O. Athinas': 'Panathinaikos',
    'Panathinaïkos': 'Panathinaikos',
    'Panathinaikos (Gre)': 'Panathinaikos',

    'Panserraïkos': 'Panserraikos',

    'apollon': 'Apollon',
    'Apollon': 'Apollon Limassol',
    'Apollon Limassol (Cyp)': 'Apollon Limassol',

    'Ael': 'AEL Limassol',
    #'Limassol': 'AEL Limassol',
    #'AEL Limassol': 'Apollon Limassol',
    'AEL Limassol (Cyp)': 'Apollon Limassol',

    'A.O. Krete': 'AO Krete',

    'Panionios G.S. Smyrnis Athinas': 'Panionios',

    'AEK Athinai': 'AEK Athens F.C.',
    'Aek': 'AEK Athens F.C.',
    'AEK': 'AEK Athens F.C.',
    'AEK Athens (Gre)': 'AEK Athens F.C.',
    'AEK Athens F.C.': 'AEK Athens',

    'AE Larissa (Gre)': 'AEL 1964',
    'Larisa': 'AEL 1964',
    # AE Larissa (Gre)

    'Asteras': 'Asteras Tripoli',

    'Giannina': 'PAS Giannina',

    # Austria



    'Wacker Innsbruck (Aut)': 'Wacker Innsbruck',
    'FC Wacker Innsbruck': 'Wacker Innsbruck',
    'FC Wacker Tirol (Innsbruck)': 'Wacker Innsbruck',
    'Wacker': 'Wacker Innsbruck',

    'Wolfsberger AC/SK St. Andrä': 'Wolfsberger AC',




    'Wiener Sport-Club (Aut)': 'Wiener Sport-Club',
    'Admira Wien (Aut)': 'Admira Wien',
    'FK Austria (Aut)': 'FK Austria',

    'SC Rheindorf Altach': 'Rheindorf Altach',


    'Ried': 'SV Ried',
    'SV Ried im Innkreis': 'SV Ried',

    'FC Tirol Innsbruck (Aut)': 'Tirol Innsbruck',
    'Tirol': 'Tirol Innsbruck',
    'FC Tirol Innsbruck': 'Tirol Innsbruck',
    

    #'Vorwärts': 'Vorwärts Steyr',
    'SK Vorwärts Steyr': 'Vorwärts Steyr',

    'FC Kärnten': 'Austria Kärnten',
    'Austria K': 'Austria Kärnten',
    'Kärnten': 'Austria Kärnten',
    'SK Austria Kärnten': 'Austria Kärnten',

    'Austria L': 'Austria Lustenau',


    'Admira Wien': 'Admira Wacker Mödling',
    'VfB Admira/Wacker Mödling': 'Admira Wacker Mödling',
    'FC Admira Wacker Mödling': 'Admira Wacker Mödling',
    'FC Admira/Wacker': 'Admira Wacker Mödling',
    'Admira M': 'Admira Wacker Mödling',
    'Admira': 'Admira Wacker Mödling',
    'SCN FC Admira/Wacker': 'Admira Wacker Mödling',

    'FK Austria': 'Austria Wien',
    'FK Austria Wien': 'Austria Wien',
    'Fk Austria Wien': 'Austria Wien',
    'Austria W': 'Austria Wien',
    'Austria Wien (Aut)': 'Austria Wien',

    'GAK': 'Grazer AK',

    'SK Sturm Graz': 'Sturm Graz',


    #'FC Linz': 'LASK Linz',

    'SK Vöest Linz (Aut)': 'SK Vöest Linz',
    'SK Vöest Linz': 'FC Linz',

    'Linzer ASK': 'LASK Linz',
    'LASK Linz (Aut)': 'LASK Linz',
    'LASK': 'LASK Linz',

    'SV Pasching': 'FC Pasching',
    'Pasching': 'FC Pasching',

    'Mattersburg': 'SV Mattersburg',

    'Austria S': 'Austria Salzburg',
    'SV Austria Salzburg': 'Austria Salzburg',


    'F. C. Salzburg': 'FC Red Bull Salzburg',
    'RB Salzburg': 'FC Red Bull Salzburg',
    'Fc Salzburg': 'FC Red Bull Salzburg',
    'Salzburg': 'FC Red Bull Salzburg',
    'FC Red Bull Salzburg': 'Red Bull Salzburg',

    #Wacker (Austria)

    'Kapfenberger SV': 'Kapfenberg',


    'Rapid De Viena': 'Rapid Vienna',
    'Sk Rapid Wien': 'Rapid Vienna',
    'SK Rapid Wien (Aut)': 'Rapid Vienna',
    'Rapid Wien (Aut)': 'Rapid Vienna',
    'SK Rapid Wien': 'Rapid Vienna',
    'FK Rapid Wien': 'Rapid Vienna',
    'Rapid Wien': 'Rapid Vienna',

    'FC Salzburg - Austria': 'FC Salzburg',
    'FC Salzburg': 'FC Red Bull Salzburg',

    'SC Austria Lustenau': 'Austria Lustenau',

    'SC Wiener Neustadt': 'Wiener Neustadt',
    
    # Switzerland

    'La Chaux-de-Fonds': 'FC La Chaux-de-Fonds',

    'FC Wil 1900': 'FC Wil',
    'Wil': 'FC Wil',

    'FC Thun': 'Thun',

    'FC Vaduz': 'Vaduz',

    'FC Lausanne-Sport': 'Lausanne Sports',
    'Lausanne-Sports': 'Lausanne Sports',
    'Fc Lausanne-Sport': 'Lausanne Sports',
    'Lausanne Sports (Sui)': 'Lausanne Sports',
    'Lausanne Sports': 'Lausanne-Sport',


    'Kriens': 'SC Kriens',

    'Delémont': 'SR Delémont',
    'Delemont': 'SR Delémont',

    'Servette FC Genéve': 'Servette FC',
    'Servette Genève': 'Servette FC',
    'Servette FC Genève': 'Servette FC',
    'Servette FC (Genève)': 'Servette FC',
    'Servette': 'Servette FC',

    'FC Yverdon-Sports': 'Yverdon-Sport FC',
    'Yverdon-Sport': 'Yverdon-Sport FC',
    'Yverdon': 'Yverdon-Sport FC',

    'Lugano': 'FC Lugano',

    'Etoile Carouge FC': 'Étoile Carouge',
    'Etoile C': 'Étoile Carouge',
    'Etoile Carouge': 'Étoile Carouge',

    'FC Basel': 'Basel',
    'FC Basel (Sui)': 'Basel',

    'Zürich': 'FC Zürich',


    'Stade Lausanne': 'FC Stade Lausanne',
    'Stade Laussane - Switzerland': 'Stade Lausanne',

    'FC Aarau': 'Aarau',

    'FC Schaffhausen': 'Schaffhausen',

    'Sion': 'FC Sion',

    'Neuchâtel': 'Neuchâtel Xamax',
    'Xamax': 'Neuchâtel Xamax',
    'Neuchatel Xamax': 'Neuchâtel Xamax',
    'Xamax Neuchâtel (Sui)': 'Neuchâtel Xamax',

    'Bellinzona': 'AC Bellinzona',

    'FC St. Gallen': 'St. Gallen',
    'St Gallen': 'St. Gallen',
    'Sankt Gallen': 'St. Gallen',
    'FC Sankt Gallen': 'St. Gallen',

    'Fc Zürich': 'FC Zurich',
    'Zurich': 'FC Zurich',
    'FC Zurich': 'FC Zürich',
    'FC Zürich (Sui)': 'FC Zürich',

    'Grasshopper-Club': 'Grasshoppers Zurich',
    'Grasshoppers': 'Grasshoppers Zurich',
    'Grasshoppers Zurich': 'Grasshopper Club Zürich',

    'Bsc Young Boys': 'BSC Young Boys',
    'BSC Young Boys Bern': 'BSC Young Boys',

    'Young Boys\' Club': 'BSC Young Boys',
    'Young Boys (Sui)': 'BSC Young Boys',
    'Young Boys Club': 'BSC Young Boys',
    'Young Boys': 'BSC Young Boys',
    'BSC Young Boys (Sui)': 'BSC Young Boys',

    # Italy

    'Sassuolo': 'US Sassuolo',

    'Treviso FC': 'Treviso',

    'Delfino Pescara': 'Pescara',

    'US Lecce': 'Lecce',
    'AC Cesena': 'Cesena',

    'Berrichonne Châteauroux': 'Berrichonne',

    'Cagliari Calcio': 'Cagliari',
    'Cagliari (Ita)': 'Cagliari',

    'Calcio Catania': 'Catania',

    'Ancona Calcio': 'Ancona',

    'Ascoli Calcio': 'Ascoli',

    'AC Chievo Verona': 'Chievo',

    'Hellas Verona': 'Verona',
    'Hellas Verona (Ita)': 'Verona',

    'Livorno Calcio': 'Livorno',

    'Messina PFC': 'Messina',

    'Modena FC': 'Modena',

    'Piacenza FC': 'Piacenza',

    'Salernitana Sport': 'Salernitana',

    'SS Lazio': 'Lazio',

    'Milán': 'AC Milan', # mediotiempo
    'Milan AC': 'AC Milan',
    'AC Milan (Ita)': 'AC Milan',

    'Juventus (Ita)': 'Juventus',
    'FC Juventus': 'Juventus',
    'Juventus FC': 'Juventus',

    'Arezzo - Italy': 'Arezzo',

    'A.S. Bari': 'Bari',
    'AS Bari': 'Bari',

    'AC Siena': 'Siena',

    'Como Calcio': 'Como',

    'Atalanta BC': 'Atalanta',
    'BC Atalanta': 'Atalanta',

    'Empoli FC': 'Empoli',
    'FC Empoli': 'Empoli',


    'Bologna F.C. 1909': 'Bologna',
    'Bologna (Ita)': 'Bologna',
    'Bologna FC': 'Bologna',

    'Brescia': 'Brescia Calcio',

    'AC Fiorentina': 'Fiorentina',
    'Fiorentina (Ita)': 'Fiorentina',
    'Fiorentina - Italy': 'Fiorentina',
    'Fiorentina': 'AFC Fiorentina',

    'Genoa - Italy': 'Genoa',
    'Genoa CFC': 'Genoa',
    'Genoa 1893': 'Genoa',

    'Inter De Milán': 'Inter Milan',
    'FC Internazionale': 'Inter Milan',
    'Internazionale': 'Inter Milan',
    'F.C. Internazionale Milano': 'Inter Milan',
    'Inter': 'Inter Milan',
    'Internazionale (Ita)': 'Inter Milan', 

    
    'Mantova F.C.': 'Mantova',

    'AC Milan - Italy': 'AC Milan',
    'A.C. Milan': 'AC Milan',
    'A. C. Milan': 'AC Milan',
    'Milan': 'AC Milan',

    'Modena (Italy)': 'Modena F.C.',
    'Modena': 'Modena F.C.',
    'Modena - Italy': 'Modena',

    'Napoli (Ita)': 'Napoli', 
    'SSC Napoli': 'Napoli',
    'S.S.C. Napoli': 'Napoli',

    'Palermo (Italy)': 'Palermo',
    'US Citta di Palermo': 'Palermo',
    'US Città di Palermo': 'Palermo',

    'Novara Calcio': 'Novara',

    'Reggina': 'Reggina Calcio',

    'Reggiana - Italy': 'Reggiana',
    'Reggiana': 'AC Reggiana 1919',

    'A.S. Roma': 'Roma',
    'AS Roma (Ita)': 'Roma',
    'AS Roma': 'Roma',

    'Udinese Calcio': 'Udinese',

    'Sampdoria (Italy)': 'Sampdoria',
    'Sampdoria (Ita)': 'Sampdoria',
    'UC Sampdoria': 'Sampdoria',
    'US Sampdoria': 'Sampdoria',
    'Sampdoria - Italy': 'Sampdoria',
    'U.C. Sampdoria': 'Sampdoria',

    'Torine (Italy)': 'Torino FC',
    'Torino Calcio': 'Torino FC',
    'Torino': 'Torino FC',
    'Torino (Ita)': 'Torino FC',


    'Venezia - Italy': 'Venezia',
    


    'A. C. Lanerossi-Vicenza': 'Vicenza Calcio', # name map
    'lanerossi': 'Lanerossi',
    'Lanerossi': 'Vicenza Calcio',
    'Lanerossi Vicenza': 'Vicenza Calcio',
    'AC Lanerossi-Vicenza': 'Vicenza Calcio',
    'Vicenza': 'Vicenza Calcio',

    # Hungary

    'Dunaferr FC': 'Dunaújváros FC',


    'Ferencvaros (Hungary)': 'Ferencvárosi TC',
    'Ferencvaros': 'Ferencvárosi TC',
    'Ferencváros (Hun)': 'Ferencvárosi TC',
    'Ferencvárosi TC': 'Ferencváros',

    'Ujpest Dosza': 'Újpest FC',
    'Újpesti Dózsa (Hun)': 'Újpest FC',
    'Újpest Dózsa (Hun)': 'Újpest FC',
    'Ujpest': 'Újpest FC',

    # Czech

    'FC Viktoria Plzeň': 'Viktoria Plzeň',
    'Viktoria Plzen': 'Viktoria Plzeň',

    'Sparta De Praga': 'Sparta Prague',
    'Sparta Praha (TCH)': 'Sparta Prague',
    'AC Sparta Praha': 'Sparta Prague',
    'AC Sparta Prague': 'Sparta Prague',
    'Sparta Praha': 'Sparta Prague',


    'Slavia Prague': 'SK Slavia Prague',
    'SK Slavia Praha': 'SK Slavia Prague',

    'SSK Vitkovice - Czech': 'SSK Vitkovice',
    'SSK Vitkovice': 'FC Vítkovice',

    # Slovakia
    'SK Bratislava': 'ŠK Slovan Bratislava',
    'Slovan Bratislava': 'ŠK Slovan Bratislava',
    'Sk Slovan Bratislava': 'ŠK Slovan Bratislava',


    # Poland



    'Amika Wronski (Poland)': 'Amica Wronki',
    'Amica': 'Amica Wronki',

    'Arka': 'Arka Gdynia',

    'Cracovia': 'KS Cracovia',

    'Dyskobolia': 'Dyskobolia Grodzisk Wielkopolski',
    'Groclin': 'Dyskobolia Grodzisk Wielkopolski', # check?
    
    'GKS Bełchatów': 'GKS Belchatow',
    'Bełchatów': 'GKS Bełchatów',

    'Dospel Katowice': 'GKS Katowice',
    'Katowice': 'GKS Katowice',

    'Górnik Zabrze (Pol)': 'Gornik Zabrze',
    'Gornik Zabrze (Pol)': 'Gornik Zabrze',
    'Gornik': 'Gornik Zabrze',
    'Gornik Zabrze': 'Górnik Zabrze',
    'Górnik Z': 'Górnik Zabrze',
    'Górnik': 'Górnik Zabrze',

    'Garbarnia Szczakowianka': 'Szczakowianka Jaworzno',

    'Jagiellonia Bialystok': 'Jagiellonia Białystok',
    'Jagiellonia': 'Jagiellonia Białystok',


    'Korona': 'Korona Kielce',

    'Lechia': 'Lechia Gdańsk',

    'Kks Lech Poznán': 'Lech Poznań',
    'Lech': 'Lech Poznań',
    'Lech Poznan': 'Lech Poznań',
    'Lech Poznan (Pol)': 'Lech Poznań',

    'Legia Warsaw (Pol)': 'Legia Warszawa',
    'Legia Warszawa (Pol)': 'Legia Warszawa',
    'CWKS Warsaw': 'Legia Warszawa',
    'Legia Warsaw': 'Legia Warszawa',
    'Legia Warzaw': 'Legia Warszawa',
    'Legia': 'Legia Warszawa',
    'Legia-Daewoo Warsaw (Poland)': 'Legia Warsaw',
    'Legia-Daewoo Warsaw': 'Legia Warsaw',
    'Legia-Daewoo Warsaw - Poland': 'Legia Warsaw',
    'Legia Daewoo Warszawa': 'Legia Warsaw',
    'Legia-Daewoo Warszawa': 'Legia Warsaw',
    'Legia-Daewoo': 'Legia Warsaw',
    'Legia Daewoo': 'Legia Warsaw',

    'ŁKS Łodź (Pol)': 'ŁKS Łódź',
    'LKS Lodz': 'ŁKS Łódź',
    'ŁKS': 'ŁKS Łódź',
    'Łódzki KS': 'ŁKS Łódź',
    'LKS-Ptak Lodz': 'ŁKS Łódź',
    'LKS-Ptak': 'ŁKS Łódź',
    'ŁKS-Ptak': 'ŁKS Łódź',
    'ŁKS-Ptak Łódź': 'ŁKS Łódź',

    'Rocar Bucuresti': 'Rocar București',

    'FC Viitorul Constanţa': 'Viitorul Constanța',

    'CSU Voinţa Sibiu': 'Voinţa Sibiu',

    'FCM Progresul Braila': 'CF Brăila',

    'Odra Wodzisław Śląski': 'Odra Wodzisław',
    'Odra': 'Odra Wodzisław',
    'Odra W': 'Odra Wodzisław',
    'Odra Wodzislaw': 'Odra Wodzisław',
    #'Odra': 'Odra Opole', 'Odra Wodzislaw',
    
    'Orlen': 'Wisła Płock',
    'Orlen Plock': 'Wisła Płock',

    'Rakow Czestochowa': 'Raków Częstochowa',
    'Rakow': 'Raków Częstochowa',

    'RKS-FAMEG Radomsko': 'RKS Radomsko',
    'Radomsko': 'RKS Radomsko',

    'Wisla Plock': 'Wisła Płock',
    'Wisła Plock': 'Wisła Płock',

    'Ostrowiec': 'Ostrowiec Świętokrzyski',
    'KSZO Ostrowiec': 'Ostrowiec Świętokrzyski',
    'KSZO': 'Ostrowiec Świętokrzyski',

    'Piast': 'Piast Gliwice',

    'Polonia W': 'Polonia Warsaw',

    'Polonia B': 'Polonia Bytom',
    'Polonia Bytom (Pol)': 'Polonia Bytom',

    'Szombierki Bytom (Pol)': 'Szombierki Bytom',


    'Pogoń': 'Pogoń Szczecin',
    'Pogon': 'Pogoń Szczecin',
    'Pogon Szczecin': 'Pogoń Szczecin',

    'Sokol Tychy': 'GKS Tychy',

    'Lukullus/Swit Nowy Dwór': 'Świt Nowy Dwór Mazowiecki',

    #'Ruch': 'Ruch Chorzów',

    'Ruch Chorzow': 'Ruch Chorzów',
    'Ruch Chorzow (Pol)': 'Ruch Chorzów',
    'K. S. Ruch': 'Ruch Chorzów',
    'K.S Ruch (Poland)': 'Ruch Chorzów',
    'K.S. Ruch': 'Ruch Chorzów',
    'KS Ruch': 'K.S. Ruch',

    'Ruch C': 'Ruch Chorzów',
    
    'Ruch Radzionkow': 'Ruch Radzionków',
    'Ruch R': 'Ruch Radzionków',

    'Slask Wroclaw': 'Śląsk Wrocław',
    'Slask Wroclaw (Pol)': 'Śląsk Wrocław',
    'Śląsk': 'Śląsk Wrocław',
    'Slask': 'Śląsk Wrocław',

    'Stomil': 'Stomil Olsztyn',

    'Widzew-Piast Łódź': 'Widzew Łódź',
    'Widzew Łódz': 'Widzew Łódź',
    'Widzew Lódz': 'Widzew Łódź',
    'Widzew': 'Widzew Łódź',
    'Widzew Lódz (Pol)': 'Widzew Łódź',
    'Widzew Lodz': 'Widzew Łódź',

    'Wisla Kraków': 'Wisla Krakow',
    'Wisla Krakow - Poland': 'Wisla Krakow',
    'Wisla Krakov': 'Wisła Kraków',
    'Wisla Krakow': 'Wisła Kraków',
    'Wisła K': 'Wisła Kraków',
    'Wisla': 'Wisla Krakow',
    'Wisła': 'Wisla Krakow',

    'Zaglebie Sosnowiec': 'Zagłębie Sosnowiec',

    'Zaglebie Lubin': 'Zagłębie Lubin',
    'Zaglebie Lubin (Pol)': 'Zagłębie Lubin',
    'Zaglebie L': 'Zagłębie Lubin',
    'Zagłębie L': 'Zagłębie Lubin',
    'Zaglebie': 'Zagłębie Lubin',


    'Zagłębie S': 'Zagłębie Sosnowiec',



    # Turkey


    'Kardemir DÇ Karabükspor': 'Kardemir Karabükspor',
    'Kardemir DÇ Karabükspor K': 'Kardemir Karabükspor',
    'D.C. Karabuk': 'Kardemir Karabükspor',
    'D.Ç. Karabükspor': 'Kardemir Karabükspor',
    'Karabükspor': 'Kardemir Karabükspor',

    'Eskisehirspor': 'Eskişehirspor',

    'Gaziantep': 'Gaziantepspor',
    'Gaziantepspor K': 'Gaziantepspor',

    'Karsiyaka': 'Karşıyaka',

    'Kasımpaşa SK': 'Kasımpaşa',

    'MKE Ankaragücü SK': 'Ankaragücü',

    'Manisaspor K': 'Manisaspor',

    'SB Elazığspor': 'Elazığspor',

    'Sariyer Istanbul': 'Sarıyer G.K.',



    'Ankaragucu': 'Ankaragücü',

    'C. Dardanel': 'Dardanel S.A.Ş.',
    'Çanakkale Dardanelspor': 'Dardanel S.A.Ş.',
    'Canakkale Dardanelspor': 'Dardanel S.A.Ş.',
    'Dardanel AS Çanakkale': 'Dardanel S.A.Ş.',

    'Zeytinburnuspor Istanbul': 'Zeytinburnuspor',


    'Yimpas Yozgatspor': 'Yimpaş Yozgatspor',

    'İstanbul BB SK': 'İstanbul BB',

    'Bucaspor K İzmir': 'Bucaspor',


    'Akçaabat Sebatspor K': 'Akçaabat Sebatspor',

    'Akhisar Belediye': 'Akhisar Belediyespor',

    'Sakaryaspor AS Adapazari': 'Sakaryaspor',
    'Sakaryaspor K Adapazarı': 'Sakaryaspor',
    'Sakaryaspor K Adapazari': 'Sakaryaspor',

    'BB Ankaraspor K': 'Ankaraspor',

    'Altay GK Izmir': 'Altay',
    'Altay GSK Izmir': 'Altay',
    'Altay Izmir': 'Altay',


    'Trabszonspor': 'Trabzonspor',
    'Trabzon': 'Trabzonspor',
    'Trabzonspor K': 'Trabzonspor',
    'Trabzonspor (Tur)': 'Trabzonspor',
    'Trabzonspor As': 'Trabzonspor',

    'Diyarbakir': 'Diyarbakırspor',
    'Diyarbakirspor': 'Diyarbakırspor',
    'Diyarbakirspor K': 'Diyarbakırspor',
    'Diyarbakırspor K': 'Diyarbakırspor',

    'Genclerbirligi': 'Gençlerbirliği',
    'Gençlerbirligi': 'Gençlerbirliği',
    'Genclerbirligi Ankara': 'Gençlerbirliği',
    'Gençlerbirligi SK Ankara': 'Gençlerbirliği',
    'Gençlerbirliği OFTAŞ Ankara': 'Gençlerbirliği',
    'Gençlerbirliği SK Ankara': 'Gençlerbirliği',

    'Göztepe AS': 'Göztepe',
    'Göztepe SHTAS Izmir': 'Göztepe',
    'Göztepe SK Izmir': 'Göztepe',
    'Goztepe': 'Göztepe',

    'Elazigspor K': 'Elazığspor',
    'Elazigspor': 'Elazığspor',

    'Denizlispor K': 'Denizli BSK',
    'Denizli SK': 'Denizli BSK',
    'Denizli': 'Denizli BSK',

    'Kayserispor K': 'Kayserispor', # Kayserispor and Erciyespor switched names in 2004...
    'Kayseri': 'Kayserispor', 

    'Kayseri Erciyesspor K': 'Kayseri Erciyesspor',
    'Erciyesspor': 'Kayseri Erciyesspor',
    'Erciyespor': 'Kayseri Erciyesspor',

    'Hacettepespor': 'Hacettepe',
    'Hacettepespor K Ankara': 'Hacettepe',

    # The spors...

    'Adana': 'Adanaspor',
    'Adanaspor AS': 'Adanaspor',
    'Adanaspor SFAS': 'Adanaspor',

    'Seker': 'Sekerspor',
    'Sekerspor': 'Şekerspor',



    'Erzurumspor K': 'Erzurumspor',
    'Erzurum': 'Erzurumspor',

    'Kocaeli': 'Kocaelispor',
    'Kocaelispor (İzmit)': 'Kocaelispor',
    'Kocaelispor K Izmit': 'Kocaelispor',

    'Bursa': 'Bursaspor',
    'Bursaspor K': 'Bursaspor',

    'Samsun': 'Samsunspor',
    'Samsunspor K': 'Samsunspor',

    'MP Antalyaspor': 'Antalyaspor',
    'Antalya': 'Antalyaspor',
    'Antalyaspor K': 'Antalyaspor',
    'Antalyaspor SFTSAŞ': 'Antalyaspor',

    'Van': 'Vanspor',
    'Vanspor K': 'Vanspor',

    'Rize': 'Rizespor',
    'Çaykur Rizespor K': 'Rizespor',
    'Çaykur Rizespor': 'Rizespor',

    'Diyarbakır': 'Diyarbakırspor',

    'Istanbulspor AS': 'Istanbulspor',
    'Istanbulspor SFTAS': 'Istanbulspor',
    'Istanbul': 'Istanbulspor',

    'Beşiktaş JK İstanbul': 'Beşiktaş',
    'Besiktas JK Istanbul': 'Beşiktaş',
    'Besiktas Istanbul': 'Beşiktaş',
    'Besiktas Jk': 'Beşiktaş',
    'Beşiktaş (Tur)': 'Beşiktaş',
    'Besiktas (Tur)': 'Beşiktaş',
    'Besiktas': 'Beşiktaş',
    'Beşiktaş J.K.': 'Beşiktaş',

    'Malatyaspor K': 'Malatyaspor',
    'Malatya': 'Malatyaspor',

    'Konyaspor K': 'Konyaspor',

    'Siirt Jet-PA SK': 'Siirt S.K.',
    'Siirt Jet-PA': 'Siirt S.K.',

    'Sivasspor K': 'Sivasspor',

    'Galatasaray - Turkey': 'Galatasaray',
    'Galatasaray (Tur)': 'Galatasaray',
    'Galatasary': 'Galatasaray',
    'Galatasaray Istanbul': 'Galatasaray',
    'Galatasaray SK Istanbul': 'Galatasaray',
    'Galatasaray SK İstanbul': 'Galatasaray',

    'Fenerbahçe SK İstanbul': 'Fenerbahçe',
    'Fenerbahçe SK Istanbul': 'Fenerbahçe',
    'Fenerbahce Istanbul': 'Fenerbahçe',
    'Fenerbahçe Sk': 'Fenerbahçe',
    'Fenerbahce': 'Fenerbahçe',
    'Fenerbahçe S.K.': 'Fenerbahçe',
    'Fenerbahçe (Tur)': 'Fenerbahçe',

    'Vestel Manisaspor K': 'Manisaspor',
    'Vestel Manisaspor': 'Manisaspor',


    'Gençlerbirliği OFTAŞ': 'Hacettepe',
    'Anakaragücü': 'Ankaragücü',

    # Ukraine

    'Illichivets\' Mariupol\'': 'Illichivets Mariupol',
    'Illichivets\' Mariupol': 'Illichivets Mariupol',

    'Zakarpattja Uzhhorod': 'Hoverla Uzhhorod',

    'Metalurh-Zaporizhzhja Zaporizhzhja': 'Metalurh Zaporizhzhja',
    'Metalurh-Zap. Zaporizhzhja': 'Metalurh Zaporizhya',
    'Metalurh Zaporizhzhja': 'Metalurh Zaporizhya',

    'Stal\' Alchevs\'k': 'Stal Alchevsk',

    'Chornomorets\' Odesa': 'Chornomorets Odesa',

    'FC L\'viv': 'FC Lviv',
    'FC L\'viv': 'FC Lviv',
    'FC L\'viv': 'FC Lviv',                                    

    'Obolon\' Kyiv': 'Obolon Kyiv',

    'PFC Sevastopol\'': 'FC Sevastopol',

    'Tavrija Simferopol\'': 'Tavriya Simferopol',

    'Volyn\' Luts\'k': 'Volyn Lutsk',
    
    'Dynamo Kiev -- Ukraine': 'Dynamo Kiev',
    'Dinamo Kiev': 'FC Dynamo Kyiv',
    'Dynamo Kiev': 'FC Dynamo Kyiv',
    'Dinamo Kiev (URS)': 'FC Dynamo Kyiv',

    # Russia

    'Volga Nizhniy Novgorod': 'Volga Nizhny Novgorod',

    'FK Krasnodar': 'FC Krasnodar',

    'Torpedo Moskva': 'FC Torpedo Moscow',
    'Moscow Torpedo': 'FC Torpedo Moscow',
    'Torpedo Moscow (URS)': 'FC Torpedo Moscow',
    'Torpedo Moscow -- Russia': 'Torpedo Moscow',
    'Torpedo Moscow': 'FC Torpedo Moscow',

    'Torpedo-Metallurg': 'FC Torpedo-ZIL Moscow',

    'Torpedo-ZIL': 'FC Torpedo-ZIL Moscow',
    'Torpedo-Metall': 'FC Torpedo-ZIL Moscow',

    'Dinamo Moskva': 'FC Dynamo Moscow',
    'Moscow Dynamo': 'FC Dynamo Moscow',

    'Rubin Kazan -- Russia': 'Rubin Kazan',
    'Fc Rubin Kazan': 'Rubin Kazan',

    
    'FC Zenit Saint Petersburg': 'Zenit Saint Petersburg',
    'Zenit Sankt-Peterburg': 'Zenit Saint Petersburg',

    #Cevle (Bryn) # Very likely Gavle in Sweden.

    #Denmark

'Næstved': 'Næstved BK',

    'B1903 Křbenhavn (Den)': 'FC København',
    'AB København (Den)': 'FC København',
    'KB Křbenhavn (Den)': 'FC København',
    'KB København (Den)': 'FC København',

    'Brřndby IF (Den)': 'Brøndby',
    'Brondby IF (Den)': 'Brøndby',
    'Brondby If': 'Brøndby',

    'Herfølge': 'Herfølge BK',

    'Køge': 'HB Køge',
    'Køge BK': 'HB Køge',
    'Křge BK (Den)': 'HB Køge',

    'Nordsjaelland': 'Nordsjælland',
    'Farum BK': 'Nordsjælland',
    'Farum': 'Nordsjælland',
    'Nordjælland': 'Nordsjælland',
    'FC Nordsjælland': 'Nordsjælland',

    'Haderslev Fodboldklub': 'SønderjyskE',

    'Lyngby BK (Den)': 'Lyngby BK',
    'Lyngby': 'Lyngby BK',
    'Lyngby FC': 'Lyngby BK',
    'Lyngby - Denmark': 'Lyngby',


    'Ikast (Denmark)': 'Ikast FS',
    'Ikast': 'Ikast FS',
    'Ikast fS': 'Ikast FS',
    'Ikast - Denmark': 'Ikast',

    'Viborg - Denmark': 'Viborg',
    'Viborg': 'Viborg FF',

    'Norresundby Boldklub - Denmark': 'Norresundby Boldklub',
    'Norresundby Boldklub': 'Nørresundby Boldklub',

    'Odense Bk': 'OB',
    'OB Odense (Den)': 'OB',
    'B1913 Odense (Den)': 'OB',
    'B1909 Odense (Den)': 'OB',
    'B1909 (Den)': 'OB',

    'Silkeborg': 'Silkeborg IF',

    'Midtjylland': 'FC Midtjylland',

    'Haderslev FK': 'SønderjyskE',

    'Vejle BK (Den)': 'Vejle',
    'Vejle BK': 'Vejle',
    'Vejle B': 'Vejle',

    'B 93': 'B.93',

    'Esbjerg fB (Den)': 'Esbjerg fB',
    'Esbjerg': 'Esbjerg fB',

    'Horsens': 'AC Horsens',
    'HFK Sønderjylland': 'SønderjyskE',
    'København': 'FC København',

    'AGF (Den)': 'AGF Aarhus',
    'AGF': 'AGF Aarhus',
    'AGF Aarhus (Den)': 'AGF Aarhus',

    'Brøndby': 'Brøndby IF',



    # Sweden



    'Assyriska F': 'Assyriska',

    'Öster': 'Östers IF',
    'Osters IF (Swe)': 'Östers IF',
    'Östers IF (Swe)': 'Östers IF',
    'Östers': 'Östers IF',
    'Oster': 'Östers IF',
    'Östers IF Växjö': 'Östers IF',



    'Ljungskile': 'Ljungskile SK',


    'Landskrona': 'Landskrona BoIS',
    'Kalmar': 'Kalmar FF',

    'Jonkopping (Sweden)': 'Jonkopping',
    'Trelleborgs FF': 'Trelleborg',
    'Örebro': 'Örebro SK',
    'Örebrö': 'Örebro SK',
    'Gefle': 'Gefle IF',

    'Häcken': 'BK Häcken',
    'Halmstad': 'Halmstads BK',
    'Halmstads BK (Swe)': 'Halmstads BK',
    'Halmstads': 'Halmstads BK',
    'Hamlstads BK': 'Halmstads BK',

    'Enköping': 'Enköpings SK',
    'Enköpings': 'Enköpings SK',

    'IFK Malmö (Swe)': 'Malmö FF',
    'Malmö Ff': 'Malmö FF',
    'Malmo FF (Swe)': 'Malmö FF',
    'Malmo FF': 'Malmö FF',
    'Malmö FF (Swe)': 'Malmö FF',
    'Malmö': 'Malmö FF',
    'Malmo': 'Malmö FF',
    'Malmö': 'Malmö FF',

    'Ldb Malmo': 'FC Rosengård',
    'LdB FC Malmö': 'FC Rosengård',
    'Malmö FF DFF': 'FC Rosengård',

    'Gif Sundsvall': 'GIF Sundsvall',

    'Goteborg Orgryte': 'Örgryte IS',
    'Örgryte Göteborg (Swe)': 'Örgryte IS',

    'IFK Norrköping (Swe)': 'IFK Norrköping',
    'IFK Norrköping FK': 'IFK Norrköping',
    'Norrkoeping': 'IFK Norrköping',
    'Norrkoping': 'IFK Norrköping',
    'IFK Norrkoping': 'IFK Norrköping',
    'Norrköping': 'IFK Norrköping',

    'Hammarby': 'Hammarby IF',
    'Hammarby IF FF': 'Hammarby IF',

    'Losc Lille': 'Lille OSC',
    'Lille': 'Lille OSC',


    'Helsingborg IF': 'Helsingborgs IF',
    'Helsingborg': 'Helsingborgs IF',
    'Helsingborgs': 'Helsingborgs IF',



    'Orebro': 'Örebro SK',
    'Örebro SK FK': 'Örebro SK',

    'If Elfsborg': 'IF Elfsborg',
    'Elfsborg': 'IF Elfsborg',

    'Gif Sundsvall -- Sweden': 'Gif Sundsvall',

    'IFK Gothenburg - Sweden': 'IFK Göteborg',
    'IFK Gothenburg': 'IFK Göteborg',
    'Göteborg': 'IFK Göteborg',
    'IFK Goteborg': 'IFK Göteborg',
    'IFK Goteborg (Swe)': 'IFK Göteborg',
    'JFK Gothenburg (Sweden)': 'IFK Göteborg',
    'IFK Göteborg (Swe)': 'IFK Göteborg',

    'Djurgårdens IF F': 'Djurgårdens IF',
    'Djurgårdens IF F': 'Djurgårdens IF',
    'Djurgardens IF': 'Djurgårdens IF',
    'Djurgårdens IF (Swe)': 'Djurgårdens IF',
    'Djurgardens': 'Djurgårdens IF',
    'Djurgarden': 'Djurgårdens IF',
    'Djurgården': 'Djurgårdens IF',

    'Örgryte': 'Örgryte IS',

    'BK Frem': 'Frem',

    'Bryne IL': 'Bryne',

    'FK Haugesund': 'Haugesund',

    'GAIS Göteborg': 'GAIS',

    'Halmstad BK': 'Halmstads BK',

    'Hønefoss BK': 'Hønefoss',

    'AIK Solna': 'AIK Fotboll',
    'Aik': 'AIK Fotboll',
    'A.I.K.': 'AIK Fotboll',
    'AIK': 'AIK Fotboll',

    'G.A.I.S.': 'GAIS',    
    'Jonkopping': 'Jönköpings Södra IF',
    'Sundsvall': 'GIF Sundsvall',


    # Norway


    'Strømsgodset IF': 'Strømsgodset',
    'Stromsgodset': 'Strømsgodset',
    'Strømsgods': 'Strømsgodset',
    'Strřmsgodset IF (Nor)': 'Strømsgodset',


    'Moss': 'Moss FK',
    'Moss FK (Nor)': 'Moss FK',

    'Bodo Glimt': 'Bodø/Glimt',
    'Bodo': 'Bodø/Glimt',
    'Bodo/Glimt': 'Bodø/Glimt',
    'Bodo/Glimt -- Norway': 'Bodo/Glimt',
    'Bodø /Glimt': 'Bodo/Glimt',
    'Bodø/Glimt FK': 'Bodo/Glimt',
    'FK Bodø/Glimt': 'Bodo/Glimt',

    'Ham-Kam': 'Hamarkameratene',
    'Hamarkam.': 'Hamarkameratene',
    'Ham Kam': 'Hamarkameratene',
    

    'Lillestrom': 'Lillestrøm',
    'Lillestrřm SK (Nor)': 'Lillestrøm',

    'Lyn Oslo -- Norway': 'Lyn Oslo',
    'Lyn': 'Lyn Oslo',
    'Lyn Oslo (Nor)': 'Lyn Oslo',

    'Odd Grenland -- Norway': 'Odds BK',
    'Odd Grenland': 'Odds BK',
    'Odd Grenlöand': 'Odds BK',
    'Odd Grenl': 'Odds BK',
    'Odd': 'Odds BK',

    'Rosenborg Bk': 'Rosenborg BK',
    'Rosenborg Trondheim -- Norway': 'Rosenborg Trondheim',
    'Rosenborg Trondheim': 'Rosenborg BK',
    'Rosenborg BK (Nor)': 'Rosenborg BK',
    'Rosenborg': 'Rosenborg BK',

    'SK Brann': 'Brann',
    'Brann Bergen': 'Brann',


    'Stabaek -- Norway': 'Stabaek',



    'Tromso': 'Tromsø IL',
    'Tromsø': 'Tromsø IL',

    'Viking Stavangar -- Norway': 'Viking FK',
    'Viking Stavanger': 'Viking FK',

    'Vĺlerengen IF (Nor)': 'Vålerenga',
    'Vålerenga IF': 'Vålerenga',
    'Valerenga': 'Vålerenga',
    'Vålerengen IF (Nor)': 'Vålerenga',

    'Sogndal IL': 'Sogndal',
    'Sogndal Fotball': 'Sogndal',



    # Iceland
    'Fylkir (Iceland)': 'Fylkir',
    'Fylkir - Iceland': 'Fylkir',
    'IBV - Iceland': 'IBV',

    'Valur': 'Valur FC',
    'Valur Reykjavik (Isl)': 'Valur FC',

    # Finland
    'FinnPa - Finland': 'FinnPa',

    # Germany

    'Augsburg': 'FC Augsburg',

    'Darmstadt': 'SV Darmstadt 98',

    'Wismut Karl-Marx-Stadt (GDR)': 'Wismut Karl-Marx-Stadt',
    'FC Karl-Marx-Stadt (GDR)': 'FC Karl-Marx-Stadt',
    'FC Karl-Marx-Stadt': 'Erzgebirge Aue',
    'Wismut Karl-Marx-Stadt': 'Erzgebirge Aue',

    'Greuther Fürth': 'SpVgg Greuther Fürth',
    'Fürth': 'SpVgg Greuther Fürth',

    'TSV Alemannia Aachen': 'Alemannia Aachen',
    'Alemannia': 'Alemannia Aachen',

    'Sc Freiburg': 'SC Freiburg',
    'FC Freiburg': 'SC Freiburg',

    'Tennis Borussia (Germany)': 'Tennis Borussia Berlin',



    'Borussia Mönchengladbach (Ger)': 'Borussia Mönchengladbach',
    'B. Mönchengladbach (Ger)': 'Borussia Mönchengladbach',
    'VfL Borussia Mönchengladbach': 'Borussia Mönchengladbach',
    'Borussia Moenchengladbach': 'Borussia Mönchengladbach',
    'Borussia Monchengladbach': 'Borussia Mönchengladbach',
    'M\'gladbach': 'Borussia Mönchengladbach',
    'M’gladbach': 'Borussia Mönchengladbach',

    'Fc St. Pauli': 'FC St. Pauli',
    'St Pauli': 'FC St. Pauli',
    'St. Pauli': 'FC St. Pauli',
    'FC Sankt Pauli': 'FC St. Pauli',

    'Fc Kaiserslautern': '1. FC Kaiserslautern',
    '1. FC Kaiserslautern (Ger)': '1. FC Kaiserslautern',
    '1.FC Kaiserslautern (Ger)': '1. FC Kaiserslautern',
    '1.FC Kaiserslautern': '1. FC Kaiserslautern',
    '1 FC Kaiserslautern': '1. FC Kaiserslautern',
    'Kaiserslautern': '1. FC Kaiserslautern',
    'K\'lautern': '1. FC Kaiserslautern',
    'K’lautern': '1. FC Kaiserslautern',

    'Fc Köln': '1. FC Köln',
    '1.FC Köln': '1. FC Köln',
    '1 FC Koln': '1. FC Köln',
    'FC Koln': '1. FC Köln',
    'FC Köln (Ger)': '1. FC Köln',
    

    #Tennis Borussia (Germany)
    #Hansa (Germany)
    #03 Lunenburg (Germany)

    'Arminia B': 'Arminia Bielefeld',
    'Arminia': 'Arminia Bielefeld',
    'Bielefeld': 'Arminia Bielefeld',
    'DSC Arminia Bielefeld': 'Arminia Bielefeld',


    'Unterhaching': 'SpVgg Unterhaching',

    'Extremadura': 'CF Extremadura',

    'Stuttgart': 'VfB Stuttgart',

    'Rostock': 'Hansa Rostock',
    'Hansa': 'Hansa Rostock',
    'FC Hansa Rostock': 'Hansa Rostock',
    'Hansa Rostock (GDR)': 'Hansa Rostock',

    'Ulm': 'SSV Ulm 1846',


    'Hannover': 'Hannover 96',

    'Hamburger SV (FRG)': 'Hamburger SV',
    'Hamburger SV (Ger)': 'Hamburger SV',
    'HSV': 'Hamburger SV',
    'Hamburg': 'Hamburger SV',
    'Hamburger': 'Hamburger SV',

    'Msv Duisburg': 'MSV Duisburg',
    'MSV': 'MSV Duisburg',
    'Duisburg': 'MSV Duisburg',
    'Köln': '1. FC Köln',
    'Nürnberg': '1. FC Nürnberg',

    'Hoffenheim 1899': '1899 Hoffenheim',
    'Hoffenheim': '1899 Hoffenheim',
    'TSG 1899 Hoffenheim': '1899 Hoffenheim',

    'Eintracht Frankfurt - Germany': 'Eintracht Frankfurt',
    'Eintracht Frankfurt (FRG)': 'Eintracht Frankfurt',
    'Eintracht Frankfurt/Main': 'Eintracht Frankfurt',
    'Frankfurt': 'Eintracht Frankfurt',
    'SG Eintracht Frankfurt': 'Eintracht Frankfurt',
    'Eintracht F': 'Eintracht Frankfurt',

    'Fsv Frankfurt': 'FSV Frankfurt',

    'Freiburg': 'SC Freiburg',
    'Freiburger': 'SC Freiburg',

    'SV Werder Bremen': 'Werder Bremen',
    'Werder': 'Werder Bremen',
    'Bremen': 'Werder Bremen',
    'Werder Bremen (Ger)': 'Werder Bremen',

    'Dortmund': 'Borussia Dortmund',
    'BV 09 Borussia Dortmund': 'Borussia Dortmund',
    'Borussia D': 'Borussia Dortmund',
    'Borussia Dortmund (Ger)': 'Borussia Dortmund',

    'Mönchengladbach': 'Borussia Mönchengladbach',
    'Borussia M': 'Borussia Mönchengladbach',
    'Borussia MG': 'Borussia Mönchengladbach',
    'Borussia M\'gladbach': 'Borussia Mönchengladbach',

    'Wolfsburg': 'VfL Wolfsburg',

    'Düsseldorf': 'Fortuna Düsseldorf',
    'Fortuna Dusseldorf': 'Fortuna Düsseldorf',

    'Braunschweig': 'Eintracht Braunschweig',
    'Eintracht Braunschwieg': 'Eintracht Braunschweig',
    'Eintracht Braunschweig (Ger)': 'Eintracht Braunschweig',


    'TSV 1860': '1860 Munchen',
    'TSV Munich': '1860 Munchen',
    '1860 München': '1860 Munchen',
    'TSV München 1860': '1860 Munchen',
    '1860': '1860 Munchen',
    'TSV Munich 1860': '1860 Munchen',
    'Munchen 1860': '1860 Munchen',
    '1860 Munchen -- Germany': '1860 Munchen',
    '1860 Munchen': 'TSV 1860 München',
    'TSC Muenchen 1860': 'TSV 1860 München',
    'TSV 1860 München (Ger)': 'TSV 1860 München',
    'TSV 1860 Munchen': 'TSV 1860 München',
    '1860 Munich': 'TSV 1860 München',

    'Bayer Leverkusen - Germany': 'Bayer Leverkusen',
    'Leverkusen': 'Bayer Leverkusen',
    'Bayer 04 Leverkusen': 'Bayer Leverkusen',

    'Energie': 'Energie Cottbus',
    'Cottbus': 'Energie Cottbus',
    'FC Energie Cottbus': 'Energie Cottbus',

    'FSV Mainz 05': 'Mainz 05',
    '1. FSV Mainz 05': 'Mainz 05',
    'Mainz': 'Mainz 05',
    '1.FSV Mainz 05': 'Mainz 05',


    'Bayern Munchen (Ger)': 'Bayern Munich',
    'Bayern München (Ger)': 'Bayern Munich',
    'FC Bayern München': 'Bayern Munich',
    'FC Bayern Munchen': 'Bayern Munich',
    'Bayern': 'Bayern Munich',
    'Munchen': 'Bayern Munich',
    'München': 'Bayern Munich',
    'Bayern Munich - Germany': 'Bayern Munich',
    'Bayern Munchen': 'FC Bayern Munich',
    'Bayern Munich': 'FC Bayern Munich',
    'Bayern München': 'FC Bayern Munich',

    'VFL Bochum - Germany': 'VfL Bochum',
    'VFL Bochum': 'VfL Bochum',
    'Bochum': 'VfL Bochum',

    'Borussia Brandaachen': 'Borussia Brand-Aachen',



    'Fortune Sittard': 'Fortuna Sittard',

    'Hamburg SV': 'Hamburger SV',
    'Hamburg SV - Germany': 'Hamburger SV',

    'H.S.V. Hannover': 'Hannover 96',
    'Hannover \'96': 'Hannover 96',

    'hertha': 'Hertha BSC',
    'Hertha': 'Hertha BSC',
    'Hertha BSC Berlin': 'Hertha BSC',

    'BFC Dynamo Berlin (GDR)': 'Dynamo Berlin',


    'Karlsruher': 'Karlsruher SC',
    'Karlsruhe': 'Karlsruher SC',
    'Karlsruhe FV': 'Karlsruher FV',

    'FC Cologne': '1. FC Köln',
    'F. C. Koln': '1. FC Köln',
    '1.FC Koln': '1. FC Köln',


    'Nurnberg': 'FC Nurnberg',
    'FC Nürnberg (Ger)': 'FC Nurnberg',
    'FC Nurnberg - Germany': 'FC Nurnberg',
    '1.FC Nürnberg (FRG)': '1. FC Nuremberg',
    'FC Nurnberg': '1. FC Nuremberg',
    '1 FC Nuremberg': '1. FC Nuremberg',
    'Nuernberg': '1. FC Nuremberg',
    '1.FC Nürnberg': '1. FC Nuremberg',

    'Reutlingen': 'SSV Reutlingen 05',

    '1.FC Saarbrücken': '1. FC Saarbrücken',
    'Saarbrucken': '1. FC Saarbrücken',
    'Saarbruecken': '1. FC Saarbrücken',
    '1. FC Saarbrucken': '1. FC Saarbrücken',
    'FC Saarbrücken': '1. FC Saarbrücken',
    'FC Saarbrucken': '1. FC Saarbrücken',

    'FC Schalke': 'Schalke 04',
    'FC Schalke 04 (FRG)': 'Schalke 04',
    'Schalke´04': 'Schalke 04',
    'FC Schalke 04': 'Schalke 04',
    'Schalke': 'Schalke 04',
    'FC Schalke 04 Gelsenkirchen': 'Schalke 04',



    'V.F.B. Stuttgart': 'VfB Stuttgart',
    'VfB Stuttgart (Ger)': 'VfB Stuttgart',

    'Tusberg Landshut - Germany': 'Tusberg Landshut',

    # Ireland

    'Waterford (Irl)': 'Waterford United',

    'Dundalk (Irl)': 'Dundalk F.C.',
    'Dundalk (Ireland)': 'Dundalk F.C.',
    'Dundalk': 'Dundalk F.C.',

    'Galway (Ireland)': 'Galway United FC',
    'Galway': 'Galway United FC',
    
    # Northern Ireland

    'Linfield (Nir)': 'Linfield F.C.',
    'Linfield (Northern Ireland)': 'Linfield F.C.',
    'Linfield Belfast (Nir)': 'Linfield F.C.',
    'Linfield': 'Linfield F.C.',

    'Limerick (Irl)': 'Limerick F.C.',

    'Glentoran (Nir)': 'Glentoran',

    'Glenavon (Northern Ireland)': 'Glenavon F.C.',
    'Glenavon': 'Glenavon F.C.',
    'Glenavon FC': 'Glenavon F.C.',

    'Ards (Nir)': 'Ards F.C.',
    'Ards (Northern Ireland)': 'Ards F.C.',
    'Ards': 'Ards F.C.',

    #Coleraine (Northern Ireland)

    # Scotland

    'Falkirk Fc': 'Falkirk',

    'Motherwell Fc': 'Motherwell',

    'Dumfermline': 'Dunfermline',
    'Dunfermline': 'Dunfermline Athletic F.C.',

    'Aberdeen': 'Aberdeen F.C.',
    'Aberdeen (Sco)': 'Aberdeen F.C.',
    'Aberdeen Dons': 'Aberdeen F.C.',



    'Celtic Glasgow (Sco)': 'Glasgow Celtic',
    'Celtic Glasgow - Scotland': 'Glasgow Celtic',
    'Celtic (Sco)': 'Glasgow Celtic',
    'Celtic F.C.': 'Glasgow Celtic',
    'Celtic FC': 'Glasgow Celtic',
    #'Celtic': 'Glasgow Celtic',
    'Celtic Glasgow': 'Glasgow Celtic',
    'Glasgow Celtic F.C.': 'Glasgow Celtic',

    'Glasgow Rangers (Sco)': 'Glasgow Rangers',
    'Glasgow Rangers F.C.': 'Glasgow Rangers',
    'Glasgow Rangers - Scotland': 'Glasgow Rangers',
    #'Rangers': 'Glasgow Rangers',
    'Rangers FC': 'Glasgow Rangers',
    'Rangers F.C.': 'Glasgow Rangers',
    'Rangers (Sco)': 'Glasgow Rangers',

    'Hearts': 'Heart of Midlothian',
    'Hearts of Midlothian': 'Heart of Midlothian',
    'Hearts of Midlothian F.C.': 'Heart of Midlothian',
    'Heart of Midlothian F.C.': 'Heart of Midlothian',
    'Heart of Midl': 'Heart of Midlothian',
    'Hearts of Midl': 'Heart of Midlothian',
    'Heart of M': 'Heart of Midlothian',
    'Heart Of Midlothian (Sco)': 'Heart of Midlothian',

    'Kilmarnock (Sco)': 'Kilmarnock F.C.',
    'Kilmarnock': 'Kilmarnock F.C.',
    'Kilmarnock FC': 'Kilmarnock F.C.',

    # England

    'Barnsley Fc': 'Barnsley',

    'Wimbledon': 'Wimbledon FC',
    'Walsall F.C.': 'Walsall',


    'Millwall': 'Millwall F.C.',
    'Millwall FC': 'Millwall F.C.',

    'Arsenal - England': 'Arsenal',
    'Royal Arsenal': 'Arsenal',
    'Arsenal (Eng)': 'Arsenal',

    'Macclesfield Town F.C.': 'Macclesfield Town',

    'Aston Villa F.C.': 'Aston Villa',
    'Aston Villa (Eng)': 'Aston Villa',
    'Aston Villa - England': 'Aston Villa',

    'Birmingham': 'Birmingham City',
    'Birmingham City F.C.': 'Birmingham City',

    'Blackpool': 'Blackpool F.C.',
 
    'Bristol (England)': 'Bristol City',
    'Bristol City F.C.': 'Bristol City', 

    'Burnley (Eng)': 'Burnley',
    'Burnley F.C.': 'Burnley',
    'Burnley FC': 'Burnley',
    
    'Charlton': 'Charlton Athletic',
    'Charlton Athletic F.C.': 'Charlton Athletic',

    'Chelsea FC': 'Chelsea',
    'Chelsea F.C.': 'Chelsea',

    'Coventry City FC': 'Coventry City',
    'Coventry': 'Coventry City',

    'Derby Country': 'Derby County',
    'Derby': 'Derby County',
    'Derby County (Eng)': 'Derby County',
    'Derby County - England': 'Derby County',
    
    'Everton FC': 'Everton',
    'Everton - England': 'Everton',
    'Everton F.C.': 'Everton',
    'Everton (Eng)': 'Everton',

    'Fulham - England': 'Fulham',

    'Huddersfield Town F.C.': 'Huddersfield Town',

    'Ipswich': 'Ipswich Town',
    'Ipswich Town (Eng)': 'Ipswich Town',
    'Ipswich Town F.C.': 'Ipswich Town',
    
    'Leeds': 'Leeds United',
    'Leeds United (Eng)': 'Leeds United',
    'Leeds United A.F.C.': 'Leeds United',
    'Leeds United - England': 'Leeds United',

    'Leicester': 'Leicester City',
    'Leicester City - England': 'Leicester City',

    'Liverpool Fc': 'Liverpool',
    'Liverpool F.C.': 'Liverpool',
    'Liverpool FC': 'Liverpool',
    'Liverpool (Eng)': 'Liverpool',

    'Swindon': 'Swindon Town',

    'Manchester City FC': 'Manchester City',
    'Manchester City FC - England': 'Manchester City',
    'Manchester C': 'Manchester City',
    'Man City': 'Manchester City',
    'Manchester City F.C.': 'Manchester City',
    'Manchester City (Eng)': 'Manchester City',

    'Manchester Utd.': 'Manchester United',
    'Newton Heath': 'Manchester United',
    'Man Utd': 'Manchester United',
    'Manchester U': 'Manchester United',
    'Manchester United - England': 'Manchester United',
    'Manchester United (Eng)': 'Manchester United',
    'Manchester United FC': 'Manchester United',

    'Oldham': 'Oldham Athletic',

    'Middlesbrough FC': 'Middlesbrough',
    'Middleborough FC': 'Middlesbrough F.C.',
    'Middlesbrough F.C.': 'Middlesbrough',

    'Qpr': 'Queens Park Rangers',
    'QPR': 'Queens Park Rangers',
    'Queen\'s Park Rangers': 'Queens Park Rangers',
    'Queens\' Park Rangers': 'Queens Park Rangers',

    'Bradford': 'Bradford City',

    'Blackburn': 'Blackburn Rovers',

    'Newcastle - England': 'Newcastle United',     
    'Newcastle U': 'Newcastle United',     
    'Newcastle': 'Newcastle United',    
    'Newcastle United - England': 'Newcastle United',
    'Newcastle United F.C.': 'Newcastle United',
    
    'Norwich': 'Norwich City',
    'Norwich City F.C.': 'Norwich City',

    'Notts Forest': 'Nottingham Forest', # Presumably there is not really a Notts Forest. (except in San Diego)
    'Nottingham Forest F.C.': 'Nottingham Forest',
    'Nottingham Forest (Eng)': 'Nottingham Forest',
    'Nott\'m Forest': 'Nottingham Forest',

    'Plymouth Argyle': 'Plymouth Argyle F.C.',

    'Port Vale': 'Port Vale F.C.',

    'Portsmouth (England)': 'Portsmouth',
    'Portsmouth FC': 'Portsmouth',
    'Portsmouth F.C.': 'Portsmouth',

    
    'Preston North End F.C.': 'Preston North End',

    'Sheffield W': 'Sheffield Wednesday',
    'Sheffield Wed': 'Sheffield Wednesday',
    'Sheffield W.': 'Sheffield Wednesday',
    'Sheffield Wednesday - England': 'Sheffield Wednesday',


    'Sheffield United - England': 'Sheffield United',
    'Sheffield Utd': 'Sheffield United',

    'Stockport County F.C.': 'Stockport County',

    'Hull': 'Hull City',

    'Bolton': 'Bolton Wanderers',
    'Bolton Wanderers - England': 'Bolton Wanderers',

    'Stoke': 'Stoke City',
    'Stoke (England)': 'Stoke City',
    'Stoke City F.C.': 'Stoke City',

    'Sunderland - England': 'Sunderland',
    'Sunderland A.F.C.': 'Sunderland',

    'Swansea': 'Swansea City',

    'Spurs': 'Tottenham Hotspur',
    'Tottenham Hotspur (Eng)': 'Tottenham Hotspur',
    'Tottenham': 'Tottenham Hotspur',
    'Tottenham Hotspur - England': 'Tottenham Hotspur',
    'Tottenham Hotspurs': 'Tottenham Hotspur',

    'West Bromwich Albion - England': 'West Bromwich Albion',
    'West Brom': 'West Bromwich Albion',
    'WBA': 'West Bromwich Albion',
    'West Brom Albion': 'West Bromwich Albion',
    'West Bromwich': 'West Bromwich Albion',

    'West Ham': 'West Ham United',

    
    'Wigan': 'Wigan Athletic',
    'Wigan Athletic F.C.': 'Wigan Athletic',


    'Wolverhampton': 'Wolverhampton Wanderers',
    'Wolverhampton Wanderers F.C.': 'Wolverhampton Wanderers',
    'Wolverhampton Wanderers (Eng)': 'Wolves',
    'Wolverhampton Wanderers': 'Wolves',

    'Workington AFC': 'Workington A.F.C.',    


    # South America

    # Venezuela

    'Atl. Venezuela': 'Atlético Venezuela',

    'Real Esppor': 'Deportivo La Guaira',
    'Dvo. La Guaira': 'Deportivo La Guaira',

    'Tucanes de Amazonas FC': 'Tucanes de Amazonas',

    'Atl. El Vigia': 'Atlético El Vigía',

    'Atletico San Cristobal': 'Atlético San Cristóbal',



    'Valencia (Venezuela)': 'Carabobo FC',

    'Aragua Fc': 'Aragua FC',

    'Estudiantes (Venezuela)': 'Estudiantes de Mérida',

    'Minerven': 'Minervén',

    'Dvo. Táchira': 'Deportivo Táchira',
    'Deportivo Tachira': 'Deportivo Táchira',
    'Tachira': 'Deportivo Táchira FC',
    'Táchira': 'Deportivo Táchira FC',
    'Deportivo Táchira': 'Deportivo Táchira FC',


    'Caracas F.C.': 'Caracas FC',
    'Caracas Fc': 'Caracas FC',

    # Mergers!
    'Italchacao -- Venezuela': 'Italchacao',
    'Italchacao': 'Deportivo Petare',
    'Deportivo Italchacao': 'Deportivo ItalChacao',
    'Deportivo ItalChacao': 'Deportivo Petare',
    'Deportivo Chacao': 'Deportivo Petare',
    'Dvo. Petare': 'Deportivo Petare',

    'Marítimo (Venezuela)': 'Club Sport Marítimo de Venezuela',
    'Maritimo (Venezuela)': 'Marítimo (Venezuela)',
    'Maritimo (Venezuela': 'C.S. Marítimo de Venezuela',


    # Colombia

    'CD Itagüí-Ditaires': 'Itagüí',

    'Universitario de Popayán': 'Universitario Popayán',


    'Equidad': 'La Equidad',
    'CD La Equidad (Bogotá)': 'La Equidad',

    'Bucaramanga': 'Atlético Bucaramanga',
    'Bucuramanga': 'Atlético Bucaramanga',
    'Atletico Bucaramanga': 'Atlético Bucaramanga',
    'Atletico Bucuramanga': 'Atlético Bucaramanga',
    'Atl Bucaramanga': 'Atlético Bucaramanga',
    'Atl. Bucaramanga': 'Atlético Bucaramanga',



    'CD Lara': 'Deportivo Lara',

    'Atletico Huila': 'Atlético Huila',
    'Huila': 'Atlético Huila',
    'Atl. Huila': 'Atlético Huila',

    'Pasto': 'Deportivo Pasto',
    'Dep. Pasto': 'Deportivo Pasto',

    'Tolima': 'Deportes Tolima',
    'Dep. Tolima': 'Deportes Tolima',

    'Dep. Quindío': 'Deportes Quindío',
    'Quindío': 'Deportes Quindío',
    'Quindio': 'Deportes Quindío',
    'Deportes Quindio': 'Deportes Quindío',

    'Boyaca Chico FC': 'Boyacá Chicó',
    'Chicó FC': 'Boyacá Chicó',
    'Boyacá Chicó FC': 'Boyacá Chicó',

    'Depor FC Aguablanca (Cali)': 'Depor Aguablanca',

    'Fortaleza FC (Soacha)': 'Fortaleza F.C.',

    'Pacifico FC (Buenaventura)': 'Pacífico FC',

    'Sucre FC (Sincelejo)': 'Sucre FC',

    'Alianza Petrolera (Barrancabermeja)': 'Alianza Petrolera',

    'Academía FC (Bogotá)': 'Academía FC',

    'Corporación Tuluá': 'Cortuluá',

    'Tuluá': 'Cortuluá',
    'Dep. Tuluá': 'Cortuluá',
    'Tulua': 'Cortuluá',
    'Tulúa': 'Cortuluá',

    'Caldas': 'Once Caldas',

    'CD Santa Fe': 'Independiente Santa Fe',
    'Ind. De Santa Fe': 'Independiente Santa Fe',
    'Indep. Santa Fé': 'Independiente Santa Fe',
    'Independ Santa Fe': 'Independiente Santa Fe',
    'Ind. Santa Fe': 'Independiente Santa Fe',
    'Santa Fe (Colombia)': 'Independiente Santa Fe',
    'Indep. Santa Fe': 'Independiente Santa Fe',

    'Academia': 'Academía FC',
    'Academia FC': 'Academía FC',

    'Uniautónoma FC (Sabanalarga)': 'Uniautónoma FC',

    'Deportivo Rionegro': 'Rionegro',
    'Dep. Unicosta': 'Unicosta',

    'Centauros Villavicencio': 'Centauros',
    'Centauros Villavicencio (Popayán)': 'Centauros',

    'Cúcuta Dep.': 'Cúcuta Deportivo',
    'Deportivo Cúcuta': 'Cúcuta Deportivo',
    'Cucuta Deportivo': 'Cúcuta Deportivo',
    'Cúcuta': 'Cúcuta Deportivo',

    'Boyacá Chicó Fc': 'Boyacá Chicó',
    'Boyaca Chico': 'Boyacá Chicó',
    'Chicó': 'Boyacá Chicó',

    'Independiente De Medellín': 'Independiente Medellín',
    'Dep. Indep. Medellín': 'Independiente Medellín',
    'DI Medellin': 'Independiente Medellín',
    'DI Medellín': 'Independiente Medellín',
    'IMedellín': 'Independiente Medellín',
    'Medellin': 'Medellín',
    'Medellín': 'Independiente Medellín',
    'Independiente Medellin': 'Independiente Medellín',
    'Indep. Medellín': 'Independiente Medellín',
    'DIM': 'Independiente Medellín',
    'CD Indep Medellín': 'Independiente Medellín',
    'CD Independiente Medellín': 'Independiente Medellín',


    'Nacional (Colombia)': 'Atletico Nacional',
    'Atletico Nacional Medellin': 'Atletico Nacional',
    'Atletico Nacional': 'Atlético Nacional',
    'Atl. Nacional': 'Atlético Nacional',

    'Atlético Junior': 'Junior de Barranquilla',
    'Atl. Junior': 'Junior de Barranquilla',
    'Corporacion Popular Junior': 'Junior de Barranquilla',

    'Junior (Colombia)': 'Junior de Barranquilla',
    'Atletico Junior': 'Junior de Barranquilla',
    'Atlético Junior': 'Junior de Barranquilla',
    'Atlético Júnior': 'Junior de Barranquilla',

    'CD America Cali': 'América de Cali',
    'America (Colombia)': 'América de Cali',
    'América De Cali': 'América de Cali',
    'Cali': 'América de Cali',
    'América Cali': 'América de Cali',
    'América (Cali)': 'América de Cali',
    'America (Cali)': 'América de Cali',
    'America Cali': 'América de Cali',
    'America de Cali - Colombia': 'America de Cali',
    'America-Cali': 'América de Cali',
    'America de Cali': 'América de Cali',

    'Buenaventura - Colombia': 'Buenaventura',
    'Bueneventura (Colombia)': 'Buenaventura',

    'Dep. Cali': 'Deportivo Cali',
    'D.Cali': 'Deportivo Cali',
    'Deportivo Cali - Colombia': 'Deportivo Cali',

    'Millonarios - Colombia': 'Millonarios',
    'Millionarios -- Colombia': 'Millonarios',
    'Millionarios': 'Millonarios FC',
    'Millonarios': 'Millonarios FC',
    'CD Millonarios': 'Millonarios FC',

    'Soacha FC - Colombia': 'Soacha FC',

    # Ecuador

    'Portoviejo': 'LDU Portoviejo', # check.
    'LDU Portoviejo': 'LDU de Portoviejo',

    'Quevedo': 'Deportivo Quevedo',
    'Dep. Quevedo': 'Deportivo Quevedo',


    'Tecnico Universitario': 'Técnico Universitario',
    'Técnico Univ.': 'Técnico Universitario',

    'Manta F.C.': 'Manta',

    'El Nacional (Ecuador)': 'CD El Nacional',
    'Nacional (Ecuador)': 'CD El Nacional',
    'El Nacional': 'CD El Nacional',
    'El Nacional (Quito)': 'CD El Nacional',


    'Valdez S.C.': 'Valdez SC',
    'LDU (Quito)': 'LDU Quito',
    'LDU': 'LDU Quito',
    'America de Quito': 'América de Quito',
    'Aucas': 'SD Aucas',

    'Barcelona -- Ecuador': 'Barcelona Sporting Club',
    'Barcelona (Ecuador)': 'Barcelona Sporting Club',
    'Barcelona SC': 'Barcelona Sporting Club',    
    
    'LDU (Quito)': 'LDU Quito',
    'Olmedo (Riobamba)': 'CD Olmedo',
    'Everest': 'CD Everest',

    'Imbabura SC': 'Imbabura',
    'Imbabura S.C.': 'Imbabura',

    # Peru

    'Col. San Agustín': 'San Agustín',

    'Cienciano (Cuzco)': 'Cienciano',
    'Cienciano (Cusco)': 'Cienciano',


    'Loretana': 'La Loretana',

    'Ciclista': 'Ciclista Lima', # general

    'Alcides': 'Alcides Vigo',

    'Pesquero': 'Deportivo Pesquero',
    'Deportivo Pesquero': 'Deportivo Wanka', # merger

    'Minas': 'Unión Minas',
    'Union Minas': 'Unión Minas',


    'Total Clean': 'Total Chalaco', # name map
    'Total Chalaco (Callao)': 'Total Chalaco',

    'Atletico Chalaco': 'Atlético Chalaco',

    'Sport Boys (Callao)': 'Sport Boys',

    'Defensor Arica': 'Defensor Lima',


    'Municipal Lima': 'Deportivo Municipal',
    'Dep. Municipal': 'Deportivo Municipal',
    'Municipal (Peru)': 'Deportivo Municipal',
    'Deportivo Municipal (Lima)': 'Deportivo Municipal',


    'Atletico Torino': 'Atlético Torino',

    'Juan Bielovucic': 'Sport Juan Bielovucic',

    'Lima Cricket': 'Lima CFC',

    'Atletico Universidad': 'Atlético Universidad',
    'Atl. Universidad': 'Atlético Universidad',
    'Atlético Universidad Nacional (Arequipa)': 'Atlético Universidad',
    'Atlético Universidad Nacional': 'Atlético Universidad',

    'Atlético Minero (Matucana)': 'Atlético Minero',

    'Alianza Atlético (Sullana)': 'Alianza Atlético',

    'Real Garcilaso (Cusco)': 'Real Garcilaso',

    'FBC Melgar (Arequipa)': 'FBC Melgar',

    'Cobresol FBC (Moquegua)': 'Cobresol FBC',

    'Deportivo U.P.A.O.': 'Sport Coopsol Trujillo',
    'Deportivo UPAO': 'Sport Coopsol Trujillo',
    'UPAO': 'Sport Coopsol Trujillo',
    'Coopsol': 'Sport Coopsol Trujillo',
    'Coopsol Trujillo': 'Sport Coopsol Trujillo',
    'Sport Coopsol': 'Sport Coopsol Trujillo',

    'Aviación F.A.P.': 'Deportivo Coopsol',

    'Universidad SMP': 'Universidad San Martín',
    'Universidad San Martin': 'Universidad San Martín',
    'Univ. San Martín': 'Universidad San Martín',
    'Univ.San Martin de P.': 'Universidad San Martín',
    'Univ.San Martín de P.': 'Universidad San Martín',
    'Univ. San Martin de P.': 'Universidad San Martín',
    'Univ. San Martín de P.': 'Universidad San Martín',
    'San Martín': 'Universidad San Martín',

    'Sports Boys': 'Sport Boys',

    'Huaral': 'Unión Huaral',
    'Sport Unión Huaral': 'Unión Huaral',
    'Union Huaral': 'Unión Huaral',
    'Unión Huaral (Huaral)': 'Unión Huaral',



    'León De Huánuco': 'León de Huánuco',
    'Leon de Huanuco': 'León de Huánuco',
    'León de Huanuco': 'León de Huánuco',

    'Sport Ancash': 'Sport Áncash',
    'Sport Áncash (Huaraz)': 'Sport Áncash',

    'Sport Alianza': 'Alianza Lima',
    'Alianza Lima (Lima)': 'Alianza Lima',
    'Alianza Lima - Peru': 'Alianza Lima',

    'Sporting Cristal (Lima)': 'Sporting Cristal',
    'Sporting Cristal -- Peru': 'Sporting Cristal',
    'Spirting Cristal': 'Sporting Cristal',

    'Total Clean (Arequipa)': 'Total Clean',

    'Universitario Lima': 'Universitario',
    'Universitario (Peru)': 'Universitario',
    'Universitario (Lima)': 'Universitario',
    'Federacion Universitaria': 'Universitario', # name map

    'Pacífico FC (Lima)': 'Pacífico FC',

    'Centro Iqueno': 'Centro Iqueño',

    'Jorge Chavez': 'Jorge Chávez (Callao)',

    'Melgar': 'FBC Melgar',
    'Mariano Melgar': 'FBC Melgar',
    'FBC. Melgar': 'FBC Melgar',
    'Melgar FBC': 'FBC Melgar',

    'Colegio Nacional de Iquitos': 'Colegio Nacional Iquitos',

    'Universidad San Martín de Porres (Lima)': 'Universidad San Martín de Porres',

    # Bolivia

    'Guabira': 'Guabirá',

    'Union Central': 'Unión Central',
    'Unión Central': 'Unión Tarija',


    'Nacional (Potosí)': 'Nacional Potosí',

    'Destroyers': 'Club Destroyers',
    'La Paz': 'La Paz FC',

    'Petrolero (Yacuiba)': 'Club Petrolero',
    'Petrolero Yacuiba': 'Club Petrolero',


    '31 de Octubre': 'Club 31 de Octubre',
    'Aurora (Bolivia)': 'Club Aurora',
    'Bolivar': 'Bolívar',
    'Bolívar': 'Club Bolívar',
    'Real Potosi': 'Real Potosí',
    'San Jose (Oruro)': 'CD San José',
    'San Jose (Bolivia)': 'San José (Bolivia)',
    'San Jose (Bolivia': 'CD San José',
    'San José (Bolivia)': 'CD San José',

    #'Universitario Sucre': 'Club Universitario',
    'Universitario Sucre': 'CD Universitario San Francisco Xavier',
    'Universitario (Sucre)': 'CD Universitario San Francisco Xavier',
    'U. De Sucre': 'CD Universitario San Francisco Xavier',

    # Paraguay

    'Resistencia': 'Resistencia SC',
    'Universal': 'Club Universal',

    'Olimpia Asuncion': 'Club Olimpia',
    'Olímpia (Paraguay)': 'Club Olimpia',

    'Recoleta': 'Deportivo Recoleta',
    'Dep. Recoleta': 'Deportivo Recoleta',

    'Silvio Pettirossi': 'Club Silvio Pettirossi',
    'General Díaz': 'Club General Díaz',

    'Libertad (Paraguay)': 'Club Libertad',

    'Atl. Colegiales': 'Atlético Colegiales',
    'Colegiales': 'Atlético Colegiales',
    'Atletico Colegiales': 'Atlético Colegiales',

    'Sportivo Luqueno': 'Sportivo Luqueño',
    'Sp.Luqueño': 'Sportivo Luqueño',
    'Sp. Luqueño': 'Sportivo Luqueño',

    'Luqueño': 'Sportivo Luqueño', # maybe not?
    'Luqueno': 'Sportivo Luqueño', # maybe not?

    #'Guarani': 
    'Guarani': 'Club Guaraní',
    'Guaraní': 'Club Guaraní',
    'Guarani (Paraguay)': 'Club Guaraní',
    'Olimpia (Paraguay)': 'Club Olimpia',
    'Olimpia Paraguay': 'Club Olimpia',

    'Club Atlético Cerro': 'CA Cerro',


    'Cerro Cora': 'Cerro Corá',
    'Cerro C': 'Cerro Corá',
    'C Cora': 'Cerro Corá',

    'C. Porteño': 'Cerro Porteño',
    'Cerro P': 'Cerro Porteño',
    'C Porteno': 'Cerro Porteño',
    'C. Porteno': 'Cerro Porteño',
    'Cerro Porteno': 'Cerro Porteño',

    'Nacional (Paraguay)': 'Club Nacional',
    'Nacional De Paraguay': 'Club Nacional',
    'Nacional (Asunción)': 'Club Nacional',
    'Nacional (Asuncion)': 'Club Nacional',

    'Nueve de Octubre': '12 de Octubre',
    '12 Octubre': '12 de Octubre',
    '12 de octubre': '12 de Octubre',

    'Sol': 'Club Sol de América',
    'Sol de America': 'Club Sol de América',
    'Sol de América': 'Club Sol de América',


    # Chile

    'Ñublense (Chillán)': 'Ñublense',
    'Nublense': 'Ñublense',

    'Prov. Curicó Unido': 'Curicó Unido',
    'Curico Unido': 'Curicó Unido',
    'Provincial Curicó Unido': 'Curicó Unido',

    'Dep. Melipilla': 'Deportes Melipilla',

    #'Morning Star': 'Santiago Morning', # Add name map. # too broad?

    'Puerto Montt': 'Deportes Puerto Montt',
    'Pto. Montt': 'Deportes Puerto Montt',


    'Stgo. Morning': 'Santiago Morning', 
    'Stgo Morning': 'Santiago Morning', 
    'S. Morning': 'Santiago Morning', 
    'Santiago Morning (Santiago)': 'Santiago Morning', 

    'Dep. Temuco': 'Deportes Temuco',
    'Temuco': 'Deportes Temuco',

    'Atacama': 'Regional Atacama',

    'Osorno': 'Provincial Osorno',
    'Prov. Osorno': 'Provincial Osorno',


    'Ferroviarios': 'C.D. Ferroviarios de Chile',
    'CCD Arturo Fernández Vial': 'Arturo Fernández Vial',
    'Fernandez Vial': 'Arturo Fernández Vial',
    'F. Vial (Concepción)': 'Arturo Fernández Vial',
    'Fernández Vial': 'Arturo Fernández Vial',
    'Naval': 'Deportes Naval',
    'Badminton': 'Badminton F.C.',
    'Antofagasta': 'Deportes Antofagasta',

    'Stgo. Wanderers': 'Santiago Wanderers',
    'S. Wanderers': 'Santiago Wanderers',
    'S.Wanderers': 'Santiago Wanderers',

    'Rangers (Chile)': 'C.S.D. Rangers',
    'Rangers de Talca': 'C.S.D. Rangers',
    'Rangers (Talca)': 'C.S.D. Rangers',

    'Iquique': 'Deportes Iquique',
    'Dep. Iquique': 'Deportes Iquique',

    'Audax CS Italiano': 'Audax Italiano',
    'Audax I.': 'Audax Italiano',
    'A. Italiano': 'Audax Italiano',
    'A.Italiano': 'Audax Italiano',
    'Audax Italiano (Santiago)': 'Audax Italiano',

    'Colo Colo (Santiago)': 'Colo Colo',
    'Colo Colo - Chile': 'Colo Colo',
    'Colo Colo': 'Colo-Colo',

    'Univ. de  Concepción': 'Universidad de Concepción',
    'Universidad de Concepcion': 'Universidad de Concepción',
    'Univ. de Concepcion': 'Universidad de Concepción',
    'Univ. de Concepción': 'Universidad de Concepción',
    'U. de Concepción': 'Universidad de Concepción',
    'Univ. Concepción': 'Universidad de Concepción',
    
    'Coquimbo': 'Coquimbo Unido',
    'Coquimbo U.': 'Coquimbo Unido',

    'Concepcion': 'Concepción',

    'Deportes Concepcion': 'Deportes Concepción',
    'Dep. Concepcion': 'Deportes Concepción',

    'Everton (Viña del Mar)': 'Everton de Viña del Mar',
    'Everton Chile': 'Everton de Viña del Mar',
    'Everton de Viña del Mar': 'Everton (Chile)',

    'Magallanes - Chile': 'Magallanes',
    'Magallanes': 'Deportes Magallanes',

    'O\'Higgins': 'O\'Higgins F.C.',
    'O\'higgins': 'O\'Higgins F.C.',
    'O´Higgins': 'O\'Higgins F.C.',
    'O\'Higgins (Rancagua)': 'O\'Higgins F.C.',

    'Palestino (Santiago)': 'Palestino',
    'Palestino (Chile)': 'Palestino',
    'Palestino - Chile': 'Palestino',
    'Palestino': 'CD Palestino',

    'San Felipe - Chile': 'Unión San Felipe',
    'Union San Felipe': 'Unión San Felipe',
    'U. San Felipe': 'Unión San Felipe',



    'Unión Española (Santiago)': 'Unión Española',
    'Union Espanola': 'Unión Española',
    'U.Espanola': 'Unión Española',
    'U. Espanola': 'Unión Española',
    'U. Española': 'Unión Española',
    'Union Española': 'Unión Española',


    'U. De Chile': 'Universidad de Chile',
    'U. de Chile': 'Universidad de Chile',
    'Universidad de Chile - Chile': 'Universidad de Chile',
    'Club Universidad de Chile': 'Universidad de Chile',
    'U.de Chile': 'Universidad de Chile',
    'Univ. de Chile': 'Universidad de Chile',
    'Universidad de Chile (Santiago)': 'Universidad de Chile',

    'Universidad Católica (Santiago)': 'CD Universidad Católica',
    'Católica': 'CD Universidad Católica', # generous
    'U. Católica': 'CD Universidad Católica',
    'U.Catolica': 'CD Universidad Católica',
    'U. Catolica': 'CD Universidad Católica',
    'Univ. Catolica': 'CD Universidad Católica',
    'Universidad Catolica - Chile': 'CD Universidad Católica',
    'Universidad Catolica': 'CD Universidad Católica',
    'Uni. Católica': 'CD Universidad Católica',
    'Univ. Católica': 'CD Universidad Católica',
    'Univ. Católica': 'CD Universidad Católica',
    'CD Universidad Católica': 'Universidad Católica',


    'Cobreloa (Calama)': 'Cobreloa',

    'Cobresal (El Salvador)': 'Cobresal',


    'Huachipato (Talcahuano)': 'Huachipato',

    'Dep. Copiapó SA': 'Deportes Copiapó',

    'Municipal Iquique': 'Deportes Iquique',

    'San Felipe': 'Unión San Felipe', # general.

    # Uruguay

    'Tacuarembó FC (Tacuarembó)': 'Tacuarembó',
    'Tacuarembo': 'Tacuarembó',
    'Tacuarembó FC': 'Tacuarembó',
    'Deportivo Tacuarembó': 'Tacuarembó',


    'Central Espanol': 'Central Español',
    'Central Español FC': 'Central Español',


    'Miramar': 'Miramar Misiones',
    'Miramar Misiones FC': 'Miramar Misiones',
    'CS Miramar Misiones': 'Miramar Misiones',


    'CS Uruguay (Coronado)': 'Uruguay de Coronado',
    'Uruguay (Coronado)': 'Uruguay de Coronado',

    'Nacional De Uruguay': 'CA Nacional',
    #'CA Nacional': 'Nacional',

    'C.U.R.C.C.': 'CURCC',

    'Juventud LP': 'Juventud de Las Piedras',
    'Juventud (Las Piedras)': 'Juventud de Las Piedras',
    'CA Juventud (Las Piedras)': 'Juventud de Las Piedras',

    'CA Cerro': 'Cerro',

    'Cerro Largo FC (Melo)': 'Cerro Largo',

    'Cerrito': 'CS Cerrito',

    'Rocha': 'Rocha FC',
    'Rocha FC (Rocha)': 'Rocha FC',

    'Frontera': 'Frontera Rivera',



    'CCyD El Tanque Sisley': 'El Tanque Sisley',
    'El Tanque': 'El Tanque Sisley',

    'CA Plaza (Colonia)': 'Plaza Colonia',
    'Plaza': 'Plaza Colonia',

    
    'CD Colonia (Juan Lacaze, CO)': 'Deportivo Colonia',
    'Dep. Colonia': 'Deportivo Colonia',
    'Colonia': 'Deportivo Colonia',

    'IA Sud America': 'IA Sud América',
    'Sud América': 'IA Sud América',

    'Paysandú BV': 'Paysandú Bella Vista',
    'Bella Vista (Paysandú)': 'Paysandú Bella Vista',

    'Paysandú FC (Paysandú)': 'Paysandú FC',

    'Villa Esp.': 'Villa Española',
    'Villa Espanola': 'Villa Española',
    'CSyD Villa Española': 'Villa Española',

    'Huracan B.': 'Huracán Buceo',
    'H.Buceo': 'Huracán Buceo',
    'H. Buceo': 'Huracán Buceo',
    'Huracán B.': 'Huracán Buceo',

    'Club Deportivo Maldonado': 'Deportivo Maldonado',
    'CD Maldonado (Maldonado)': 'Deportivo Maldonado',
    'Maldonado': 'Deportivo Maldonado',
    'CD Maldonado': 'Deportivo Maldonado',
    'Dep. Maldonado': 'Deportivo Maldonado',

    'Rampla Jrs.': 'Rampla Juniors',
    'Rampla': 'Rampla Juniors',

    'Dep. La Serena': 'Deportes La Serena',
    'La Serena': 'Deportes La Serena',


    'Atletico Wanderers': 'Montevideo Wanderers', # name map.
    'Montevideo Wanderers FC': 'Montevideo Wanderers',
    'M.Wanderers': 'Montevideo Wanderers',
    'M. Wanderers': 'Montevideo Wanderers',

    'River Plate Montevideo': 'River Plate (Montevideo)',
    'CA River Plate Montevideo': 'River Plate (Montevideo)',

    'Bella Vista': 'CA Bella Vista',
    'C.A. Bella Vista': 'CA Bella Vista',
    'CA Bella Vista (M\'video)': 'CA Bella Vista',
    'Bella Vista (Montevideo)': 'CA Bella Vista',

    'Fénix': 'CA Fénix',

    'Danubio FC': 'Danubio F.C.',
    'Danubio': 'Danubio FC',

    'Defensor Sporting Club': 'Defensor SC',
    'Defensor': 'Defensor SC',
    'Defensor Sporting': 'Defensor SC',
    'Defensor Sp.': 'Defensor SC',
    'Defensor Sp': 'Defensor SC',

    'Liverpool FC (Montevideo)': 'Liverpool (Montevideo)',
    'Liverpool (Uruguay)': 'Liverpool (Montevideo)',

    'C Nacional dF': 'CA Nacional',
    'Nacional de Montevideo': 'CA Nacional',
    'Club Nacional de Football': 'CA Nacional',
    'Nacional -- Uruguay': 'CA Nacional',
    'Nacional (Montevideo)': 'CA Nacional',
    'Nacional (Uruguay)': 'CA Nacional',

    'Penarol': 'Peñarol',
    'CA Peñarol': 'Peñarol',
    'CA Penarol': 'Peñarol',
    'C.A. Peñarol': 'Peñarol',

    'Racing Sayago': 'Racing Club de Montevideo',
    'Racing Club de Sayago': 'Racing Club de Montevideo',
    'Racing CM': 'Racing Club de Montevideo',
    'Racing (Montevideo)': 'Racing Club de Montevideo',

    'Rampla Juniors FC': 'Rampla Juniors',

    'River Plate (Uruguay)': 'River Plate (Montevideo)',

    'CA Atenas (San Carlos)': 'Atenas',

    # Argentina



    'Atlético Tigre': 'Tigre',

    'Los Andes de Lomas de Zamora': 'Los Andes',

    'Platense (VL)': 'CA Platense',
    'Platense (VLo)': 'CA Platense',

    'Almagro': 'Club Almagro',

    'Talleres (RdE)': 'Talleres de Remedios de Escalada',
    'Talleres (Remedios de Escalada)': 'Talleres de Remedios de Escalada',

    'Club Atlético River Plate': 'River Plate',
    'CA River Plate': 'River Plate',

    'Chacarita': 'Chacarita Juniors',
    'Chacarita Js': 'Chacarita Juniors',
    'Chacarita Jrs': 'Chacarita Juniors',



    'Lomas Athletic': 'Lomas AC',
    'Lomas': 'Lomas AC',

    'Barracas Athletic': 'Barracas AC',

    'Reformer': 'Reformer AC',


    #'Talleres': 'Talleres de Córdoba', # Overreach.
    'Talleres (C)': 'Talleres de Córdoba',
    'Talleres (Cordoba)': 'Talleres de Córdoba',
    'Talleres (COR)': 'Talleres de Córdoba',
    'Talleres (Cor)': 'Talleres de Córdoba',
    'Talleres(Cba.)': 'Talleres de Córdoba',
    'Talleres (Cba)': 'Talleres de Córdoba',
    'Talleres (Cba.)': 'Talleres de Córdoba',
    'Talleres Cba': 'Talleres de Córdoba',
    'Talleres Cba.': 'Talleres de Córdoba',
    'Talleres (Córdoba)': 'Talleres de Córdoba',
    'Tallares (Córdoba)': 'Talleres de Córdoba',


    'Deportivo Italiano': 'Sportivo Italiano',

    'Quilmes Atlético Club': 'Quilmes AC',
    'Quilmes': 'Quilmes AC',
    'Quilmes Club': 'Quilmes AC',

    'Estudiantes de Eva Perón': 'Estudiantes de La Plata',
    'Estudiantes(LP)': 'Estudiantes de La Plata',
    'Estudiantes (La Plata)': 'Estudiantes de La Plata',
    'Estudiantes LP': 'Estudiantes de La Plata',
    'Estudiantes-LP': 'Estudiantes de La Plata',
    'Estudiantes (Argentina)': 'Estudiantes de La Plata',
    'Estudiantes de la Plata': 'Estudiantes de La Plata',
    'Estudiantes LPG': 'Estudiantes de La Plata',
    'Estudiantes (LP)': 'Estudiantes de La Plata',
    'Estudiantes(La Plata)': 'Estudiantes de La Plata',
    'Estudiantes Lp': 'Estudiantes de La Plata',

    'Estudiantes (C)': 'Estudiantes de Caseros',
    'Atlético Estudiantes': 'Estudiantes de Caseros',
    'CA Estudiantes': 'Estudiantes de Caseros',
    'Estudiantes (Bs.As.)': 'Estudiantes de Caseros',

 

    'Rosario C.': 'Rosario Central',
    'CA Rosario Central': 'Rosario Central',
    'Rosario Ctral': 'Rosario Central',
    'Rosario Cent.': 'Rosario Central',

    'Ferrocarril': 'Ferro Carril Oeste',
    'Ferrocarril Oeste': 'Ferro Carril Oeste',
    'Ferro C. O.': 'Ferro Carril Oeste',
    'Ferro Carril Oeste(BA)': 'Ferro Carril Oeste',
    'Ferro Carril Oeste (BA)': 'Ferro Carril Oeste',
    'Ferro Carril Oeste (BUE)': 'Ferro Carril Oeste',
    'Club Ferro Carril Oeste': 'Ferro Carril Oeste',
    'Ferro Carril Oeste (Buenos Aires)': 'Ferro Carril Oeste',
    'Ferro Carril Oeste C. O.': 'Ferro Carril Oeste',


    'Atl. Tucumán': 'Atlético Tucumán',

    'Argentinos Jrs.': 'Argentinos Juniors',
    'Argentinos Js': 'Argentinos Juniors',
    'Arg Juniors': 'Argentinos Juniors',
    'Argentinos Jrs': 'Argentinos Juniors',
    'Argentinos J': 'Argentinos Juniors',

    'Olimpo De Bahía Blanca': 'Olimpo',
    'Olimpo (BB)': 'Olimpo',
    'Club Olimpo': 'Olimpo',
    'Olimpo (Bahía Blanca)': 'Olimpo',


    'Union Santa Fe': 'Unión de Santa Fe',
    'Unión De Santa Fe': 'Unión de Santa Fe',
    'Unión (Sta.Fe)': 'Unión de Santa Fe',
    'Union (SF)': 'Unión de Santa Fe',
    'Unión (SF)': 'Unión de Santa Fe',
    'Unión SF': 'Unión de Santa Fe',
    'Unión (Santa Fe)': 'Unión de Santa Fe',
    'Unión(Sta.Fe)': 'Unión de Santa Fe',
    'Unión (Sta Fe)': 'Unión de Santa Fe',
    'Unión (Sta. Fe)': 'Unión de Santa Fe',
    'Unión (SFN)': 'Unión de Santa Fe',
    'UNIÓN (SFN)': 'Unión de Santa Fe',

    'Kimberley (MdP)': 'Kimberley',
    'Kimberley (Mar del Plata)': 'Kimberley',
    'Kimberley(Mar del Plata)': 'Kimberley',
    'Kimberley(MdP)': 'Kimberley',
    'Kimberley (MDQ)': 'Kimberley',

    'Los Andes (LdZ)': 'Los Andes de Lomas de Zamora',

    'Dep Espanol': 'Deportivo Español',
    'Dep. Español': 'Deportivo Español',



    'San Martín (Mza)': 'San Martín de Mendoza',
    'San Martín (MDZ)': 'San Martín de Mendoza',
    'San Martín (Mendoza)': 'San Martín de Mendoza',
    'San Martín (M)': 'San Martín de Mendoza',
    'San Martín (Mdz)': 'San Martín de Mendoza',

    'San Martín (SJ)': 'San Martín de San Juan',
    'San Martín De San Juán': 'San Martín de San Juan',


    'San Martín (T)': 'San Martín de Tucumán',
    'San Martin (T)': 'San Martín de Tucumán',
    'San Martín (Tuc)': 'San Martín de Tucumán',
    'San Martín (Tuc.)': 'San Martín de Tucumán',
    'San Martin (Tuc)': 'San Martín de Tucumán',
    'San Martín (TUC)': 'San Martín de Tucumán',
    'San Martín (Tucumán': 'San Martín de Tucumán',
    'San Martín (Tucumán)': 'San Martín de Tucumán',

    'Juventud Antoniana (S)': 'Juventud Antoniana',
    'Juventud Antoniana (Salta)': 'Juventud Antoniana',
    'Juv. Antoniana (S)': 'Juventud Antoniana',

    'Deportivo Roca (RN)': 'Deportivo Roca',
    'Deportivo Roca (Río Negro)': 'Deportivo Roca',

    'Gimnasia y Esgrima BA': 'Gimnasia y Esgrima Buenos Aires',


    'Gimnasia y Esgrima M': 'Gimnasia y Esgrima (Mendoza)',
    'Gimnasia y Esgrima(Mza)': 'Gimnasia y Esgrima (Mendoza)',
    'Gimnasia y Esgrima (M)': 'Gimnasia y Esgrima (Mendoza)',
    'Gimnasia y Esgrima MDZ': 'Gimnasia y Esgrima (Mendoza)',
    'Gimnasia y Esgrima(Mendoza)': 'Gimnasia y Esgrima (Mendoza)',
    'Gimnasia y Esgrima (Mza.)': 'Gimnasia y Esgrima (Mendoza)',
    'Gimnasia y Esgrima (Mza)': 'Gimnasia y Esgrima (Mendoza)',
    'Gimnasia y Esgrima (Mendoza)': 'Gimnasia y Esgrima de Mendoza',
    'Gimnasia y Esgrima (MDZ)': 'Gimnasia y Esgrima de Mendoza',


    'Central Córdoba (Sgo)': 'Central Córdoba (Santiago)',
    'Central Córdoba (Santiago del Estero)': 'Central Córdoba (Santiago)',
    'Central Córdoba (SDE)': 'Central Córdoba (Santiago)',

    'Central Córdoba (R)': 'Central Córdoba (Rosario)',

    'Gimnasia (ER)': 'Gimnasia y Esgrima (Entre Ríos)',

    'Gimnasia Jujuy': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia y Esgrima (Jujuy)': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia y Esgrima J': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia y Esgrima (JUJ)': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia (Jujuy)': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia y Esg. (Jujuy)': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia J': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia y Esgrima (J)': 'Gimnasia y Esgrima de Jujuy',
    'Gimnasia y Esg.(Jujuy)': 'Gimnasia y Esgrima de Jujuy',

    'Gimnasia y Esgrima Eva Perón': 'Gimnasia y Esgrima La Plata', # La Plata renamed Eva Peron
    'Gimnasia y Esgrima(LP)': 'Gimnasia y Esgrima La Plata',
    'Gimnasia (La Plata)': 'Gimnasia y Esgrima La Plata',
    'Gimnasia LP': 'Gimnasia y Esgrima La Plata',
    'Gymnasia LP': 'Gimnasia y Esgrima La Plata',
    'Gimnasia Esgrima (LP)': 'Gimnasia y Esgrima La Plata',
    'Gimnasia y Esgrima (LP)': 'Gimnasia y Esgrima La Plata',
    'Gimnasia y Esgrima LPG': 'Gimnasia y Esgrima La Plata',
    'Gimnasia (LP)': 'Gimnasia y Esgrima La Plata',
    'Gimnasia Lp': 'Gimnasia y Esgrima La Plata',

    'Gimnasia y Esgrima LP': 'Gimnasia y Esgrima La Plata',
    'Gimansia y Esgrima LP': 'Gimnasia y Esgrima La Plata', # correct?
    'Grimnasia y Esgrima LP': 'Gimnasia y Esgrima La Plata', # correct?


    'Gimnasia S': 'Gimnasia y Tiro (Salta)',
    'Gimnasia y Tiro (Salta)': 'Gimnasia y Tiro',

    'Independiente - Argentina': 'CA Independiente',
    'Independiente (Argentina)': 'CA Independiente',
    'Independiente (A)': 'CA Independiente',
    'Independiente (Avellaneda)': 'CA Independiente',
    'Independiente (Ave)': 'CA Independiente',

    'Velez Sarsfield': 'Vélez Sarsfield',
    'Vélez Sársfield': 'Vélez Sarsfield',
    'Vélez  Sarsfield': 'Vélez Sarsfield',

    'Arsenal (Argentina)': 'Arsenal de Sarandí',
    'Arsenal de Sarandi': 'Arsenal de Sarandí',
    'Arsenal De Sarandí': 'Arsenal de Sarandí', 


    'Colon (Argentina)': 'CA Colon',

    'Colón De Santa Fe': 'CA Colon', 
    'Colón (Sta. Fe)': 'CA Colon', 
    'Colón (Sta.Fe)': 'CA Colon',
    'Colón (SF)': 'CA Colon',
    'Colón SF': 'CA Colon',
    'Colón (Santa Fe)': 'CA Colon',
    'Colon (Santa Fe)': 'CA Colon',
    'Colon': 'CA Colon',
    'Colón': 'CA Colon',
    'CA Colon': 'CA Colón',

    'Belgrano FC': 'Belgrano AC',
    'CA Belgrano': 'Belgrano AC',
    'Belgrano': 'Belgrano AC',
    'Belgrano Athletic': 'Belgrano AC',
    'Belgrano CBA': 'Belgrano AC',
    'Belgrano Cba.': 'Belgrano AC',
    'Belgrano (Cba)': 'Belgrano AC',
    'Belgrano': 'Belgrano AC',
    'Club Atlético Belgrano': 'Belgrano AC',
    'Belgrano Cba': 'Belgrano AC',
    'Belgrano (Cba.)': 'Belgrano AC',
    'Belgrano (COR)': 'Belgrano AC',
    'Belgrano (C)': 'Belgrano AC',
    'Belgrano (Córdoba)': 'Belgrano AC',

    'Guaraní A. Franco (Mis)': 'Guaraní Antonio Franco',
    'Guaraní A. Franco (Misiones)': 'Guaraní Antonio Franco',
    'Guaraní A. Franco (Mis.)': 'Guaraní Antonio Franco',

    'Atlético Concepción (Tucumán)': 'Atlético Concepción',
    'Atlético Concepción(Tuc)': 'Atlético Concepción',
    'Atlético Concepción(Tucumán)': 'Atlético Concepción',

    'Boca Juniors - Argentina': 'Boca Juniors',
    'CA Boca Juniors': 'Boca Juniors',
    'Deportivo Moron': 'Deportivo Morón',

    'Dep.Mandiyú (Ctes.)': 'Club Deportivo Mandiyú',
    'Mandiyú(Ctes.)': 'Club Deportivo Mandiyú',

    'Independiente': 'CA Independiente',
    'Independiente': 'CA Independiente',
    'Lanus': 'Lanús',

    'Huracán (Comodoro Rivadavia)': 'Huracán de Comodoro Rivadavia',
    'Huracán (Com.Rivadavia)': 'Huracán de Comodoro Rivadavia',
    'Huracán (CR)': 'Huracán de Comodoro Rivadavia',

    'Huracan C': 'Huracán (Corrientes)',
    'Huracán (Corrientes)': 'Huracán Corrientes',

    'Huracán (TA)': 'Huracán de Tres Arroyos',
    'Huracan (Tres Arroyos)': 'Huracán de Tres Arroyos',
    'Huracan (TA)': 'Huracán de Tres Arroyos',

    'Huracan BA': 'Huracán',
    'Huracán (BA)': 'Huracán',
    'Huracán (Buenos Aires)': 'Huracán',
    'Huracan': 'Huracán',
    'Huracán (BUE)': 'Huracán',
    
    'Atlético Rafaela': 'Atlético de Rafaela',
    'Atletico de Rafaela': 'Atlético de Rafaela',

    'Racing (Cba.)': 'Racing de Córdoba',
    'Racing (C)': 'Racing de Córdoba',
    'Racing (Córdoba)': 'Racing de Córdoba',
    'Racing(Cba.)': 'Racing de Córdoba',
    'Racing (COR)': 'Racing de Córdoba',

    'Instituto De Córdoba': 'Instituto',
    'Instituto(Cba.)': 'Instituto',
    'Instituto Cba': 'Instituto',
    'Instituto (Córdoba)': 'Instituto',
    'Instituto  (Córdoba)': 'Instituto',
    'Instituto Cba.': 'Instituto',
    'Instituto (Cba)': 'Instituto',
    'Instituto (Cba.)': 'Instituto',

    'Newell\'S Old Boys': 'Newell\'s Old Boys', 
    'Newell´s OB': 'Newell\'s Old Boys', 
    'Newell´s O.B.': 'Newell\'s Old Boys', 
    'Newell\'s O.B.': 'Newell\'s Old Boys', 
    'Newell´s O. B.': 'Newell\'s Old Boys', 
    'Newell\'s O. B.': 'Newell\'s Old Boys', 
    'Newell´s Old Boys': 'Newell\'s Old Boys', 
    'Newell\'s OB': 'Newell\'s Old Boys', 
    'Newells Old Boys': 'Newell\'s Old Boys', 
    'Newell’s Old Boys': 'Newell\'s Old Boys', 

    'Racing De Avellaneda': 'Racing Club (Argentina)',
    'Racing Avellaneda': 'Racing Club (Argentina)',
    'Racing Club (Avellaneda)': 'Racing Club (Argentina)',
    'Racing Club Avellaneda': 'Racing Club (Argentina)',
    'Racing (Argentina)': 'Racing Club de Avellaneda',
    'Racing Club (Argentina)': 'Racing Club de Avellaneda',

    'Sacachisapas': 'Sacachispas Fútbol Club', 
    'Sacachispas': 'Sacachispas Fútbol Club',



    'San Lorenzo - Argentina': 'San Lorenzo de Almagro ',
    'CA San Lorenzo': 'San Lorenzo de Almagro',
    'San Lorenzo del Almagro': 'San Lorenzo de Almagro',
    'CA San Lorenzo Alm.': 'San Lorenzo de Almagro',
    'San Lorenzo Alm.': 'San Lorenzo de Almagro',

    'Altos Hornos Zapla (J)': 'Altos Hornos Zapla (Jujuy)',
    'Altos Hornos Zapla(Juj)': 'Altos Hornos Zapla (Jujuy)',
    'Altos Hornos Zapla(Jujuy)': 'Altos Hornos Zapla (Jujuy)',
    'Altos Hornos Zapla (Jujuy)': 'Altos Hornos Zapla',

    'Cipolletti (Río Negro)': 'Cipolletti',
    'Cipolletti (RN)': 'Cipolletti',
    'Atlético Cipolletti': 'Cipolletti',

    'Alvarado (MdP)': 'Alvarado',
    'Alvarado (Mar del Plata)': 'Alvarado',


    'Independiente Rivadavia (Mza.)': 'Independiente Rivadavia',
    'Ind. Rivadavia': 'Independiente Rivadavia',
    'Ind. Rivadavia (M)': 'Independiente Rivadavia',
    'Independiente Riv.(Mza.)': 'Independiente Rivadavia',
    'Independiente Riv.(Mza)': 'Independiente Rivadavia',
    'Independiente Rivadavia (Mendoza)': 'Independiente Rivadavia',


    'Atlético Ledesma (Juj)': 'Atlético Ledesma',
    'Atlético Ledesma (Jujuy)': 'Atlético Ledesma',
    'Atlético Ledesma(Jujuy)': 'Atlético Ledesma',

    'All Boys (Buenos Aires)': 'All Boys',
    'All Boys (BA)': 'All Boys',

    'Sarmiento (J)': 'Sarmiento',
    'Sarmiento (JNI)': 'Sarmiento',
    'Sarmiento (Junín)': 'Sarmiento',

    'Almirante Brown (IC)': 'Almirante Brown',

    # Brazil
    
    'Fast': 'Fast Clube',

    'Internacional de Limeira': 'Internacional (Limeira)',

    'Sao Raimundo': 'São Raimundo',

    'Palestra Itália': 'Palmeiras',
    'Palestra Italia': 'Palmeiras',
    'EC Palmeiras': 'Palmeiras',
    'Palmeiras - Brazil': 'Palmeiras',
    'SE Palmeiras': 'Palmeiras',

    'O Independência/Sant\'Anna': 'Independência/Sant\'Anna',

    'Portuguesa-SP': 'Portuguesa (Sao Paulo)',
    'Portuguesa (Sao Paulo)': 'Portuguesa',
    'Portuguesa (Brazil)': 'Portuguesa',
    
    'Port. Santista': 'Portuguesa Santista',
    'Portuguesa S': 'Portuguesa Santista',
    'O Port. Santista': 'Portuguesa Santista',
    'Portuguesa Sant': 'Portuguesa Santista',
    'Portuguesa Sa': 'Portuguesa Santista',
    'Portuguesa San': 'Portuguesa Santista',
    'Portuguesa/San': 'Portuguesa Santista',

    'América-TO': 'América (Teófilo Otoni)',
    'América/TO': 'América (Teófilo Otoni)',

    'América/BH': 'América (Belo Horizonte)',

    'Democrata/SL': 'Democrata de Sete Lagoas',
    'Democrata/SL': 'Democrata de Sete Lagoas',

    'Democrata/GV': 'Democrata de Governador Valadares',

    'Mackenzie': 'Mackenzie College',

    'C. Colombo': 'Christovam Colombo',
    'Cristóvão Colombo': 'Christovam Colombo',

    'América (TR)': 'América de Três Rios',
    'América-TR': 'América de Três Rios',


    'Fluminense FC': 'Fluminense',
    'Fluminense-RJ': 'Fluminense',

    'T. do Brasil': 'Tigres do Brasil',

    'Fluminense-BA': 'Fluminense BA',

    'Juventude': 'EC Juventude',

    'Sport Club Internacional': 'Internacional',
    'SC Internacional': 'Internacional',
    'Internacional-RS': 'Internacional',

    'Nacional-AM': 'Nacional AM',

    'Internacional-SP': 'Internacional SP',
    
    'Internacional/SM': 'Internacional SM',



    'Botafogo São Paulo': 'Botafogo (SP)',
    'Botafogo-SP': 'Botafogo (SP)',
    'Botafogo/SP': 'Botafogo (SP)',

    'Botafogo/RJ': 'Botafogo (RJ)',
    'Botafogo FR': 'Botafogo (RJ)',
    'Botafogo-RJ': 'Botafogo (RJ)',
    'Botafogo (RJ)': 'Botafogo',


    'Botafogo-PB': 'Botafogo PB',





    'Atlético-GO': 'Atlético Goianiense',

    'Athletico Mineiro': 'Atlético Mineiro',
    'Atletico Mineiro': 'Atlético Mineiro',
    'Atlético-MG': 'Atlético Mineiro',
    'Atletico-MG': 'Atlético Mineiro',
    'Atlético/MG': 'Atlético Mineiro',
    'Atletico MG': 'Atlético Mineiro',
    'Atletico/MG': 'Atlético Mineiro',

    'Atl. Paranaense': 'Atlético Paranaense',
    'Atlético Parananense': 'Atlético Paranaense',
    'Atletico Paranaense': 'Atlético Paranaense',
    'Atletico-PR': 'Atlético Paranaense',
    'Atlético-PR': 'Atlético Paranaense',
    'Atletico PR': 'Atlético Paranaense',
    'Atletico/PR': 'Atlético Paranaense',
    'Atlético/PR': 'Atlético Paranaense',


    'CSA': 'Centro Sportivo Alagoano',

    'Sampaio Correa': 'Sampaio Corrêa',

    'Criciuma': 'Criciúma',

    'America/RN': 'América de Natal',
    'America-RN': 'América de Natal',
    'America (RN)': 'América de Natal',
    'América-RN': 'América de Natal',

    'America de Rio de Janeiro': 'America Rio',
    'America RJ': 'America Rio',
    'America Rio': 'America (RJ)',
    'America (RJ)': 'América (RJ)',
    'América-RJ': 'América (RJ)',

    'São Paulo-RS': 'Sport Club São Paulo',

    'América (SP)': 'America (Sao Paulo)',
    'América-SP': 'America (Sao Paulo)',
    

    'América Mineiro': 'América Futebol Clube',
    'América-MG': 'América Futebol Clube',
    'America-MG': 'América Futebol Clube',
    'America/MG': 'América Futebol Clube',


    'Avai': 'Avaí FC',
    'Avaí': 'Avaí FC',

    'Bangu Atlético Clube': 'Bangu',
    'Bangu AC': 'Bangu',

    'Bonsucesso (Brazil)': 'Bonsucesso FC',

    'Cruzeiro (Brazil)': 'Cruzeiro',

    'Flamengo (Brazil)': 'Flamengo',
    'CR Flamengo': 'Flamengo',
    'Flamengo-RJ': 'Flamengo',

    'Flamengo-PI': 'Flamengo PI',

    'Villa Nova-MG': 'Villa Nova',

    'Goias': 'Goiás EC',
    'Goiás': 'Goiás EC',
    'Goias EC': 'Goiás EC',
    'EC Goias': 'Goiás EC',

    'Gremio': 'Grêmio',
    'Gremio (Porto Alegre)': 'Grêmio',
    'Macaupa (Brazil)': 'Macapá',
    'Macaupo': 'Macapá',
    'National Fast Club (Brazil)': 'National Fast Club',

    'Oscar\'s FC - Brazil': 'Oscar\'s FC',

    'Parana': 'Paraná',
    'Paraná': 'Paraná Clube',
    'Parana Clube': 'Paraná Clube',

    'AA das Palmeiras': 'AA Palmeiras',
    #'AA Palmeiras': 'AA das Palmeiras',



    'Santos De Brasil': 'Santos FC',
    'Santos (Brazil)': 'Santos FC',
    #'Santos': 'Santos FC',

    'Sao Caetano': 'São Caetano',

    'Sao Paulo FC': 'São Paulo FC',
    'Sao Paulo': 'São Paulo FC',
    'São Paulo': 'São Paulo FC',
    'São Paulo-SP': 'São Paulo FC',

    'São João': 'União São João',
    'Uniao Sao Joao': 'União São João',

    'Desportiva Capixaba': 'Desportiva',

    'Operario-MS': 'Operario MS',
    'Operario MS': 'Operário MS',
    'Operário-MS': 'Operário MS',

    'Operário-MT': 'Operário MT',

    'Sport Club Belém': 'Sport Belém',


    'Sport Recife': 'SC Recife',
    'Sport (Recife)': 'Sport Club do Recife',
    'Sport Club do Recife': 'SC Recife',
    'Recife': 'SC Recife',

    'Ipatinga FC': 'Betim',
    'Ipatinga': 'Betim',

    'Uberlandia (Brazil)': 'Uberlândia Esporte Clube',
    'Uberlandia': 'Uberlândia Esporte Clube',
    
    'Vasco': 'CR Vasco da Gama',
    'Gama': 'CR Vasco da Gama',
    'Vasco da Gama -- Brazil': 'CR Vasco da Gama',
    'Vasco Da Gama': 'CR Vasco da Gama',
    'Vasco da Gama': 'CR Vasco da Gama',

    'Vitoria (Salvador)': 'EC Vitória',

    
    # CONCACAF

    # Suriname
    'Inter Monegotapoe': 'Inter Moengotapoe',
    'Inter Moengotapoe - Surinam': 'Inter Moengotapoe',
    'SV Leo Victor - Surinam': 'SV Leo Victor',
    'Robin Hood': 'S.V. Robinhood',
    'SV Robin Hood': 'S.V. Robinhood',
    'SV Robinhood': 'S.V. Robinhood',

    'SV Transvaal - Suriname': 'SV Transvaal',
    'Transvaal': 'SV Transvaal',
    'SV Transvaal -- Suriname': 'SV Transvaal',
    'Voorwaarts': 'SV Voorwaarts',

    # French Guiana
    'Sport Guyanais': 'ASL Sport Guyanais',

    'ASL Sports': 'ASL Sport Guyanais',

    # Guyana
    'Topp XX': 'Bakewell Topp XX',



    'Alpha United - Guyana': 'Alpha United',

    # Bermuda
    #Vasco da Gama (Bermuda)
    #'Vasco da Gama': 'Vasco da Gama (Bermuda)',

    'Somerset CC': 'Somerset Cricket Club',

    'North Village Red Devils': 'North Village Rams',

    'Dandy Town': 'Dandy Town Hornets',
    'Vasco Da Gama (Bermuda)': 'Vasco da Gama (Bermuda)',
    'Pembroke-Hamilton': 'Pembroke Hamilton Club',
    'Pembroke-Hamilton Club': 'Pembroke Hamilton Club',

    # Aruba
    'RCA': 'Racing Club Aruba',
    'Racing Club Aruba': 'SV Racing Club Aruba',

    # Martinique
    'RC Lorrain': 'Racing Club Lorrain',
    'RC Saint-Joseph': 'Racing Club Saint-Joseph',
    'Olympique (Marin)': 'Olympique du Marin',
    'Rapid Club Lorrain': 'RC Lorrain',
    'CS Case Pilote': 'CS Case-Pilote',
    'Olympique (Ft-de-France)': 'Olympique du Marin',
    'RC Riviere-Pilote': 'RC Rivière-Pilote',

    # Curacao
    'Undeba': 'UNDEBA',
    'Union Dep. Banda\'bou': 'UNDEBA',
    'Victory Boys': 'SV Victory Boys',


    # Guadeloupe
    #Red Star (Pointe-a-Pitre)
    'Gauloise (Basse-Terre)': 'La Gauloise de Basse-Terre',
    'Morne a l\'Eau': 'Etoile de Morne-à-l\'Eau',
    'L\'Etoile de Morne-a-l\'Eau': 'Etoile de Morne-à-l\'Eau',
    'Etoile de Monre-a-l\'Eau': 'Etoile de Morne-à-l\'Eau',
    'Etoile de Morne-a-l\'Eau': 'Etoile de Morne-à-l\'Eau',
    'Etoile de Morne-à-l\'Eau': 'L\'Etoile de Morne-à-l\'Eau',
 
    'Zenith': 'Zénith (Morne-à-l\'Eau)', # dubiuos
    'Zénith (Morne-à-l\'Eau)': 'Zenith (Guadeloupe)',
    

    'Juventus (Saint-Martin)':  'Juventus de Saint-Martin',
    'Juventus de Saint-Anne': 'ASG Juventus de Sainte-Anne',
    'Solidarite Scolair': 'Solidarite Scolaire',

    # Antigua
    'Hoppers FC - Antigua': 'Hoppers FC',

    'HitachiCentre SAP FC - Antigua': 'Hitachi Centre SAP FC',
    'Hitachi Centre SAP FC -Antigua': 'Hitachi Centre SAP FC',

    'Antigua Barricudas FC': 'Antigua Barracuda',
    'Antigua Barracuda': 'Antigua Barracuda FC',

    # Dominica

    'RD Grand Bazaar Dublanc': 'RD Grand Bazaar',

    # Barbados

    'Lambada': 'Lambada FC',

    # Jamaica
    #Cavalier (Jamiaca)
    #St. James (Jamaica)
    #Santos (Jamaica)
    'Regiment (JAM)': 'Regiment (Jamaica)',
    #Kingston (Jamaica)
    #Regiment (Jamaica)

    'Rivoli U.': 'Rivoli United',
    'Rivoli': 'Rivoli United',

    'St. Georges': 'St. George\'s',
    'St George\'s': 'St. George\'s',
    'Saint George\'s': 'St. George\'s',



    'Seba United FC': 'Montego Bay United',

    'Duhaney Park FC': 'Duhaney Park',
    'Wadadah': 'Wadadah FC',
    'Village United FC': 'Village United',

    'Tivoli G.': 'Tivoli Gardens',
    'Tivoli Gardens FC': 'Tivoli Gardens',

    'Savannah FC': 'Savannah SC',
    'Saint George\'s SC': 'St. George\'s SC',

    'Naggo\'s Head': 'Naggo Head',

    'Hazard U.': 'Portmore United',
    'Hazard United': 'Portmore United',
    'Portmore': 'Portmore United',
    'Portmore U.':'Portmore United',
    'Portmore United FC': 'Portmore United',

    'Boys Town FC (Kingston)': 'Boys\' Town F.C.',
    'Boys Town': 'Boys\' Town',
    'Boys\' Town FC': 'Boys\' Town',
    'Boys\' Town F.C.': 'Boys\' Town',

    'August Town FC': 'August Town',

    'Arnett Gardens FC': 'Arnett Gardens',
    'Arnett Gardens-- Jamaica': 'Arnett Gardens',
    'Arnett Gardens F.C.': 'Arnett Gardens',
    'Arnett G.': 'Arnett Gardens',

    'Harbour V.': 'Harbour View',
    'Harbour View FC -- Jamaica': 'Harbour View',
    'Harbour View FC - Jamaica': 'Harbour View',
    'Harbour View FC': 'Harbour View',

    # Haiti
    #Victory Club (Haiti)
    #Victory Sportif (Haiti)

    'Roulado': 'Roulado FC',

    'Valencia FC (Haiti)': 'Valencia (Haiti)',

    # Haiti

    'Tempete - Haiti': 'Tempête FC',
    'Tempete FC': 'Tempête FC',
    'Tempete (Haiti)': 'Tempête FC',
    'Tempête': 'Tempête Football Club',
    'Tempête FC': 'Tempête Football Club',
    'Tempete': 'Tempête Football Club',

    'Racing Des Gonaives - Haiti': 'Racing des Gonaïves',
    'Racing Club de Gonaives': 'Racing des Gonaïves',
    'RC Gonaives': 'Racing des Gonaïves',


    'Etoile Haitienne': 'Etoile Haïtienne',
    'Haitien': 'R.C. Haïtien',

    'Baltimore Sportif Club': 'Baltimore Sportif',

    'Victory Sportif (Haiti)': 'Victory Sportif Club',
    'Victory Club (Haiti)': 'Victory Sportif Club',
    'Racing Club (Haiti)': 'Racing Club Haitien',
    'Racing (Haiti)': 'Racing Club Haitien',
    'Racing Club Haitien': 'R.C. Haïtien',
    'Violette': 'Violette AC',
    
    # Trinidad

    'Tobago U.': 'Tobago United',

    'Jabloteh': 'San Juan Jabloteh',
    'Jabloteh - Trinidad': 'San Juan Jabloteh',
    'CL Financial San Juan Jabloteh': 'San Juan Jabloteh',
    'CL Financial San Juan Jabloteh R.C.': 'San Juan Jabloteh',
    'CLICO San Juan Jabloteh': 'San Juan Jabloteh',

    'AIA Caledonia': 'Caledonia AIA',
    'Caledonia AIA/Fire': 'Caledonia AIA',
    'Caledonia Aia': 'Caledonia AIA',
    'Caledonia F.C.': 'Caledonia AIA',

    'Defence Force (Mucurapo)': 'Defence Force',
    'Defence Force F.C.': 'Defence Force',

    'Arima Fire F.C.': 'Arima Fire',
    'Regiment (TRI)': 'Regiment (Trinidad)',
    #ASL (Trinidad)
    #Regiment (Trinidad)



    'Joe Public - T & T': 'Joe Public',
    'Joe Public - Trinidad': 'Joe Public',
    'Joe Public -- T & T': 'Joe Public',
    'Joe Public': 'Joe Public F.C.',
    'Joe Public FC': 'Joe Public F.C.',
    'San Juan Jabloteh -- T & T': 'San Juan Jabloteh',
    'United Petrotrin -- T & T': 'United Petrotrin',
    'Trintoc': 'United Petrotrin',
    'Trintoc FC': 'United Petrotrin',
    'W Connection FC': 'W Connection',

    'W. Connection - Jamaica': 'W Connection',
    'W. Connection -- Jamaica': 'W Connection',
    'W. Connection FC - Trinidad': 'W Connection',

  
    'Vibe CT 105 W. Connection': 'W Connection',
    'Williams Connection FC': 'W Connection',
    'Vibe CT 105 W. Connection': 'W Connection',
    'Vibe CT 105 W Connection': 'W Connection',
    'Vibe CT 105W Connection F.C.': 'W Connection',
    'Morvant-Arima Fire': 'Arima Fire',
    'Williams Connection': 'W Connection',
    'Police F.C.': 'Police FC',
    'Devonshire Colts FC': 'Devonshire Colts',
    'Doc\'s Khewalaas F.C.': 'Doc\'s Khelwalaas',
    'Neal & Massy Caledonia AIA': 'Caledonia AIA',
    'San Fernando Starworld Strikers': 'South Starworld Strikers',
    'National Quarries North-East Stars': 'North East Stars',
    'Superstar Rangers': 'St Ann\'s Rangers',
    'St Anns Rangers': 'St Ann\'s Rangers',



    # El Salvador

    'AD El Transito': 'ADET',

    'Municipal Limeno': 'CD Municipal Limeño',
    'Municipal Limeño': 'CD Municipal Limeño',
    'Limeño': 'CD Municipal Limeño',
    'CD Municipal Limeno': 'CD Municipal Limeño',

    'CD Vista Hermosa': 'Vista Hermosa',


    'CD Dragon': 'CD Dragón',
    'Dragon': 'CD Dragón',

    'Independiente (El Salvador)': 'Juventud Independiente',
    'Juventud Ind.': 'Juventud Independiente',

    'San Salvador': 'San Salvador FC',

    'Chalatenango': 'CD Chalatenango',


    'Arcense': 'CD Arcense',

    'Univ. de El Salvador': 'Universidad de El Salvador',
    'UES': 'Universidad de El Salvador',

    'Nejapa FC': 'Alacranes Del Norte', #add name map.

    #'Aguila': 'C.D. Águila',
    'CD Aguila': 'C.D. Águila',
    'Aguila - El Salvador': 'CD Aguila',
    'CD Aguila - El Salvador': 'CD Aguila',
    'C.D. Aguila': 'C.D. Águila',
    'CD Aguila': 'C.D. Águila',
    'CD Aguila (El Salvador)': 'C.D. Águila',
    'CD Águila': 'C.D. Águila',

    'Alianza Fc': 'Alianza F.C.',
    'Alianza FC - El Salvador': 'Alianza F.C.',
    'Alianza (El Salvador)': 'Alianza F.C.',
    'Alianza - El Salvador': 'Alianza F.C.',


    'Atletico Balboa': 'Atlético Balboa',
    'Balboa': 'Atlético Balboa',

    'Fas': 'FAS',
    'FAS - El Salvador': 'FAS',
    'C.D. FAS': 'FAS',
    'C.D. Fas': 'FAS',
    'CD Fas': 'FAS',
    'CD FAS': 'FAS',
    'Deportivo FAS': 'FAS',

    'Indep. Nacional 1906': 'Independiente Nacional 1906',
    'Nacional 1906': 'Independiente Nacional 1906',

    'CD Isidro-Metapán': 'Isidro Metapan',
    'A. D. Isidro Metapán': 'Isidro Metapan',
    'Metapan - El Salvador': 'Isidro Metapan',
    'Isidro Metapan - El Salvador': 'Isidro Metapan',
    'Isidro': 'Isidro Metapan',
    'Isidro Metapán': 'CD Isidro Metapán',
    'CD Isidro-Metapan': 'CD Isidro Metapán',
    'CD Isidro (Metapan)': 'CD Isidro Metapán',
    'Isidro-Metapan': 'Isidro Metapán',
    'Isidro Metapan': 'Isidro Metapán',
    'Isidro-Metapán': 'Isidro Metapan',
    'Isidro Metápan': 'Isidro Metapan',

    'Juventud Olimpica': 'Juventud Olímpica',
    
    'Santa Tecla': 'Santa Tecla FC',

    'Firpo': 'Luis Angel Firpo',
    'L.A. Firpo': 'Luis Angel Firpo',
    'Luis Angel Firpo-- El Salvador': 'Luis Angel Firpo',
    'Luis Angel Firpo - El Salvador': 'Luis Angel Firpo',
    'LA Firpo': 'Luis Angel Firpo',
    'FC Firpo': 'Luis Angel Firpo',
    'Luis Angel Firpo': 'Luis Ángel Firpo',
    'Luis Àngel Firpo': 'Luis Ángel Firpo',

    'Veraguense': 'Atlético Veragüense',
    'Veragüense': 'Atlético Veragüense',
    'Atlético Veraguense': 'Atlético Veragüense',
    'Atl. Veragüense': 'Atlético Veragüense',

    #'Guerra y Marina': 'Marte (Mexico)', # name map.
    'Marte (Mexico)': 'CD Marte (Mexico)',    

    # Unclear
    'Marte FC': 'Marte',


    # El Salvador
    'Club Deportivo Marte': 'C.D. Atlético Marte',
    'Atletico Marte': 'C.D. Atlético Marte',
    'C.D. Atletico Marte': 'C.D. Atlético Marte',
    'C.D. Atlético Marte': 'Atlético Marte',

    # Nicaragua 

    'San Marcos Carazo': 'FC San Marcos',

    'Chinandega': 'Chinandega FC',
    
    'Ocotal': 'Deportivo Ocotal',

    'America (Nicaragua)': 'América Managua',
    'America Managua': 'América Managua',

    'Juventus (Nicaragua)': 'Juventus Managua',
    'Deportivo Walter Ferreti': 'Deportivo Walter Ferretti',
    'Walter Ferretti': 'Deportivo Walter Ferretti',
    'Walter Ferreti': 'Deportivo Walter Ferretti',

    'Diriangén': 'Diriangén FC',
    'Diriangen FC': 'Diriangén FC',
    
    'Managua': 'Managua F.C.',
    'Managua FC': 'Managua F.C.',

    'Deportivo Jalapa (Jalapa)': 'Jalapa (Nicaragua)',

    'Parmalat FC (Managua)': 'Parmalat (Nicaragua)',

    'Real Esteli FC - Nicaragua': 'Real Estelí F.C.',
    'Real Estelí': 'Real Estelí F.C.',
    'Real Estelí': 'Real Estelí F.C.',
    'Esteli': 'Real Estelí F.C.',
    'Real Esteli': 'Real Estelí F.C.',
    'Real Estelí': 'Real Estelí F.C.',

    # Panama
    #Projusa (Veraguas)

    'Policía Nacional': 'SD Atlético Nacional',

    'Río Abajo FC': 'Río Abajo',

    'Colón River': 'Colón River FC',



    #'San Francisco': 'San Francisco FC', # ambiguous.
    'San Francisco FC - Panama': 'San Francisco FC',
    'San Francisco F.C.': 'San Francisco FC',

    'Tauro FC - Panama': 'Tauro FC',
    'Tauro F.C.': 'Tauro FC',
    'Tauro': 'Tauro FC',

    'Chepo': 'Chepo FC',

    # Belize
    #Boca Juniors (S. Ignacio)
    'Juventus (Orange Walk)': 'Juventus (Belize)',

    'Boca Juniors (S. Ignacio)': 'Boca FC',
    
    'Acros': 'Acros Crystal',

    'Hankok Verdes': 'Club Verdes',
    'Hankook Verdes': 'Club Verdes',
    'Hankook Verdes United': 'Club Verdes',
    'Leslie Verdes': 'Club Verdes',
    'Real Verdes': 'Club Verdes',
    'Verdes FC': 'Club Verdes',

    # Guatemala


    'Peñarol La Mesilla': 'Halcones', # name map
    'Peñarol LM': 'Halcones', # name map
    'Halcones LM': 'Halcones', 
    'Halcones La Mesilla': 'Halcones', 

    'Iztapa': 'Deportivo Iztapa',

    'Fundazucar': 'Azucareros',
    'Deportivo Izabal JC': 'Izabal JC',
    'Izabal': 'Izabal JC',

    'Petapa': 'Deportivo Petapa',
    
    'Deportivo Teculutan': 'Deportivo Teculután',
    'Teculutan': 'Deportivo Teculután',

    'Amatitlan': 'Amatitlán',

    'Deportivo Carcha': 'Deportivo Carchá',
    'Carchá': 'Deportivo Carchá',
    'Dep. Carcha': 'Deportivo Carchá',
    'Dep. Carchá': 'Deportivo Carchá',
    'Carcha': 'Deportivo Carchá',

    'Juv. Retalteca': 'Juventud Retalteca',
    'Juventud R.': 'Juventud Retalteca',

    'Heredia-Jaguares': 'Heredia Jaguares',
    'Deportivo Heredia': 'Heredia Jaguares de Peten',
    'Heredia Jaguares': 'Heredia Jaguares de Peten',

    'Aurora': 'Aurora F.C.',

    'Coban - Guatemala': 'Coban',
    'Coban': 'Cobán Imperial',
    'Cobán': 'Cobán Imperial',
    'Coban Imperial': 'Cobán Imperial',
    
    'CSD Comunicaciones - Guatemala': 'CSD Comunicaciones',
    'Comunicaciones': 'C.S.D. Comunicaciones',
    'Comunciaciones': 'C.S.D. Comunicaciones',
    'Communicaciones': 'C.S.D. Comunicaciones',
    'C.S.D. Comunicaciones': 'CSD Comunicaciones',
    'Comunicaciones - Guatemala': 'CSD Comunicaciones',
    'CSD Comunicaciones-- Guatemala': 'CSD Comunicaciones',


    'Escuintla': 'Deportivo Escuintla',

    'Jalapa': 'CD Jalapa',
    'Deportivo Jalapa': 'CD Jalapa',

    'Galcasa': 'CSD Galcasa',

    'Municipal - Guatemala': 'CSD Municipal',
    'Municipal': 'CSD Municipal',
    'CSD Municipal -- Guatemala': 'CSD Municipal',

    'Suchitepequez - Guatemala': 'Suchitepequez',
    'Suchitepequez': 'C.D. Suchitepéquez',
    'Suchitepéquez': 'C.D. Suchitepéquez',
    'Deportivo Suchitepequez': 'C.D. Suchitepéquez',
    'Deportivo Suchitepéquez': 'C.D. Suchitepéquez',



    'Xelaju': 'Xelajú MC',
    'Xelajú': 'Xelajú MC',
    'Xelaju MC': 'Xelajú MC',
    'Zacapa': 'Deportivo Zacapa',

    # Honduras
    
    'Victoria (La Ceiba)': 'CD Victoria',
    'Victoria Honduras': 'CD Victoria',
    'Victoria (Honduras)': 'CD Victoria',

    'Olanchano': 'Atlético Olanchano',

    #Atletico Indio (Honduras)

    'Marathon - Honduras': 'CD Marathón',
    'C. D. Marathón': 'CD Marathón',
    'Marathon': 'CD Marathón',
    'Marathón': 'CD Marathón',
    'CD Marathon': 'CD Marathón',
    'Marathón (San Pedro Sula)': 'CD Marathón',

    'Motagua (Tegucigalpa)': 'CD Motagua',
    'Motagua - Honduras': 'CD Motagua',
    'Motagua': 'CD Motagua',
    'Motagua -- Honduras': 'Motagua',
    'Motagua': 'CD Motagua',


    'Olimpia (Tegucigalpa)': 'CD Olimpia',
    'Olimpia - Honduras': 'CD Olimpia',
    'Olimpia (Honduras)': 'CD Olimpia',
    'Olympia - Honduras': 'CD Olimpia',
    'CD Olympia-- Honduras': 'CD Olimpia',
    'Olimpia -- Honduras': 'Olimpia',
    'Olímpia (H)': 'CD Olimpia',

    'Universidad (Honduras)': 'Pumas UNAH',

    'Platense (Honduras)': 'Platense F.C.',
    'CD Platense': 'Platense FC',
    'Platense F.C.': 'Platense FC',
    'Platense (Puerto Cortés)': 'Platense FC',
    #'Platense': 'Platense F.C.', ambiguous?

    'Real España (San Pedro Sula)': 'Real CD Espana',
    'Real Espana - Honduras': 'Real CD Espana',
    'Real Espana-- Honduras': 'Real CD Espana',
    'Real CD Espana': 'Real C.D. España',
    # 'Real Espana': 'Real C.D. España', ambiguous?

    'Parrillas One (San Pedro Sula)': 'Parrillas One',

    # Costa Rica

    'A.D. Sagrada Familia': 'Sagrada Familia',

    'Osa': 'Municipal Osa',

    'UCR': 'Universidad de Costa Rica',
    'Univ. de Costa Rica': 'Universidad de Costa Rica',

    'P.Zeledon': 'Pérez Zeledón',
    'Perez Zeledon': 'Pérez Zeledón',
    'AD Municipal Pérez Zeledón': 'Perez Zeledon',
    'ADM Perez Zeledon': 'Perez Zeledon',
    'Perez Zeledón': 'Perez Zeledon',
    'Pérez Zeledon': 'Perez Zeledon',
    'Peréz Zeledón': 'Perez Zeledon',
    'Peréz Zeledón': 'Perez Zeledon',

    #Uruguay (Coronado)
    'Alajuelense - Costa Rica': 'Alajuelense',
    'Alajuela': 'Alajuelense',
    'LD Alajuelense-- Costa Rica': 'Alajuelense',
    'Alajuela -- Costa Rica': 'Alajuelense',
    'L.D. Alajuelense': 'Alajuelense',
    'LD Alajuelense': 'Alajuelense',

    'Brujas': 'Brujas FC',

    'Herediano - Costa Rico': 'CS Herediano',
    'C.S. Herediano': 'CS Herediano',
    'Herediano': 'C.S. Herediano',
    'Herediano - Costa Rica': 'Herediano',

    'Punarenas FC - Costa Rica': 'Puntarenas FC',
    'Puntarenas FC - Costa Rica': 'Puntarenas FC',
    'Municipal Puntarenas': 'Puntarenas FC',
    'Puntarenas - Costa Rica': 'Puntarenas',
    'Puntarenas': 'Puntarenas FC',

    'San Lorenzo - Costa Rica': 'San Lorenzo Costa Rica',

    'Santa Teresita FC -Costa Rica': 'Santa Teresita FC',

    'Saprissa - Costa Rica': 'Saprissa',
    'Deportivo Saprissa -Costa Rica': 'Saprissa',
    'CD Saprissa-- Costa Rica': 'Saprissa',
    'Deportivo Saprissa S.A.D.': 'Saprissa',
    'Deportivo Saprissa': 'Saprissa',

    # Mexico

    'Atlante FC - Mexico': 'Atlante',
    'Atlante CF': 'Atlante',
    'Club Atlante': 'Atlante',


    'Guerreros De Tabasco': 'Guerreros de Tabasco',

    'Lobos Buap': 'Lobos de la BUAP',
    'Lobos BUAP': 'Lobos de la BUAP',

    'Club San Luis': 'San Luis FC',
    'CD Veracruz': 'Tiburones Rojos de Veracruz',
    'Vera Cruz': 'Tiburones Rojos de Veracruz',


    'TM Gallos Blancos': 'Querétaro FC',

    'Club Tecos U.A.G':  'Tecos',
    'Estudiantes Tecos': 'Tecos',
    'Estudiantes UAG': 'Estudiantes Tecos',


    'Chiapas Fc': 'Chiapas FC',

    'Jaguares Chiapas': 'Jaguares de Chiapas',
    'Chiapas': 'Jaguares de Chiapas',
    'Jaguares - Mexico': 'Jaguares de Chiapas',
    'Jaguares': 'Jaguares de Chiapas',

    'Cobras Cd. Juárez': 'Cobras de Ciudad Juárez',
    'Cobras': 'Cobras de Ciudad Juárez',
    'Cobras Ciudad Juarez': 'Cobras de Ciudad Juárez',
    'Cobras Queretaro': 'Cobras de Ciudad Juárez',


    'Dorados Sinaloa': 'Dorados de Sinaloa',
    'Dorados': 'Dorados de Sinaloa',

    #Mirador (Mexico)
    #Gray Bears (Mexico)

    'Centro Union': 'Club América', # name map

    'América-Méx': 'Club América',
    'Club America': 'Club América',
    'Club America - Mexico': 'Club América',
    'Club America -- Mexico': 'Club América',

    'Asturias': 'CF Asturias',
    'Club Asturias': 'CF Asturias',

    'Real Club Espana': 'Real Club España',

    'Atlante - Mexico': 'Atlante',
    'Atlante FC': 'Atlante',
    'Atlante F.C.': 'Atlante',

    'CD Atlas': 'Atlas',
    'Atlas - Mexico': 'Atlas',
    'Deportivo Atlas': 'Atlas',
    'Club Atlas': 'Atlas',
    'CA Atlas': 'Atlas',
    'F.C. Atlas': 'Atlas',

    'Atlas Academicos - Mexico': 'Atlas Academicos',

    'Atletico Espanol': 'Atlético Español',

    'A. Potosinas': 'Atlético Potosino',
    'Potosino': 'Atlético Potosino',
    'Atletico Potosino': 'Atlético Potosino',
    'A.Potosino': 'Atlético Potosino',
    'A. Potosino': 'Atlético Potosino',

    'Bachilleres - Mexico': 'Bachilleres',
    'Bachilleres (Mexico)': 'Bachilleres',
    'Bachillieres (Mexico)': 'Bachilleres',

    'Club Celaya': 'Celaya',
    'Celaya FC': 'Celaya',

    'Colibries': 'Colibríes de Morelos',
    'Colibríes': 'Colibríes de Morelos',

    'Colima Jaguares (Mexico)': 'Colima Jaguares',
    'Colima Jaguares - Mexico': 'Colima Jaguares',

    'CD Cruz Azul': 'Cruz Azul',
    'Cruz Azul (Mexico)': 'Cruz Azul',
    'Cruz Azul - Mexico': 'Cruz Azul',
    'Cruz Azul -- Mexico': 'Cruz Azul',

    'Indios': 'Indios de Ciudad Juárez',
    'Indios de Ciudad Juarez': 'Indios de Ciudad Juárez',

    'Club Irapuato': 'CD Irapuato',
    'C.D. Irapuato': 'CD Irapuato',
    'Deportivo Irapuato': 'CD Irapuato',
    'CD Irapuato': 'Irapuato',

    'CD Guadalajara - Mexico': 'CD Guadalajara',
    'CD Guadalajara -- Mexico': 'CD Guadalajara',
    'CD Guadalajara Chivas - Mexico': 'CD Guadalajara',
    'Chivas -- Mexico': 'Chivas',
    'C.D. Guadalajara Chivas': 'CD Guadalajara',
    'Guadalajara': 'CD Guadalajara',
    'Chivas Guadalajara': 'CD Guadalajara',
    'C.D. Guadalajara': 'CD Guadalajara',
    'Chivas': 'CD Guadalajara',
    'Guadalajara (Mexico)': 'CD Guadalajara',

    'Gallos de Jalisco - Mexico': 'Gallos de Jalisco',
    'Gallos de Jalisco': 'Club Jalisco',
    'Jalisco - Mexico': 'Jalisco',
    'Jalisco': 'Club Jalisco',

    'Mexico Cricket': 'Mexico Cricket Club',

    'León': 'Club León',
    'Leon': 'Club León',
    'Club Leon': 'Club León',
    'CD León': 'Club León',
    'Leon -- Mexico': 'Club León',

    'México Country Club': 'México Cricket Club',
    'Mexico Cricket Club': 'México Cricket Club',

    'Morelia - Mexico': 'Morelia',
    'Morelia -- Mexico': 'Morelia',
    'Morelia': 'Monarcas Morelia',
    'Atletico Morelia': 'Monarcas Morelia',
    'Athletico Morelia': 'Monarcas Morelia',
    'Athlético Morelia': 'Monarcas Morelia',
    'Athlético Morelia': 'Monarcas Morelia',
    'Atlético Morelia': 'Monarcas Morelia',
    'CA Monarcas Morelia': 'Monarcas Morelia',

    #'Monterrey': 'CF Monterrey',
    'Monterrey - Mexico': 'CF Monterrey',
    'Monterrey (Mexico)': 'CF Monterrey',
    'C.F. Monterrey': 'CF Monterrey',
    'Club de Fútbol Monterrey Rayados, A.C.': 'CF Monterrey',
    'Club Tigres': 'UANL',
    'Chivas de Guadalajara': 'CD Guadalajara',
    'Club Santos Laguna': 'Santos Laguna',
    'Club Puebla': 'Puebla',
    'Atlas FC': 'Atlas',
    'Toluca FC': 'Toluca',
    'Club de Fútbol Pachuca': 'Pachuca',
    'Atlético de San Luis': 'Atlético San Luis',
    'Liga Deportiva Alajuelense': 'Alajuelense',
    'Club Deportivo Olimpia': 'CD Olimpia',
    'Real Club Deportivo España': 'Real C.D. España',
    'Club Sport Cartaginés Deportiva S.A.': 'Cartaginés',
    'Club Social y Deportivo Xelajú Mario Camposeco': 'Xelajú MC',
    'Academia de Futbol Sporting San Miguelito': 'Sporting San Miguelito',
    'Defence Force Football Club': 'Defence Force',
    'Club Monterrey': 'CF Monterrey',

    'Necaxa -- Mexico': 'Necaxa',
    'Necaxa - Mexico': 'Necaxa',
    'Necaxa (Mexico)': 'Necaxa',
    'Club Necaxa': 'Necaxa',

    'Deportivo Oro': 'C.D. Oro',
    'Oro - Mexico': 'CD Oro',
    'Oro': 'C.D. Oro',
    'C.D. Oro': 'CD Oro',
    
    'Club Querétaro': 'Querétaro FC', # right?
    'Queretaro': 'Querétaro FC',
    'Querétaro': 'Querétaro FC',
    'Queretaro FC': 'Querétaro FC',

    # Actually separate teams?

    'Atletas Campesinos': 'Querétaro FC',
    'Campesinos': 'Querétaro FC',
    'Gallos Blancos': 'Querétaro FC',

    #'Cobras de Queretaro': 'Querétaro FC',

    'CF Pachuca': 'Pachuca',
    'Pachuca - Mexico': 'Pachuca',
    'A.Pachuca': 'Pachuca',
    'Club Pachuca': 'Pachuca',
    'Pachuca FC': 'Pachuca',
    'Pachuca-- Mexico': 'Pachuca',
    'Pachuca CF': 'C.F. Pachuca',
    'Pachuca AC': 'C.F. Pachuca',
    'C.F. Pachuca': 'Pachuca',

    'Puebla FC - Mexico': 'Puebla',
    'Puebla - Mexico': 'Puebla',
    'Puebla F.C.': 'Puebla',
    'Puebla FC': 'Puebla',
    'Puebla AC': 'Puebla',

    'Rovers FC (Mexico)': 'Rovers FC Mexico',

    'San Luis': 'San Luis FC',
    'San Luis F.C.': 'San Luis FC',

    'Laguna F.C.': 'Laguna FC',
    'Laguna': 'Laguna FC',

    'Santos - Mexico': 'Santos Laguna',
    'Santos Laguna - Mexico': 'Santos Laguna',
    'Santos -- Mexico': 'Santos Laguna',

    'Club Sol - Mexico': 'Club Sol',

    'Tampico': 'Tampico Madero FC',
    'Tampico-Madero': 'Tampico Madero FC',
    'Tampico Madero': 'Tampico Madero FC',

    'Tapatio': 'CD Tapatio',
    'Tapatios': 'CD Tapatio',
    'Tapatio - Mexico': 'CD Tapatio',
    'Tapatillo': 'CD Tapatio',
    'CD Tapatio': 'CD Tapatío',

    'UAG - Mexico': 'Tecos',
    'Tecos - Mexico': 'Tecos',
    'U.A. de G.': 'Tecos',
    'UAG': 'Tecos',
    'U.A.G.': 'Tecos',
    'UAG Tecos': 'Tecos',
    'Tecos U.A.G.': 'Tecos',
    'Tecos UAG': 'Tecos',

    'Tijuana': 'Club Tijuana',
    'Tijuana Chivas - Mexico': 'Tijuana Chivas',
    'Nacional de Tijuana - Mexico': 'Nacional de Tijuana',

    'Nacional (Mexico)': 'Nacional de Guadalajara',
    'Club Deportivo Nacional': 'Nacional de Guadalajara',

    'CD Toluca': 'Toluca',
    'Toluca -- Mexico': 'Toluca',
    'Toluca - Mexico': 'Toluca',
    'Deportivo Toluca': 'Toluca',

    'Torreon': 'Torreón FC',
    'Torreon FC': 'Torreón FC',

    'Leones Negros': 'Universidad de Guadalajara',
    'Univ. de Guadalajara - Mexico': 'Universidad de Guadalajara',
    'Univ. de Guadalajara': 'Universidad de Guadalajara',
    'Univ Guadalajara': 'Universidad de Guadalajara',
    'Universidad Guadalajara': 'Universidad de Guadalajara',
    'U.de G.': 'Universidad de Guadalajara',
    'U de G': 'Universidad de Guadalajara',
    'U Guadalajara': 'Universidad de Guadalajara',

    'Tigres UANL - Mexico': 'UANL',
    'Club Tigres - Mexico': 'UANL',
    'Tigres': 'UANL',
    'U.A.N.L': 'UANL',
    'U.A.N.L.': 'UANL',
    'Tigres UANL': 'UANL',
    'UANL Tigres': 'UANL',
    'Tigres de la UANL': 'UANL',

    'Pumas Unam': 'UNAM',
    'UNAM Pumas - Mexico': 'UNAM',
    'UNAM - Mexico': 'UNAM',
    'UNAM -- Mexico': 'UNAM',
    'Unam Pumas': 'UNAM',
    'Pumas': 'UNAM',
    'Pumas UNAM': 'UNAM',
    'UNAM Pumas': 'UNAM',
    'U.N.A.M.': 'UNAM',
    'Pumas U.N.A.M.': 'UNAM',
    'Universidad Nacional': 'UNAM', # Bad name.
    'Universidad (Mexico)': 'UNAM',


    'Club Deportivo Veracruz': 'Veracruz',
    'Veracruz - Mexico': 'Veracruz',
    'Veracruz (Mexico)': 'Veracruz',
    'Verarcuz': 'Veracruz',
    'Tiburones Rojos de Veracruz': 'Veracruz',

    'Correcaminos': 'Correcaminos UAT',
    'UAT': 'Correcaminos UAT',
    'U.A.T.': 'Correcaminos UAT',

    'CD Zacatepec': 'Zacatepec',
    'Zacatepec 1948': 'Zacatepec',
}
